...
static cmd_t wl_msch_cmds[] = {
	...
	"\tUsage: msch_req <id> -c \"channel_list\" -f flags -t type [params] -p priority"
	" -s start_time -d duration -i interval"
...
static void wl_msch_dump_data(char *data, int type)
{
	...
	case WLC_PROFILER_REQ:
	{
		...
		MSCH_EVENTS_PRINTF("\n===============================\n");
		MSCH_EVENTS_PRINTF2("%06d.%06d REGISTER:\n", s, ss);
	...
	case WLC_PROFILER_CALLBACK:
	{
		...
		MSCH_EVENTS_PRINTF2("%06d.%06d CALLBACK: ", s, ss);
	...
	case WLC_PROFILER_MESSAGE:
	{
		...
		MSCH_EVENTS_PRINTF3("%06d.%06d MESSAGE: %s", s, ss, p->message);
		lastMessages = TRUE;
		break;
	}
	...
	case WLC_PROFILER_PROFILE_START:
		...
		MSCH_EVENTS_PRINTF("-------------------------------\n");
		MSCH_EVENTS_PRINTF2("%06d.%06d PROFILE DATA:\n", s, ss);
		wl_msch_profiler_profiler_data(4, data, 0);
		break;

	case WLC_PROFILER_PROFILE_END:
		MSCH_EVENTS_PRINTF3("%06d.%06d PROFILE END: take time %d\n", s, ss,
			(uint32)(tt - profiler_start_time));
		MSCH_EVENTS_PRINTF("-------------------------------\n\n");
		break;
	...
	default:
		fprintf(stderr, "ERROR: unsupported EVENT reason code:%d; ",
			type);
		break;
	}
}
