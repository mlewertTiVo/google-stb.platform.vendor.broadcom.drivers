 ...
 *
 * $Id: wlc_tx.c 613207 2016-01-18 06:00:27Z $
 *
 */
...
wlc_txq_delq_flush(void *ctx)
{
	...
	pktqflush(txqi->osh, txqi->delq, TRUE);
}
...
wlc_delq_scb_free(wlc_info_t *wlc, struct scb *scb)
{
	...
	pktqfilter(txqi->delq, wlc_delq_scb_free_filter, scb,
...
wlc_low_txq_scb_flush(wlc_info_t *wlc, wlc_txq_info_t *qi, struct scb *remove)
{
	...
	struct pktq *free_q = wlc_low_txq(qi->low_txq);
	...
	for (fifo_idx = 0; fifo_idx < qi->low_txq->nfifo; fifo_idx++) {
		...
		wlc_txq_pktq_pfilter(wlc, free_q, fifo_idx, wlc_low_txq_filter, &info);
...
wlc_low_txq_flush(txq_info_t *txqi, txq_t* txq)
{
	...
	for (fifo_idx = 0; fifo_idx < txq->nfifo; fifo_idx++) {
		...
		wlc_txq_pktq_pflush(txqi->wlc, &txq->q, fifo_idx);
...
wlc_txq_fill(txq_info_t *txqi, txq_t *dest_txq)
{
	...
	for (fifo_idx = 0; fifo_idx < TXQ_MAXFIFO; fifo_idx++) {
		/* skip fill from above if this fifo is stopped */
		if (!txq_stopped(dest_txq, fifo_idx)) {
#ifdef WLCNT
			...
			/* Ask feed for pkts to fill available time */
			/* CPU INTENSIVE. Callback function. Calls e.g. wlc_pull_q(). */
			added_time = dest_txq->supply(dest_txq->supply_ctx, fifo_idx, fill_space,
				&temp_q);

			/* total accumulation counters update */
			WLCNTADD(cnt->pkts, temp_q.len);
			WLCNTADD(cnt->time, added_time);
			...
			pktq_append(dest_q, fifo_idx, &temp_q);
	...
	for (fifo_idx = 0; fifo_idx < NFIFO; fifo_idx ++) {
		...
		if (!txq_hw_stopped(dest_txq, fifo_idx)) {
...
#endif /* TXQ_MUX */
{
	...
	if (cfg->assoc->state == AS_WAIT_TX_DRAIN &&
		...
		wlc_join_BSS(cfg, asc->join_targets->ptrs[asc->join_targets_last]);
	}
#endif	/* STA */
} /* wlc_txfifo_complete */
	...
	const wlc_key_info_t *key_info, ratespec_t rspec_override)
{
	...
	for (k = 0; k < cur_rate.num; k++) {
		...
		if ((pkttag->flags & WLF_RATE_AUTO) && LPC_ENAB(wlc) && k == 0) {
		...
		if (PROXD_ENAB(wlc->pub) && wlc_proxd_frame(wlc, pkttag)) {
			...
			wlc_proxd_tx_conf_subband(wlc, &phyctl, pkttag);
		}
#endif
		rate_hdr->PhyTxControlWord_1 = htol16(phyctl);

		/* PhyTxControlWord_2 */
		phyctl = wlc_acphy_txctl2_calc(wlc, rspec, 0);
...
wlc_excursion_start(wlc_info_t *wlc)
{
	...
	if (wlc->excursion_queue == wlc->active_queue) {
		WL_MQ(("MQ: %s: same queue %p\n", __FUNCTION__, wlc->excursion_queue));
		return;
	}
...
wlc_excursion_end(wlc_info_t *wlc)
{
	...
	WL_MQ(("MQ: %s: %p\n", __FUNCTION__, wlc->primary_queue));
	...
	uint8 flag, uint16* time_adj)
{
	...
	while ((pkt = pktq_pdeq(pq, prec))) {
		...
		if (flag == SYNCFIFO) {
#ifdef PROP_TXSTATUS
			if (wlc_proptxstatus_process_pkt(wlc, pkt, time_adj)) {
 ...
 */
static void
wlc_tx_fifo_epoch_save(wlc_info_t *wlc, struct pktq *pq, int prec)
{
	ASSERT(wlc->txfifo_detach_transition_queue != NULL);
	ASSERT(pq != NULL);

	if (wlc->txfifo_detach_transition_queue && pq) {
		void *pkt;

		ASSERT(prec >= 0 && prec < pq->num_prec);

		if ((pkt = pq->q[prec].tail)) {
			wlc_txh_info_t txh_info;
			wlc_get_txh_info(wlc, pkt, &txh_info);
	...
	for (i = 0, fbmp = fifo_bitmap; fbmp; i++, fbmp = fbmp >> 1) {
		...
		*/
		if (AMPDU_AQM_ENAB(wlc->pub)) {
			wlc_tx_fifo_epoch_save(wlc, &low_txq->q, i);
		}
#endif /* WLAMPDU_MAC */
