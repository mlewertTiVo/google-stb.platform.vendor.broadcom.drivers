 ...
 *
 * $Id: wlc.c 613589 2016-01-19 10:34:22Z $
 */
...
#define PRETBTT_ACPHY_4349_US           200             /* 4349 pretbtt time in us */
...
static const bcm_iovar_t wlc_iovars[] = {
	...
	(IOVF_OPEN_ALLOW), IOVT_BUFFER, OFFSETOF(wl_subcnt_info_t, data)
	},
	...
	(IOVF_OPEN_ALLOW), IOVT_BUFFER, sizeof(wl_rxfifo_cnt_t)},
	{"bssnmode", IOV_BSS_NMODE, (0), IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0}
};
...
static int wlc_iovar_block_get(wlc_info_t *wlc, void *arg, int len, int name_bytes,
	int start_var, int out_max);
...
static int wlc_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif);
...
BCMATTACHFN(wlc_attach_module_pre)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(iocvi, wlc_iocv_high_attach, 10);
...
BCMATTACHFN(wlc_attach_module)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(qosi, wlc_qos_attach, 13);
#ifdef WLOLPC
...
#endif
#if defined(WLTDLS) && !defined(WLTDLS_DISABLED)
	MODULE_ATTACH(tdls, wlc_tdls_attach, 65);
#endif /* defined(WLTDLS) && !defined(WLTDLS_DISABLED) */
	...
	MODULE_ATTACH(as, wlc_assoc_attach, 159);
...
BCMATTACHFN(wlc_attach_cb_init)(wlc_info_t *wlc)
{
	...
	if (wlc_module_add_ioctl_fn(wlc->pub, wlc, wlc_doioctl, 0, NULL)) {
...
{
#ifdef WL_RANDMAC
	MODULE_DETACH(randmac_info, wlc_randmac_detach);
#endif /* WL_RANDMAC */
	wlc_dump_detach(wlc->dumpi);

#if defined(WL_ASSOC_MGR) && !defined(WL_ASSOC_MGR_DISABLED)
	MODULE_DETACH(assoc_mgr, wlc_assoc_mgr_detach);
	wlc->pub->_assoc_mgr = FALSE;
#endif /* defined(WL_ASSOC_MGR) && !defined(WL_ASSOC_MGR_DISABLED) */
#if defined(SRHWVSDB) && !defined(SRHWVSDB_DISABLED)
	MODULE_DETACH(srvsdb_info, wlc_srvsdb_detach);
#endif
	MODULE_DETACH(testi, wlc_test_detach);
	MODULE_DETACH(pui, wlc_perf_utils_detach);
	wlc_addrmatch_detach(wlc);

	MODULE_DETACH(sui, wlc_scan_utils_detach);
	MODULE_DETACH(rspeci, wlc_rspec_detach);
	MODULE_DETACH(qosi, wlc_qos_detach);
	MODULE_DETACH(pm, wlc_pm_detach);
	MODULE_DETACH(as, wlc_assoc_detach);
	MODULE_DETACH(frag, wlc_frag_detach);
#if defined(PROP_TXSTATUS) && defined(PROP_TXSTATUS_ENABLED)
	MODULE_DETACH(wlfc, wlc_wlfc_detach);
#endif
#if defined(PKTC) || defined(PKTC_DONGLE)
	MODULE_DETACH(pktc_info, wlc_pktc_detach);
#endif
	...
	MODULE_DETACH(wlctsmap, wlc_tsmap_detach);
#endif
#if defined(MFP) && !defined(MFP_DISABLED)
...
#endif
#if defined(WLC_NDIS5IOVAR)
	MODULE_DETACH(ndis, wlc_ndis_detach);
#endif
	wlc_stf_detach(wlc);
...
#if defined(WLMESH) && !defined(WLMESH_DISABLED)
	MODULE_DETACH(mesh_info, wlc_mesh_detach);
#endif /* WLMESH && !WLMESH_DISABLED) */

#if defined(BCMULP) && !defined(WLULP_DISABLED)
	MODULE_DETACH(ulp, wlc_ulp_detach);
#endif
...
#if defined(WL_HEALTH_CHECK)
	MODULE_DETACH(hc, wlc_health_check_detach);
#endif

#ifdef WIFI_ACT_FRAME
	MODULE_DETACH(wlc_act_frame_info, wlc_act_frame_detach);
#endif
...
BCMATTACHFN(wlc_detach)(wlc_info_t *wlc)
{
	...
	wlc_dump_post_detach(wlc->dumpi);
...
BCMINITFN(wlc_up)(wlc_info_t *wlc)
{
	...
	*/
	...
	if (D11REV_GE(wlc->pub->corerev, 40))
		wlc_write_shm(wlc, M_BCMC_TIMEOUT(wlc), (uint16)wlc->pm_bcmc_wait);

	WL_MPC(("wl%d: UP\n", wlc->pub->unit));

#if defined(WL_PROXDETECT)
	if (AWDL_SUPPORT(wlc->pub) || PROXD_ENAB(wlc->pub))
		wlc_ref_tick_us_init(wlc);
#endif
...
wlc_change_band(wlc_info_t *wlc, int band)
{
	...
	if (CHSPEC_WLCBANDUNIT(wlc->chanspec) != band) {
		/* switch to first channel in the new band */
...
wlc_ioctl(wlc_info_t *wlc, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	WL_TRACE(("wl%d: wlc_ioctl: interface %s cmd %d arg %p len %d\n",
	          wlc->pub->unit, wl_ifname(wlc->wl, wlif), cmd, arg, len));
...
static int
wlc_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_IOV_BLOCK_GET: {
		 ...
		 */
		bcmerror = wlc_iovar_block_get(wlc, arg, len, ((int *)arg)[0], /* max name len */
					       ((int *)arg)[1], /* variable start index */
					       ((int *)arg)[2]);  /* Max vars out */
		break;
	}
	...
	case WLC_GET_BSS_INFO: {
		...
		current_bss->phy_noise = wlc_lq_noise_ma_upd(wlc, wlc_phy_noise_avg(pi));
 ...
 * Params:
 *     arg -- output buffer; held name_bytes, var_start and out_max
 *     len -- number of bytes available in arg
 ...
 */
static int
wlc_iovar_block_get(wlc_info_t *wlc, void *arg, int len, int name_bytes, int var_start,
	int out_max)
{
	int mod_idx, iov_mod_idx = 0;
	const bcm_iovar_t *var = NULL;
	char *out_p;
	int skip_count = 0;   /* Variables skipped to get to start */
	int out_count = 0;    /* Variables transfered */
	int iovar_bank;
	int max_iovar_bank = 1;
#if defined(WLC_PATCH_IOCTL)
	max_iovar_bank = 2;
#endif	/* WLC_PATCH_IOCTL */
	...
	for (mod_idx = 0; mod_idx < wlc->pub->max_modules && out_count < out_max; mod_idx++) {
		for (iovar_bank = 0; iovar_bank < max_iovar_bank; iovar_bank++) {
			if (iovar_bank == 0) {
				var = wlc->modulecb[mod_idx].iovars;
			}
#if defined(WLC_PATCH_IOCTL)
			else {
				var = wlc->modulecb[mod_idx].patch.iovars;
			}
#endif	/* WLC_PATCH_IOCTL */
			for (; (var != NULL) && (var->name != NULL); var++) {
				...
				*out_p++ = (char)iov_mod_idx;   /* Get module */
				*out_p++ = (char)(var->type);   /* Get variable type */
				strncpy(out_p, var->name, name_bytes);   /* Get variable name */
				out_p[name_bytes - 1] = '\0'; /* In case var->name too long */
				out_p += name_bytes;   /* Advance to next place in output buffer */
				if (++out_count >= out_max) {   /* All done? */
					break;
				}
			}
		}
	...
	if (!iocv) {
...
#endif /* WLRSDB */
		{
			WL_NONE(("wl%d: %s: calling IOVAR callback for \"%s\", name %s\n",
				WLCWLUNIT(wlc), __FUNCTION__, wlc->modulecb[i].name, name));
...
exit:
	...
	if (err && VALID_BCMERROR(err)) {
		...
		WL_ERROR(("wl%d: %s: BCME %d: %s\n", WLCWLUNIT(wlc), __FUNCTION__, err, name));
	}
...
wlc_dump_txstuck(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "wlc active q: %d, PM pending %d\n",
		pktq_len(WLC_GET_TXQ(wlc->active_queue)),
		lc->PMpending);
 ...
 */
bool BCMFASTPATH
_wlc_prec_enq(wlc_info_t *wlc, struct pktq *q, void *pkt, int prec, const char *func)
{
	return _wlc_prec_enq_head(wlc, q, pkt, prec, FALSE, func);
}

bool BCMFASTPATH
_wlc_prec_enq_head(wlc_info_t *wlc, struct pktq *q, void *pkt, int prec, bool head,
	const char *func)
{
	void *p;
	void *dqp = NULL;
	...
	if (head)
		p = _pktq_penq_head(q, prec, pkt, func);
	else
		p = _pktq_penq(q, prec, pkt, func);
	if (p == NULL)
...
wlc_do_chanswitch(wlc_bsscfg_t *cfg, chanspec_t newchspec)
{
	...
	if ((BSSCFG_NAN(cfg) || BSSCFG_AWDL(wlc, cfg) ||
...
/*
 * The ref tick to microsecond ratio is used to convert between the ref ticks and the absolute
 * time.  The ref tick runs on the PMU clock and the absolute time is based on the 20ppm
 * radio frequency reference. The ucode computes this value and it is stored in the registers
 * scc_per and scc_per_frac.  Unfortunately it takes a while for the ucode to get this number
 * on start up.  So here we will calibrate our own before the one from ucode becomes available
 */
...
void
wlc_ref_tick_us_init(wlc_info_t *wlc)
{
	uint32 init;
	uint32 start, stop;
	uint32 now, then;
	...
	tsf = &wlc->regs->tsf_timerlow;
	wlc_force_ht(wlc, TRUE, &prev_wake);

	/* start to measure in the beginning of a new PMU tick */
	init = CUR_REF_TICK(wlc);
	while ((then = CUR_REF_TICK(wlc)) == init)
	    ;   /* empty */

	/* wait for 64 ticks */
	start = R_REG(wlc->osh, tsf);
	do {
	    now = CUR_REF_TICK(wlc);
	    if (U32_DUR(then, now) >= 64)
	        break;
	}
	while (TRUE);
	stop = R_REG(wlc->osh, tsf);

	wlc_force_ht(wlc, prev_wake, NULL);

	/* multiply the result by 16 to make it 64*16=1024 ticks */
	wlc->us_per_1024ticks = U32_DUR(start, stop) << 4;
	wlc->scc_per = wlc->us_per_1024ticks/1024;
	wlc->scc_per_frac = wlc->us_per_1024ticks & 0x3ff;

done:;
}

uint
wlc_ref_tick_to_us(wlc_info_t *wlc, uint ticks)
{
	return ticks * wlc->scc_per + ((ticks * wlc->scc_per_frac) >> 10);
}
