 ...
 *
 * $Id: wlc_bsscfg_psq.c 613017 2016-01-15 23:02:23Z $
 */
...
#define wlc_bsscfg_psq_bss_dump NULL
...
BCMATTACHFN(wlc_bsscfg_psq_attach)(wlc_info_t *wlc)
{
	...
	if ((psqi->cfgh = wlc_bsscfg_cubby_reserve(wlc, sizeof(bss_bsscfg_psq_info_t *),
			wlc_bsscfg_psq_bss_init, wlc_bsscfg_psq_bss_deinit, wlc_bsscfg_psq_bss_dump,
			psqi)) < 0) {
...
wlc_bsscfg_tx_stop(wlc_bsscfg_psq_info_t *psqi, wlc_bsscfg_t *cfg)
{
	...
	WL_INFORM(("wl%d.%d: %s: pending %d packets %d\n",
	        ...
	        TXPKTPENDTOT(wlc), pktq_len(WLC_GET_TXQ(cfg->wlcif->qi))));
...
wlc_bsscfg_tx_start(wlc_bsscfg_psq_info_t *psqi, wlc_bsscfg_t *cfg)
{
	...
	WL_INFORM(("wl%d.%d: %s: TX SUPR %d pending %d packets %d\n",
	        wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
	        (wlc->block_datafifo & DATA_BLOCK_TX_SUPR) != 0,
	        TXPKTPENDTOT(wlc), pktq_len(&bpsqi->psq)));
	...
	if (!pktq_empty(txq) ||
	    !pktq_empty(wlc_low_txq(cfg->wlcif->qi->low_txq)) ||
	    FALSE) {
...
wlc_bsscfg_tx_pktcnt(wlc_bsscfg_psq_info_t *psqi, wlc_bsscfg_t *cfg)
{
	...
	if (bpsqi != NULL) {
		return (uint)pktq_len(&bpsqi->psq);
	}
	return 0;
}
