 ...
 *
 * $Id: wlc_wnm.c 608725 2015-12-29 06:13:17Z $
 */
...
static const bcm_iovar_t wnm_iovars[] = {
	...
	{"wnm_tfs_set", IOV_WNM_TFS_SET, IOVF_BSSCFG_STA_ONLY, IOVT_BUFFER, sizeof(wl_tfs_set_t)},
	{"wnm_tfs_term",
	IOV_WNM_TFS_TERM, IOVF_BSSCFG_STA_ONLY, IOVT_BUFFER, sizeof(wl_tfs_term_t)},
#endif /* STA */
	{"wnm_tfs_status", IOV_WNM_TFS_STATUS, 0, IOVT_BUFFER, 0},
	{NULL, 0, 0, 0, 0}
};
	...
	uint8 token, struct scb *scb)
{
	...
	if (status == DOT11_BSSTRANS_RESP_STATUS_ACCEPT) {
		...
		for (i = asc->join_targets_last;
			i < asc->join_targets->count; i++) {
			wlc_bss_info_t *bi = asc->join_targets->ptrs[i];
	...
	if (status == DOT11_BSSTRANS_RESP_STATUS_ACCEPT) {
		...
		max_score = wlc_bss_pref_score(bsscfg,
			asc->join_targets->ptrs[asc->join_targets_last],
			TRUE, &temp_rssi);

		/* build list of BSS non-candidates */
		for (i = asc->join_targets_last, count = 0;
			i < asc->join_targets->count && count < bss_list_count; i++) {
			wlc_bss_info_t *bi = asc->join_targets->ptrs[i];
