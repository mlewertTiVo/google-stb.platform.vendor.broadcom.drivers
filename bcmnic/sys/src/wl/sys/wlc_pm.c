 ...
 *
 *   $Id: wlc_pm.c 613490 2016-01-19 06:05:27Z $
 */
	...
	uint rxbytes, uint txbytes)
{
	...
	if (pm->pm2_rx_pkts_since_bcn == 1 && inc_rx &&
	    ...
	    pm->pm2_sleep_ret_threshold == pm->pm2_bcn_sleep_ret_time)
	...
	if (pm->dfrts_logic == WL_DFRTS_LOGIC_OFF) {
		if (first_bcn_sleep_tmr_rx) {
			...
			pm->pm2_sleep_ret_threshold = MAX(pm->pm2_sleep_ret_threshold,
				pm->pm2_sleep_ret_time);
			WL_RTDC(bsscfg->wlc, "frts_upd: timeleft=%u",
				pm->pm2_sleep_ret_threshold, 0);
			return;
		}
	}
...
restart_tmr:
	 ...
	 */
	if (first_bcn_sleep_tmr_rx ||
		(sleep_ret_time < pm->pm2_sleep_ret_threshold &&
		pm->pm2_sleep_ret_threshold != pm->pm2_bcn_sleep_ret_time)) {

		/* Override the sleep return time for this wake period */
		pm->pm2_sleep_ret_threshold = sleep_ret_time;
		 ...
		 * the modified pm->pm2_sleep_ret_threshold.
		 */
...
#ifdef WLTDLS
		TDLS_ENAB(cfg->wlc->pub) &&
		!(wlc_tdls_get_pmoverride(cfg->wlc->tdls)) &&
#endif /* WLTDLS */
		TRUE) {
...
wlc_bcn_tim_ie_pm2_action(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (cfg->pm->pm2_bcn_sleep_ret_time)
		cfg->pm->pm2_sleep_ret_threshold =
...
wlc_bcn_parse_tim_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if (SCAN_IN_PROGRESS(wlc->scan) &&
		...
		pm->pm2_sleep_ret_threshold = MAX(pm->pm2_bcn_sleep_ret_time,
...
wlc_pm_ulp_enter_cb(void *handle, ulp_ext_info_t *einfo, uint8 *cache_data)
{
	...
	for (i = 0; i < cfg_cnt; i++) {
		...
		ULP_DBG(("CAC:i:%d,cfg:%p,0x%p,rt_tm:%x,rx_pkts:%x,bcn_slp:%x,PMenb:%x\n", i, icfg,
			pm_dat, pm_dat->pm2_sleep_ret_time,
	...
	uint8 *p2_cache_data)
{
	...
	for (i = 0; i < cfg_cnt; i++) {
		...
		ULP_DBG(("CAC:i:%d,cfg:%p,pm_dat:%p,rt_tm:%x,rx_pkts:%x,bcn_slp:%x,PMenb:%x\n",
			i, icfg, pm_dat, pm_dat->pm2_sleep_ret_time,
			pm_dat->pm2_rx_pkts_since_bcn,
			pm_dat->pm2_bcn_sleep_ret_time,
			pm_dat->PMenabled));
...
#if defined(WL_PM2_RCV_DUR_LIMIT)
	{"pm2_rcv_dur", IOV_PM2_RCV_DUR, 0, IOVT_UINT16, 0},
#endif /* WL_PM2_RCV_DUR_LIMIT */
...
#endif
	{"pm2_sleep_ret", IOV_PM2_SLEEP_RET, 0, IOVT_INT16, 0},
	{"pm2_sleep_ret_ext", IOV_PM2_SLEEP_RET_EXT,
	0, IOVT_BUFFER, sizeof(wl_pm2_sleep_ret_ext_t)},
	{"pm2_refresh_badiv", IOV_PM2_REFRESH_BADIV, IOVF_BSSCFG_STA_ONLY, IOVT_INT16, 0},
	{"pm_dur", IOV_PM_DUR, 0, IOVT_UINT32, 0},
	{"pm2_radio_shutoff_dly", IOV_PM2_RADIO_SHUTOFF_DLY, 0, IOVT_UINT16, 0},
#ifdef WL_EXCESS_PMWAKE
	{"excess_pm_period", IOV_EXCESS_PM_PERIOD, IOVF_WHL, IOVT_INT16, 0},
	{"excess_pm_percent", IOV_EXCESS_PM_PERCENT, IOVF_WHL, IOVT_INT16, 0},
#endif
	{"ps_resend_mode", IOV_PS_RESEND_MODE, 0,  IOVT_UINT8, 0},
	{"sleep_between_ps_resend", IOV_SLEEP_BETWEEN_PS_RESEND, IOVF_RSDB_SET,  IOVT_UINT8, 0},
	{"pm2_bcn_sleep_ret", IOV_PM2_BCN_SLEEP_RET, 0, IOVT_INT16, 0},
	{"pm2_md_sleep_ext", IOV_PM2_MD_SLEEP_EXT, IOVF_WHL, IOVT_INT8, 0},
#ifdef WL_EXCESS_PMWAKE
	{"pfn_roam_alert_thresh", IOV_PFN_ROAM_ALERT_THRESH, 0, IOVT_BUFFER, 8},
	{"const_awake_thresh", IOV_CONST_AWAKE_THRESH, 0, IOVT_UINT32, 0},
#endif
	{NULL, 0, 0, 0, 0}
};
...
/* iovar dispatcher */
static int
wlc_pm_doiovar(void *ctx, const bcm_iovar_t *vi, uint32 actionid, const char *name,
...
/* ioctl dispatcher */
static int
wlc_pm_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
...
BCMATTACHFN(wlc_pm_attach)(wlc_info_t *wlc)
{
	...
	if ((pmi->cfgh = wlc_bsscfg_cubby_reserve_ext(wlc, 0, wlc_pm_bss_init,
			wlc_pm_bss_deinit, wlc_pm_bss_dump, PM_COPY_SIZE,
			wlc_pm_bss_get, wlc_pm_bss_set, pmi)) < 0) {
