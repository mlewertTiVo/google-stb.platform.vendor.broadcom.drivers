 ...
 *
 * $Id: wlc_ucinit.c 612761 2016-01-14 23:06:00Z $
 */
...
wlc_uci_enable_ucode_patches(wlc_hw_info_t *wlc_hw, uint32 mask, uint32 flags)
{
	...
	hndd11_write_reg32(wlc_hw->osh, &regs->psm_patchcopy_ctrl,
		...
		wlc_hw->pc_flags | PSM_PATCHCC_PCTRL_RST_RESET);
	...
	const uint32 *pa, uint pa_sz)
{
	...
	uint32 axi_base = si_get_d11_slaveport_addr(wlc_hw->sih, D11_AXI_SP_ID, wlc_hw->unit);
	...
	if (flags == UCODE_INITVALS) {
		...
		if (D11REV_IS(wlc_hw->corerev, 60)) {
			if (WLCISACPHY(wlc_hw->band)) {
				...
				WLC_UCI_ERR(("%s: wl%d: unsupported phy in corerev %d\n",
...
wlc_uci_wowl_ucode_init_rom_patches(wlc_info_t *wlc)
{
	...
	if (D11REV_IS(wlc->pub->corerev, 60) && WLCISACPHY(wlc->band)) {
