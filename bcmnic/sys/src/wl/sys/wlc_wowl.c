 ...
 *
 * $Id: wlc_wowl.c 612761 2016-01-14 23:06:00Z $
 */
...
static const bcm_iovar_t wowl_iovars[] = {
	...
	{"wowl_arp_hostip", IOV_WOWL_ARP_HOSTIP, (0), IOVT_BUFFER, 4},
                 ...
                 void *p, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WOWL_FORCE):
		if (bool_val) {
			...
			if (WLULP_ENAB(wlc->pub) && !wlc_ulp_allow_wowl_force(wlc->ulp)) {
				err = BCME_BADOPTION;
				break;
			}
	...
	if (key_info.algo != CRYPTO_ALGO_OFF) {
...
#endif /* WOWL_OS_OFFLOADS */
		{
			...
			if (wowl->key_rot && scb->bsscfg) {
				...
				else if (GTKOE_ENAB(wlc->pub) && WLULP_ENAB(wlc->pub))
...
wlc_wowl_ucode_init(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (scb && (key_info.hw_algo == WSEC_ALGO_AES)) {
		...
		else if ((D11REV_IS(wlc->pub->corerev, 46) && WLCISACPHY(wlc->band))) {
#if defined(BCMULP)
		...
		else if (D11REV_IS(wlc->pub->corerev, 60) && WLCISACPHY(wlc->band)) {
#if defined(BCMULP)
			/* ULP / rev60 / security */
			if (WLULP_ENAB(wlc->pub)) {
...
enable:
	...
	if (WLULP_ENAB(wlc->pub) && MCNX_ENAB(wlc->pub)) {
...
#ifdef BCMULP
	if (WLULP_ENAB(wlc->pub)) {
		/* Collected Data, Write to SHM /FCBS ROM */
