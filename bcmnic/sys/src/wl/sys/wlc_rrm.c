 ...
 *
 * $Id: wlc_rrm.c 609925 2016-01-05 21:37:14Z $
 */
...
#ifdef WL_FTM_11K
#if defined(WL_PROXDETECT) && defined(WL_FTM)
	{"rrm_frng", IOV_RRM_FRNG, (IOVF_SET_UP), IOVT_BUFFER, 0},
#endif /* WL_PROXDETECT && WL_FTM */
#endif /* WL_FTM_11K */
	{NULL, 0, 0, 0, 0}
};
...
static void wlc_rrm_ftm_ranging_cb(wlc_info_t *wlc, wlc_ftm_t *ftm,
	...
	wl_proxd_session_id_t sid, void *cb_ctx)
{
	...
	wl_proxd_session_id_list_t *sids = NULL;
	frngrep_t *frng_buf = NULL;
	int ranging_sids_size = 0;
	...
	ranging_sids_size = OFFSETOF(wl_proxd_session_id_list_t, ids) +
		info.num_sids * sizeof(wl_proxd_session_id_t);
	sids = (wl_proxd_session_id_list_t *) MALLOCZ(wlc->osh, ranging_sids_size);
	if (sids == NULL) {
		WL_ERROR(("%s: MALLOC failed of %d bytes\n", __FUNCTION__, ranging_sids_size));
		goto done;
	}

	err = wlc_ftm_ranging_get_sids(rctx, info.num_sids, sids);
	if (err != BCME_OK) {
		WL_ERROR(("%s: wlc_ftm_ranging_get_sids failed, err %d \n", __FUNCTION__, err));
		goto done;
	}
	...
	for (i = 0; i < sids->num_ids; i++) {
		...
		err = wlc_ftm_ranging_get_result(rctx, sids->ids[i], 0, &res);
		if (err != BCME_OK) {
			WL_ERROR(("%s: status %d getting result for session sid %d\n",
					__FUNCTION__, err, sids->ids[i]));
			continue;
		}
		err = wlc_ftm_get_session_info(ftm, sids->ids[i], &session_inf);
		if (err != BCME_OK) {
	...
	if (sids) {
		MFREE(wlc->osh, sids, ranging_sids_size);
	}
}
	...
	while (tlvs) {
		if (tlvs->id == DOT11_MNG_NEIGHBOR_REP_ID) {
			...
			if (nbr_rep_ie->len > DOT11_NEIGHBOR_REP_IE_FIXED_LEN) {
				...
				wlc_rrm_parse_mr_tlvs(wlc, mr_tlvs, &mr_tlv_len, nbr_rep);
			}
...
wlc_rrm_begin_ftm(wlc_rrm_info_t *rrm_info)
{
	...
	for (i = 0; i < frng_req->num_aps; i++)
	{
		...
		err = bcm_pack_xtlv_entry((void **)&req_tlvs, &req_len,
			WL_PROXD_TLV_ID_PEER_MAC, sizeof(struct ether_addr),
			&frng_req->targets[i].bssid, BCM_XTLV_OPTION_ALIGN32);
		if (err != BCME_OK) {
		...
		err = bcm_pack_xtlv_entry((void **)&req_tlvs, &req_len, WL_PROXD_TLV_ID_NUM_BURST,
			sizeof(uint16), &num_burst, BCM_XTLV_OPTION_ALIGN32);
		if (err != BCME_OK) {
			WL_ERROR(("wl%d: %s: status %d, bcm_pack_xtlv_entry failed\n",
				wlc->pub->unit, __FUNCTION__, req_len));
			goto done;
		...
		err = bcm_pack_xtlv_entry((void **)&req_tlvs, &req_len, WL_PROXD_TLV_ID_CHANSPEC,
			sizeof(uint32), &chanspec, BCM_XTLV_OPTION_ALIGN32);
		if (err != BCME_OK) {
			WL_ERROR(("wl%d: %s: status %d, bcm_pack_xtlv_entry failed\n",
				wlc->pub->unit, __FUNCTION__, req_len));
			goto done;
	...
	if (err != BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_ftm_ranging_start failed \n",
				wlc->pub->unit, __FUNCTION__));
	}
	...
	if (err != BCME_OK && rctx != NULL)	{
		...
		rrm_info->rrm_frng_req->rctx = NULL;
	}
	return err;
}
	...
	dot11_rmrep_ftm_range_t *rmrep_ftmrange, int len)
{
	...
	for (i = 0; i < frng_rep.range_entry_count; i++) {
		...
		frng_rep.range_entries[i].max_err = (((range_entry->max_err[0] & 0xff) << 16) |
			((range_entry->max_err[1] & 0xff) << 8) | (range_entry->max_err[2] & 0xff));
...
wlc_rrm_create_frngrep_element(uint8 **ptr, rrm_bsscfg_cubby_t *rrm_cfg, frngrep_t *frngrep)
{
	...
	*ptr += TLV_HDR_LEN + DOT11_MNG_IE_MREP_FRNG_FIXED_LEN;
	...
	for (i = 0; i < frngrep->range_entry_count; i++) {
		...
		ftm_range->max_err[0] = (frngrep->range_entries[i].max_err >> 16) & 0xFF;
		ftm_range->max_err[1] = (frngrep->range_entries[i].max_err >> 8) & 0xFF;
		ftm_range->max_err[2] = frngrep->range_entries[i].max_err & 0xFF;
		ftm_range->rsvd = 0;
		*ptr += sizeof(dot11_ftm_range_entry_t);
	}
