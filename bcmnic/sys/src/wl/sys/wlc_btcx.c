 ...
 *
 * $Id: wlc_btcx.c 613017 2016-01-15 23:02:23Z $
 */
...
#endif
	{"btc_status", IOV_BTC_STATUS, (IOVF_GET_UP), IOVT_UINT32, 0},
#ifdef WLAIBSS
	{"btc_aibss_status", IOV_BTC_AIBSS_STATUS, 0, IOVT_BUFFER, 0
	},
#endif /* WLAIBSS */
#ifdef STA
#ifdef WLBTCPROF
	{"btc_select_profile", IOV_BTC_SELECT_PROFILE, 0, IOVT_BUFFER, 0},
#endif
#endif /* STA */
	{NULL, 0, 0, 0, 0}
};
...
wlc_btcx_aibss_set_prot(wlc_btc_info_t *btc, bool prot_on, bool cmn_cts2self)
{
	if (AIBSS_ENAB(btc->wlc->pub) && BT3P_HW_COEX(btc->wlc) &&
		...
		btcx_config = wlc_read_shm(btc->wlc, btc->bt_shm_addr +
			M_BTCX_CONFIG_OFFSET(btc->wlc));
		...
		wlc_write_shm(btc->wlc, btc->bt_shm_addr +
			M_BTCX_CONFIG_OFFSET(btc->wlc), btcx_config);
	}
}
...
wlc_btcx_aibss_set_strobe(wlc_btc_info_t *btc, int strobe_on)
{
	if (AIBSS_ENAB(btc->wlc->pub) && BT3P_HW_COEX(btc->wlc) &&
		...
		wlc_write_shm(btc->wlc, btc->bt_shm_addr + M_BTCX_CONFIG_OFFSET(btc->wlc),
				btcx_config);
...
wlc_btcx_aibss_chk_clk_sync(wlc_btc_info_t *btc)
{
	...
	pri_map_lo = wlc_read_shm(btc->wlc, btc->bt_shm_addr + M_BTCX_PRI_MAP_LO_OFFSET(btc->wlc));
	...
	*/
				...
				if (!(btc->aibss_info->local_btinfo & BT_INFO_BT_CLK_SYNCED) &&
					...
					*/
					if (!(pri_map_lo & BT_ACL_TASK_BIT)) {
						...
						wlc_write_shm(btc->wlc, btc->bt_shm_addr +
							M_BTCX_PRI_MAP_LO_OFFSET(btc->wlc),
							pri_map_lo);
	...
	if (btc->aibss_info->acl_grant_set) {
		pri_map_lo &= ~BT_ACL_TASK_BIT;
		wlc_write_shm(btc->wlc, btc->bt_shm_addr +
			M_BTCX_PRI_MAP_LO_OFFSET(btc->wlc), pri_map_lo);
		btc->aibss_info->acl_grant_set = FALSE;
		btc->aibss_info->acl_grant_cnt = 0;
	}
}
...
wlc_btc_hflg(wlc_info_t *wlc, bool set, uint16 val)
{
	...
	btc_blk_ptr = 2 * wlc_bmac_read_shm(wlc->hw, M_BTCX_BLK_PTR(wlc));
	btc_hflg = wlc_bmac_read_shm(wlc->hw, btc_blk_ptr + M_BTCX_HOST_FLAGS_OFFSET(wlc));
	...
	wlc_bmac_write_shm(wlc->hw, btc_blk_ptr + M_BTCX_HOST_FLAGS_OFFSET(wlc), btc_hflg);
}
...
static int8 wlc_btcx_get_btpwr(wlc_btc_info_t *btc)
{
	...
	/* read btpwr  */
	txpwr_shm = wlc_read_shm(wlc, btcx_blk_ptr + M_BTCX_BT_TXPWR_OFFSET(wlc));
	btc->btcdyn->bt_pwr_shm = txpwr_shm; /* keep a copy for dbg & status */

	/* clear the shm after read, ucode will refresh with a new value  */
	wlc_write_shm(wlc,  btcx_blk_ptr + M_BTCX_BT_TXPWR_OFFSET(wlc), 0);
			...
			if ((wlc->hw->boardflags & BFL_FEM_BT) == 0 && /* dLNA chip */
				...
				if (new_mode == WL_BTC_FULLTDM) {
					wlc_bmac_update_shm(wlc->hw,
						btcx_blk_ptr + M_BTCX_HOST_FLAGS_OFFSET(wlc),
...
wlc_btc_update_btrssi(wlc_btc_info_t *btc)
{
	...
	btrssi_shm = wlc_bmac_read_shm(wlc->hw, btcx_blk_ptr + M_BTCX_RSSI_OFFSET(wlc));

	if (btrssi_shm) {
		/* clear shm because ucode keeps max btrssi idx */
		wlc_bmac_write_shm(wlc->hw, btcx_blk_ptr + M_BTCX_RSSI_OFFSET(wlc), 0);
