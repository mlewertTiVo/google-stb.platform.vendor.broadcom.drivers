 ...
 *
 * $Id: wlc_tdls.c 613207 2016-01-18 06:00:27Z $
 */
...
#define TDLS_DISCONNECT_PENDING_TIMEOUT	200 /**< ms */
...
#ifdef BE_TDLS
#define TDLS_LOOKASIDE_RETRY_CNT       3       /**< max retry count, for discovery request */
#endif /* BE_TDLS */
...
#ifdef BE_TDLS
#define TDLS_SCB_RETRY_CNT	3	/* max retry count, for setup request */
#endif /* BE_TDLS */
...
#define TDLS_CHSW_MAXTIMEOUT	65000
#define TDLS_CHSW_OFFTIME		30000
...
/* retry types */
enum {
	TDLS_RETRY_DISC_REQ = 1,
	TDLS_RETRY_DISC_RESP,
	TDLS_RETRY_SETUP_REQ,
	TDLS_RETRY_SETUP_RESP,
	TDLS_RETRY_SETUP_CFM
};
...
struct wl_timer *tdls_retry_timer = NULL;
...
#ifdef BE_TDLS
/* use this variable when disc/setup response-frame retransmission */
tdls_timer_arg_t tdls_retry_timer_arg = { NULL, 0 };

/* Accessor for ROM */
static
tdls_timer_arg_t*
BCMRAMFN(tdls_retry_timer_arg_get)(void)
{
	return &tdls_retry_timer_arg;
}
...
static void wlc_tdls_teardown_timer_cb(void *arg);
...
static void wlc_tdls_post_chsw(tdls_info_t *tdls, struct scb *scb, chanspec_t cur_chanspec);
...
BCMATTACHFN(wlc_tdls_attach)(wlc_info_t *wlc)
{
	...
	if (!(tdls->lookaside = (tdls_lookaside_t *)MALLOCZ(wlc->osh,
		sizeof(tdls_lookaside_t) * (tdls->max_sessions)))) {
		WL_ERROR(("wl%d: wlc_tdls_attach: out of mem, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		goto fail;
	}

	if (!(tdls->tdls_pm_timer = wl_init_timer(wlc->wl,
		wlc_tdls_pm_timer_cb, wlc, "tdls_pm_timer"))) {
	...
	if (!(tdls->pm_timer_info = (tdls_pm_timer_info_t *)MALLOCZ(wlc->osh,
		...
		WL_ERROR(("wl%d: wlc_tdls_attach: pm_timer_info alloc failed\n",
			wlc->pub->unit));
		goto fail;
	}
	...
	if (!(tdls->timer_arg = (tdls_timer_arg_t *)MALLOCZ(wlc->osh,
		sizeof(tdls_timer_arg_t) * (tdls->max_sessions)))) {
		WL_ERROR(("wl%d: wlc_tdls_attach: out of mem, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		goto fail;
	}

	for (i = 0; i < (tdls->max_sessions); i++) {
		tdls->timer_arg[i].hdl = tdls;
		tdls->timer_arg[i].idx = i;
	}
...
BCMATTACHFN(wlc_tdls_detach)(tdls_info_t *tdls)
{
	...
	MFREE(tdls->wlc->osh, tdls->lookaside,
		sizeof(tdls_lookaside_t) * (tdls->max_sessions));
	MFREE(tdls->wlc->osh, tdls->timer_arg,
		sizeof(tdls_timer_arg_t) * (tdls->max_sessions));
...
static int
tdls_backup_fn(wlc_bsscfg_t *cfg, void *pkt, int prec)
{
	struct pktq *txq = &cfg->tdls->txq;
	ASSERT(txq);

	WL_MQ(("TDLS Enq: pkt %p, prec %d\n", pkt, prec));
	pktq_penq(txq, prec, pkt);
	return BCME_OK;
}

static void *
tdls_restore_fn(wlc_bsscfg_t *cfg, int *prec)
{
	struct pktq *txq = &cfg->tdls->txq;
	void *pkt;
	...
	pkt = pktq_deq(txq, prec);

	WL_MQ(("TDLS Deq: pkt %p, prec %d\n", pkt, *prec));
	return pkt;
}
...
bss_tdls_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (TDLS_SUPPORT(wlc->pub)) {
		...
		tc->ps_allowed = TRUE;
		tc->ps_pending = FALSE;
		pktq_init(&tc->txq, WLC_PREC_COUNT, PKTQ_LEN_DEFAULT);
	}
	...
	if (BSSCFG_IS_TDLS(cfg)) {
		...
		if (BAND_2G(wlc->band->bandtype) && wlc->band->gmode == GMODE_LEGACY_B)
			...
			cck_only = FALSE;
		wlc_bsscfg_rateset_init(wlc, cfg,
...
bss_tdls_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (cfg->tdls != NULL) {
		wlc_txq_pktq_flush(wlc, &cfg->tdls->txq);
...
static void
wlc_tdls_listen_on_target_ch(tdls_info_t *tdls, scb_tdls_t *scb_tdls, struct scb *scb)
{
	int delta;
	uint32 now = OSL_SYSUPTIME();

	delta = (int)(now - scb_tdls->action_frame_sent_time);
	if (delta < scb_tdls->switch_timeout) {
		delta = scb_tdls->switch_timeout - delta;
		scb_tdls->flags |= TDLS_SCB_LISTEN_ON_TARGET_CH;
		wl_add_timer(tdls->wlc->wl, scb_tdls->peer_addr->timer, delta/1000, 0);
		scb_tdls->timer_start = TRUE;
	}
	else {
		WL_TDLS(("wl%d:%s(): SwitchTimeout expiered, delta = %dms"
			"go back to base channel.\n",
			tdls->wlc->pub->unit, __FUNCTION__, delta));
		wlc_do_chanswitch(scb->bsscfg,	scb_tdls->base_chanspec);
	}
}
...
wlc_tdls_peer_state_update(tdls_info_t *tdls, tdls_lookaside_t *peer)
{
	...
	*/
	...
	if (scb_tdls->flags & TDLS_SCB_WAIT_NULLDATA_ACK) {
		...
		wlc_tdls_do_chanswitch(tdls, bsscfg,
...
wlc_tdls_peer_timer(void *arg)
{
	...
	if (!(peer->status & TDLS_LOOKASIDE_ACTIVE) && (peer->retry_cnt > 0)) {
...
wlc_tdls_chsw_timer_cb(void *arg)
{
	...
	wlc_tdls_do_chsw(tdls, scb_peer->bsscfg);
}

static void
wlc_tdls_teardown_timer_cb(void *arg)
{
	tdls_timer_arg_t *local_timer_arg;
	tdls_info_t *tdls = NULL;
	int idx;
...
void
wlc_tdls_pm_timer_action(tdls_info_t *tdls, struct scb *scb)
{
	wlc_info_t *wlc = tdls->wlc;
	scb_tdls_t *scb_tdls = SCB_TDLS(tdls, scb);
	wlc_bsscfg_t *parent = scb_tdls->parent;
...
wlc_tdls_query(tdls_info_t *tdls, wlc_bsscfg_t *parent, void *p, struct ether_addr *ea)
{
	...
	if (!tdls->auto_op ||
			(tdls->cur_sessions >= tdls->max_sessions))
		return NULL;
...
wlc_tdls_msch_cb(void* handler_ctxt, wlc_msch_cb_info_t *cb_info)
{
	...
	if (type & MSCH_CT_ON_CHAN) {
		...
		wlc_txqueue_start(cfg, cb_info->chanspec, tdls_restore_fn);
		...
		if (scb_tdls->cur_chanspec != cb_info->chanspec) {
			...
			wlc_tdls_post_chsw(cfg->wlc->tdls, cfg->tdls->tdls_scb, cb_info->chanspec);

			/* off channel now */
			if (scb_tdls->target_chanspec == cb_info->chanspec) {
				if (cfg->tdls->chsw_req_enabled) {
					/* Set timer for next channel switch */
					wl_del_timer(cfg->wlc->wl, scb_tdls->peer_addr->chsw_timer);
					wl_add_timer(cfg->wlc->wl,  scb_tdls->peer_addr->chsw_timer,
						cfg->current_bss->beacon_period, FALSE);
				}
			} /* on channel now */
			else if (scb_tdls->base_chanspec == cb_info->chanspec) {
				if (cfg->tdls->chsw_req_enabled) {
					/* Set timer for next channel switch */
					wl_del_timer(cfg->wlc->wl, scb_tdls->peer_addr->chsw_timer);
					wl_add_timer(cfg->wlc->wl,  scb_tdls->peer_addr->chsw_timer,
						cfg->current_bss->beacon_period << 2, FALSE);
				}
			}
		}
	} else {
		...
		if ((type & MSCH_CT_OFF_CHAN_DONE) || (type & MSCH_CT_SLOT_END)) {
			wlc_txqueue_end(cfg, tdls_backup_fn);
		}

		if (type & MSCH_CT_REQ_END) {
			scb_tdls->flags &= ~TDLS_SCB_CHSW_MASK;
			/* need to update current chanspec now */
			cfg->current_bss->chanspec = cb_info->chanspec;
			scb_tdls->cur_chanspec = cb_info->chanspec;
			if (cb_info->chanspec == scb_tdls->target_chanspec) {
				wl_del_timer(cfg->wlc->wl, scb_tdls->peer_addr->chsw_timer);
				wlc_tdls_return_to_base_ch(cfg->wlc->tdls, cfg->tdls->tdls_scb);
			} else if (cb_info->chanspec == scb_tdls->base_chanspec &&
				cfg->tdls->chsw_req_enabled) {
					wl_del_timer(cfg->wlc->wl, scb_tdls->peer_addr->chsw_timer);
					wl_add_timer(cfg->wlc->wl,  scb_tdls->peer_addr->chsw_timer,
						cfg->current_bss->beacon_period >> 2, FALSE);
			}
	...
	if (scb_tdls->flags & TDLS_SCB_SENT_CHSW_RESP) {
		...
		if (!(txstatus & TX_STATUS_ACK_RCV)) {
			...
			if (scb_tdls->base_chanspec != scb_tdls->cur_chanspec) {
				cur_chanspec_save = scb_tdls->cur_chanspec;
				wlc_tdls_do_chanswitch(wlc->tdls, scb->bsscfg,
					scb_tdls->base_chanspec);
			}
		...
		wlc_tdls_suppress_pending_tx_pkts(scb->bsscfg);
...
wlc_tdls_action_frame_tx_complete(wlc_info_t *wlc, uint32 txstatus, void *arg)
{
	...
	if (scb_tdls->flags & (TDLS_SCB_SENT_SETUP_RESP | TDLS_SCB_SENT_SETUP_CFM)) {
	...
	if (!(txstatus & TX_STATUS_ACK_RCV)) {
		...
		if ((scb_tdls->retry_cnt > 0) && (scb_tdls->flags & (TDLS_SCB_SENT_SETUP_REQ))) {
	...
	if (scb_tdls->flags == TDLS_SCB_SENT_SETUP_CFM) {
		...
		req_param.duration = cfg->current_bss->beacon_period;
		...
		retval = wlc_msch_timeslot_register(wlc->msch_info,
				&cfg->current_bss->chanspec, 1, wlc_tdls_msch_cb,
				cfg, &req_param, &wlc->tdls->tdls_msch_req_hdl);
		if (retval) {
...
static int wlc_tdls_send_chsw_req(tdls_info_t *tdls, struct scb *scb, chanspec_t target_chanspec)
{
	...
	if ((switch_time = nextslot_time - crslot_time) > TDLS_CHSW_MAXTIMEOUT) {
	...
	req.switch_timeout =
		(uint16)(((req.switch_time + TDLS_CHSW_OFFTIME) > TDLS_CHSW_MAXTIMEOUT)?
		TDLS_CHSW_MAXTIMEOUT : (req.switch_time + TDLS_CHSW_OFFTIME));
	...
	scb_tdls->flags |= TDLS_SCB_SENT_CHSW_REQ;
	wlc_tdls_wake_ctrl(wlc, tdls, scb);
	wlc_tdls_send_action_frame(tdls, SCB_BSSCFG(scb), &scb->ea, p, TRUE);
...
wlc_tdls_return_to_base_ch(tdls_info_t *tdls, struct scb *scb)
{
	...
	uint64 crslot_time, nextslot_time, switch_time;
	...
	crslot_time = msch_current_time(tdls->wlc->msch_info);
	nextslot_time = wlc_msch_query_timeslot(tdls->wlc->msch_info, 0, 0);
	if ((switch_time = nextslot_time - crslot_time) > 0xffff) {
		wl_del_timer(tdls->wlc->wl, scb_tdls->peer_addr->chsw_timer);
		wl_add_timer(tdls->wlc->wl,  scb_tdls->peer_addr->chsw_timer,
			(switch_time >> 10) & 0xffffffff, FALSE);
		return BCME_OK;
	}
	scb_tdls->switch_time = (uint16)switch_time;
	scb_tdls->switch_timeout =
		((scb_tdls->switch_time + TDLS_CHSW_OFFTIME) > TDLS_CHSW_MAXTIMEOUT)?
		TDLS_CHSW_MAXTIMEOUT : (scb_tdls->switch_time + TDLS_CHSW_OFFTIME);
	...
	int len)
{
	...
	scb_cmn = SCB_CMN_CUBBY(tdls, scb);
	if (!tdls->uapsd_sleep || !(scb_cmn->flags2 & SCB2_TDLS_PU_BUFFER_STA))
	...
	struct ether_addr *ea, struct ether_addr *bssid, bool initiator)
{
	...
	if (dla) {
		if (tdls->auto_op) {
			wlc_lq_rssi_ma_reset(wlc, SCB_BSSCFG(scb), scb, dla->rssi);
		}
...
#ifdef BE_TDLS
	scb_tdls->retry_cnt = TDLS_SCB_RETRY_CNT;
#endif /* BE_TDLS */
	tdls->cur_sessions++;
...
bool
wlc_tdls_get_pmoverride(tdls_info_t *tdls)
{
	uint i;
	ASSERT(tdls != NULL);
	ASSERT(tdls->lookaside != NULL);

	for (i = 0; i < (tdls->max_sessions); i++) {
		if (tdls->lookaside[i].pmoverride_set) {
			return TRUE;
		}
	}
...
#ifdef BE_TDLS
	tdls->lookaside[i].retry_ctx = NULL;
	tdls->lookaside[i].retry_cnt = TDLS_LOOKASIDE_RETRY_CNT;
#endif /* BE_TDLS */
...
wlc_tdls_lookaside_delete(tdls_info_t *tdls, tdls_lookaside_t *peer)
{
	...
	timer = peer->timer;
	teardown_timer = peer->teardown_timer;
	chsw_timer = peer->chsw_timer;
	...
	if (peer->teardown_timer) {
		WL_TDLS(("wl%d:%s(): disarm the peer teardown_timer!\n",
			tdls->pub->unit, __FUNCTION__));
		wl_del_timer(tdls->wlc->wl, peer->teardown_timer);
	}
	if (peer->chsw_timer) {
		WL_TDLS(("wl%d:%s(): disarm the peer chsw_timer!\n",
			tdls->pub->unit, __FUNCTION__));
	...
	peer->timer = timer;
	peer->teardown_timer = teardown_timer;
	peer->chsw_timer = chsw_timer;
}
...
BCMATTACHFN(wlc_tdls_lookaside_inittimers)(tdls_info_t *tdls)
{
	...
	for (i = 0; i < (tdls->max_sessions); i++) {
		...
		if ((tdls->lookaside[i].timer =
			wl_init_timer(tdls->wlc->wl,
			wlc_tdls_peer_timer, (void *)&(tdls->timer_arg[i]),
				"tdls_peer")) == NULL) {
			WL_ERROR(("wl%d: wl_init_timer for TDLS peer timer failed\n",
				tdls->pub->unit));
			return BCME_ERROR;
		}
		if ((tdls->lookaside[i].teardown_timer =
			wl_init_timer(tdls->wlc->wl,
			wlc_tdls_teardown_timer_cb, (void *)&(tdls->timer_arg[i]),
				"teardown_timer")) == NULL) {
			WL_ERROR(("wl%d: wl_init_timer for TDLS teardown_timer failed\n",
				tdls->wlc->pub->unit));
			return BCME_ERROR;
		}
		if ((tdls->lookaside[i].chsw_timer =
			wl_init_timer(tdls->wlc->wl,
			wlc_tdls_chsw_timer_cb, (void *)&(tdls->timer_arg[i]),
				"chsw_timer")) == NULL) {
			WL_ERROR(("wl%d: wl_init_timer for TDLS chsw_timer failed\n",
				tdls->wlc->pub->unit));
			return BCME_ERROR;
		}
	}
...
BCMATTACHFN(wlc_tdls_lookaside_freetimers)(tdls_info_t *tdls)
{
	...
	for (i = 0; i < (tdls->max_sessions); i++) {
		...
		if (tdls->lookaside[i].teardown_timer) {
			WL_TDLS(("wl%d:%s(): free the teardown_timer!\n",
				tdls->pub->unit, __FUNCTION__));
			wl_free_timer(tdls->wlc->wl, tdls->lookaside[i].teardown_timer);
			tdls->lookaside[i].teardown_timer = NULL;
		}
		if (tdls->lookaside[i].chsw_timer) {
			WL_TDLS(("wl%d:%s(): free the chsw_timer!\n",
				tdls->pub->unit, __FUNCTION__));
			wl_free_timer(tdls->wlc->wl, tdls->lookaside[i].chsw_timer);
			tdls->lookaside[i].chsw_timer = NULL;
		}
	}
}
...
wlc_tdls_free_scb(tdls_info_t *tdls, struct scb *scb)
{
	...
	if (!scb_tdls->free_me && tdls->up) {
		...
		wl_del_timer(tdls->wlc->wl, scb_tdls->peer_addr->teardown_timer);
		wl_del_timer(tdls->wlc->wl, scb_tdls->peer_addr->chsw_timer);
...
wlc_tdls_disconnect(tdls_info_t *tdls, struct scb *scb, bool force)
{
	...
	if ((scb_tdls->cur_chanspec != scb_tdls->base_chanspec) &&
		...
		*/
		scb_tdls->flags |= TDLS_SCB_TEARDOWN_PENDING;
		wl_add_timer(wlc->wl,  scb_tdls->peer_addr->teardown_timer,
			TDLS_DISCONNECT_PENDING_TIMEOUT, FALSE);
		return;
	}

	if (tdls->tdls_msch_req_hdl) {
		retval = wlc_msch_timeslot_unregister(wlc->msch_info,
				&tdls->tdls_msch_req_hdl);
		if (retval) {
			WL_ERROR(("wl%d: %s: unregister to scheduler failed\n",
				tdls->wlc->pub->unit, __FUNCTION__));
			/* continue disconnect even unregister fails */
		}
		tdls->tdls_msch_req_hdl = NULL;
	}

	/*
	* this may occur for a BRCM DUT which receives a teardown frame
	* in off channel from a non BRCM peer
	*/
	if (scb_tdls->target_chanspec != scb_tdls->base_chanspec) {
		chanspec_t cur_chanspec_save = scb_tdls->target_chanspec;
		scb_tdls->target_chanspec = scb_tdls->base_chanspec;
		if (scb_tdls->cur_chanspec != scb_tdls->base_chanspec) {
			/* bring to base channel */
			wlc_tdls_switch_to_target_ch(tdls, scb, scb_tdls->target_chanspec);
			wlc_txmod_config(tdls->wlc->txmodi, scb, TXMOD_TDLS);
		}
		wl_del_timer(tdls->wlc->wl, scb_tdls->peer_addr->timer);
		scb_tdls->flags &= ~TDLS_SCB_CHSW_MASK;
	...
	scb_tdls->flags &= ~TDLS_SCB_CHSW_MASK;
	wl_del_timer(wlc->wl, scb_tdls->peer_addr->timer);
	wl_del_timer(wlc->wl, scb_tdls->peer_addr->teardown_timer);
	scb_tdls->timer_start = FALSE;
...
#ifdef BE_TDLS
	if ((tdls_retry_timer = wl_init_timer(tdls->wlc->wl, wlc_tdls_retry_cb,
		(void *)tdls_retry_timer_arg_get(), "tdls_retry")) == NULL) {
...
wlc_tdls_cal_mic_chk(tdls_info_t *tdls, struct scb *scb, uchar *tlvs, int tlvs_len, uint8 transId)
{
	...
	if ((data = MALLOCZ(tdls->osh, 256)) == NULL) {
		WL_ERROR((WLC_MALLOC_ERR, tdls->pub->unit, __FUNCTION__, 256, MALLOCED(tdls->osh)));
		goto fail;
	}

	if ((mic_save = MALLOCZ(tdls->osh, EAPOL_WPA_KEY_MIC_LEN)) == NULL) {
		WL_ERROR((WLC_MALLOC_ERR, tdls->pub->unit, __FUNCTION__, EAPOL_WPA_KEY_MIC_LEN,
			MALLOCED(tdls->osh)));
		goto fail;
	}
	...
	if (data_len + ie->len + TLV_HDR_LEN >= 256) {
		WL_ERROR(("wl%d:%s(): data[] buffer over-run = %d!\n", tdls->pub->unit,
			__FUNCTION__, (data_len + ie->len + TLV_HDR_LEN)));
		goto fail;
	}
	...
	uint16 reason, bool check_mic)
{
	...
	if (check_mic &&
		(mic_save = MALLOCZ(tdls->osh, EAPOL_WPA_KEY_MIC_LEN)) == NULL) {
		WL_ERROR((WLC_MALLOC_ERR, tdls->pub->unit, __FUNCTION__, EAPOL_WPA_KEY_MIC_LEN,
			MALLOCED(tdls->osh)));
		goto fail;
	}
...
#ifdef BE_TDLS
	if (tdls_retry_timer) {
		WL_TDLS(("wl%d:%s(): free the TDLS retry timer!\n",
			tdls->pub->unit, __FUNCTION__));
		wl_free_timer(tdls->wlc->wl, tdls_retry_timer);
		tdls_retry_timer = NULL;
	}
#endif /* BE_TDLS */
}
...
wlc_tdls_process_setup_cfm(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	wl_del_timer(wlc->wl, tdls_retry_timer);
#endif /* BE_TDLS */
	wl_del_timer(wlc->wl, scb_tdls->peer_addr->timer);
	scb_tdls->timer_start = FALSE;
	...
	/* register scheduler time slot with scb_peer's chanspc
	  * which could be same or different from the primary
	  */
	bzero(&req_param, sizeof(req_param));
	req_param.req_type = MSCH_RT_BOTH_FLEX;
	req_param.duration = bsscfg->current_bss->beacon_period;
	req_param.duration <<= 10;
	req_param.priority = MSCH_DEFAULT_PRIO;
	req_param.interval = req_param.duration;
	retval = wlc_msch_timeslot_register(wlc->msch_info,
			&scb_peer->bsscfg->current_bss->chanspec, 1,
			wlc_tdls_msch_cb, scb_peer->bsscfg, &req_param,
			&tdls->tdls_msch_req_hdl);
	if (retval) {
		WL_ERROR(("wl%d: %s(): register to scheduler failed!\n",
				tdls->pub->unit, __FUNCTION__));
		goto exit;
	}
	...
	*(uint16 *)data = cap;
	...
	do {
		...
		if (valid_wtrc) { /* already exist trc */
			wlc_pcb_fn_register(wlc->pcb, wlc_tdls_retry_complete, (void *)wtrc, p);
			break;
		}
		/* building retry context */
		if (!(trc = (tdls_retry_ctx_t *)MALLOC(tdls->osh, sizeof(tdls_retry_ctx_t)))) {
			WL_ERROR(("wl%d:%s: MALLOC failed\n",
			wlc->pub->unit, __FUNCTION__));
			break;
		}
		...
		f_clone->p = trc;
		wlc_pcb_fn_register(wlc->pcb, wlc_tdls_retry_complete, (void *)trc, p);
	} while (0);
#endif /* BE_TDLS */
...
exit:
	...
	if ((switch_time = nextslot_time - crslot_time) > TDLS_CHSW_MAXTIMEOUT) {
		wl_del_timer(wlc->wl, scb_tdls->peer_addr->chsw_timer);
		wl_add_timer(wlc->wl,  scb_tdls->peer_addr->chsw_timer,
			(uint)(switch_time >> 10), FALSE);
		scb_tdls->flags &= ~TDLS_SCB_RECV_CHSW_REQ;
		return BCME_MSCH_NOTREADY;
	}
	chsw_resp.switch_time = (uint16)switch_time;
	chsw_resp.switch_time = (chsw_resp.switch_time >= chsw_req.switch_time)?
		chsw_resp.switch_time : chsw_req.switch_time;
	chsw_resp.switch_timeout =
		((chsw_resp.switch_time + TDLS_CHSW_OFFTIME) > TDLS_CHSW_MAXTIMEOUT)?
		TDLS_CHSW_MAXTIMEOUT: (chsw_resp.switch_time + TDLS_CHSW_OFFTIME);
	chsw_resp.switch_timeout = (chsw_resp.switch_timeout >= chsw_req.switch_timeout)?
		chsw_resp.switch_timeout: chsw_req.switch_timeout;
	...
	struct wlc_frminfo *f, uint offset)
{
	...
	/* cannot process chsw response when teardown has been initiated */
	if (scb_tdls->flags & (TDLS_SCB_SENT_TEARDOWN | TDLS_SCB_RCVD_TEARDOWN |
		TDLS_SCB_TEARDOWN_BLOCK)) {
		return 0;
	}
	...
	wlc_tdls_suppress_pending_tx_pkts(scb_peer->bsscfg);
	wlc_tdls_suppress_pending_tx_pkts(scb_tdls->parent);
			...
			if (scb_tdls->target_chanspec != scb_tdls->base_chanspec) {
				if (OSL_SYSUPTIME() < scb_tdls->max_null_attempt_time) {
					...
					*/
					chanspec_t cur_chanspec_save = 0;
					cur_chanspec_save = scb_tdls->cur_chanspec;
					wlc_tdls_do_chanswitch(wlc->tdls,
						SCB_BSSCFG(scb), scb_tdls->base_chanspec);

					if (SCB_BSSCFG(scb)->tdls->chsw_req_enabled) {
						wlc_tdls_chsw_recover(wlc, scb);
					} else if (cur_chanspec_save) {
						wlc_tdls_delete_chan_ctx(wlc->tdls, scb,
							cur_chanspec_save);
					}
				}
				return;
...
#ifdef BE_TDLS
	if ((peer->flags & TDLS_LOOKASIDE_INUSE) && (peer->status & TDLS_LOOKASIDE_DISC)) {
		WL_TDLS(("wl%d:%s(): TDLS lookaside table entry %d: %s =>IDLE\n",
			tdls->pub->unit, __FUNCTION__,
...
void
wlc_tdls_do_chsw(tdls_info_t *tdls, wlc_bsscfg_t *bsscfg)
{
	scb_tdls_t *scb_tdls;
	struct scb *scb;
	bool off_channel;
	...
	scb_tdls = SCB_TDLS(tdls, scb);
	ASSERT(scb_tdls);
	if (scb_tdls->cur_chanspec == scb_tdls->target_chanspec)
		off_channel = FALSE;
	else
		off_channel = TRUE;
	if (scb_tdls->target_chanspec == scb_tdls->base_chanspec) {
	...
	if (off_channel) {
		...
		if (bsscfg->tdls->chsw_req_enabled) {
			retv = wlc_tdls_send_chsw_req(tdls, scb, scb_tdls->target_chanspec);
			if (retv && retv != BCME_MSCH_NOTREADY)
...
wlc_tdls_get_pretbtt_time(tdls_info_t *tdls)
{
	...
	FOREACH_BSS(wlc, idx, cfg) {
		if (BSS_TDLS_ENAB(wlc, cfg)) {
			...
			scb_tdls = SCB_TDLS(tdls, scb);
			if (scb_tdls->target_chanspec != scb_tdls->base_chanspec) {
				switch_time = (tdls->switch_time >
					scb_tdls->switch_time) ? tdls->switch_time :
					scb_tdls->switch_time;
...
wlc_tdls_do_chanswitch(tdls_info_t *tdls, wlc_bsscfg_t *cfg, chanspec_t newchspec)
{
	...
	if (newchspec == scb_tdls->chsw_req_chan) {
		WL_INFORM(("wl%d: %s: already requested\n", wlc->pub->unit, __FUNCTION__));
		return;
	}
	scb_tdls->chsw_req_chan = newchspec;
	...
	req.flags = 0;
	if (newchspec == scb_tdls->target_chanspec) {
		req.duration = cfg->current_bss->beacon_period;
		req.req_type = MSCH_RT_BOTH_FIXED;
	}
	else {
		req.duration = cfg->current_bss->beacon_period << 3;
	...
	if (tdls->tdls_msch_req_hdl) {
		wlc_msch_timeslot_unregister(wlc->msch_info, &tdls->tdls_msch_req_hdl);
	}

	if ((err = wlc_msch_timeslot_register(wlc->msch_info, &newchspec, 1,
		wlc_tdls_msch_cb, cfg, &req, &tdls->tdls_msch_req_hdl))
		== BCME_OK) {
			WL_INFORM(("wl%d: TDLS chsw request success\n", wlc->pub->unit));
	} else {
		WL_INFORM(("wl%d: TDLS chsw req failed %d\n",
			wlc->pub->unit, err));
	}
}
...
wlc_tdls_chsw_complete(tdls_info_t *tdls, struct scb *scb)
{
	...
	if (!(scb_tdls->flags & TDLS_SCB_RECV_CHSW_REQ)) {
		...
		if (tdls->test_chsw_timeout)
			delta += scb_tdls->switch_timeout;
#endif

		if (delta < (uint)scb_tdls->switch_time) {
#ifdef TDLS_TESTBED
			if (!tdls->test_chsw_timeout)
#endif
			delta = scb_tdls->switch_time - delta;
		...
		WL_TDLS(("wl%d:%s(): Responder waiting for Peer to transmit data "
			"frame.\n", tdls->pub->unit, __FUNCTION__));
		wlc_tdls_listen_on_target_ch(tdls, scb_tdls, scb);
	}
...
static void
wlc_tdls_post_chsw(tdls_info_t *tdls, struct scb *scb, chanspec_t cur_chanspec)
{
	scb_tdls_t *scb_tdls;
	scb_tdls = SCB_TDLS(tdls, scb);

	/* for off-channel switch, start/wait for data transfer on target channel */
	if (cur_chanspec != scb_tdls->base_chanspec) {
		wlc_tdls_chsw_complete(tdls, scb);
	}
	else {
		wlc_tdls_pm_mode_leave(tdls, scb_tdls->parent);
		if (scb_tdls->flags & TDLS_SCB_TEARDOWN_PENDING) {
			...
			scb_tdls->target_chanspec = scb_tdls->base_chanspec;
			scb_tdls->flags &= ~TDLS_SCB_CHSW_MASK;
			delta = (int)(OSL_SYSUPTIME() - scb_tdls->action_frame_sent_time);
			if (delta < scb_tdls->switch_time)
				delta = (scb_tdls->switch_time - delta);
			else
				delta = 0;
			scb_tdls->flags &= ~TDLS_SCB_TEARDOWN_PENDING;
			scb_tdls->flags |= TDLS_SCB_TEARDOWN_BLOCK;
			wl_add_timer(tdls->wlc->wl,  scb_tdls->peer_addr->teardown_timer,
				delta/1000 + 1,	FALSE);
			scb_tdls->timer_start = TRUE;
...
wlc_tdls_chsw_send_nulldata(tdls_info_t *tdls, struct scb *scb, uint32 now)
{
	...
	if (delta < (uint)scb_tdls->switch_timeout) {
		...
		uint64 crslot_time, nextslot_time, switch_time;
		WL_TDLS(("wl%d:%s(): SwitchTimout expired, "
			"go back to base channel.\n",
			tdls->pub->unit, __FUNCTION__));
		crslot_time = msch_current_time(wlc->msch_info);
		nextslot_time = wlc_msch_query_timeslot(wlc->msch_info, 0, 0);
		...
		scb_tdls->switch_time = (uint16)switch_time;
		wlc_tdls_do_chanswitch(tdls, scb->bsscfg,
			scb_tdls->base_chanspec);
	}
...
wlc_tdls_retry_complete(wlc_info_t *wlc, uint32 txstatus, void *arg)
{
	...
	tmr_arg = tdls_retry_timer_arg_get();
	...
	if (tdls_retry_timer) {
		tdls_retry_ctx_t *prev_trc = (tdls_retry_ctx_t *)tmr_arg->hdl;
		if ((prev_trc != NULL) && (prev_trc != trc)) {
			if (prev_trc->f)
				MFREE(tdls->osh, prev_trc->f, sizeof(struct wlc_frminfo));
			MFREE(tdls->osh, prev_trc, sizeof(tdls_retry_ctx_t));
		}
		wl_del_timer(wlc->wl, tdls_retry_timer);
		/* very important!, prevent duplicate free memory */
		tmr_arg->hdl = NULL;
	}
	...
	if (!(txstatus & TX_STATUS_ACK_RCV)) {
		WL_TDLS(("wl%d:%s(): no ACK!\n", wlc->pub->unit, __FUNCTION__));
		tmr_arg->hdl = arg;
		wl_add_timer(wlc->wl, tdls_retry_timer, TDLS_RETRY_INTERVAL, 0);
		return;
	}
stop:
	if (trc) {
