 ...
 *
 * $Id: wlc_pktfetch.c 613060 2016-01-16 01:32:36Z $
 */
	...
	struct wlc_frminfo *f,	wlc_key_info_t *key_info, bool skip_iv)
{
	...
	if (SPLITRX_DYN_MODE4(wlc->osh, f->p)) {
		if ((f->wrxh->rxhdr.HdrConvSt) & ETHER_HDR_CONV_TYPE) {
...
#endif
#ifdef BDO
		BDO_PKTFETCH_REQUIRED(wlc, lsh, f) ||
#endif
#ifdef WL_TBOW
	...
	wlc_frminfo_t *f, bool promisc_frame, bool ordered, bool amsdu_msdus)
{
	...
	*/
	if (SPLITRX_DYN_MODE3(wlc->osh, f->p)) {
		pinfo->host_offset = copycount + WL_HWRXOFF_AC;
		PKTSETFRAGTOTLEN(wlc->osh, f->p,
			PKTFRAGTOTLEN(wlc->osh, f->p) + copycount + WL_HWRXOFF_AC);
	}
	...
	*/
		...
		if ((f->wrxh->rxhdr.HdrConvSt) & ETHER_HDR_CONV_TYPE) {
			...
			*  40+ Ethernet Hdr
			*/
			pinfo->host_offset = (WL_HWRXOFF_AC + DOT3HDR_OFFSET);
			PKTSETFRAGTOTLEN(wlc->osh, f->p, PKTFRAGTOTLEN(wlc->osh, f->p) +
					WL_HWRXOFF_AC + DOT3HDR_OFFSET);
		}
		else {
			/* This is Type-1 conversion so hostoffset should be
			* 40+ Ethernet Hdr + LLC hdr
			*/
			pinfo->host_offset = (WL_HWRXOFF_AC + DOT3HDR_OFFSET + LLC_HDR_LEN);
			PKTSETFRAGTOTLEN(wlc->osh, f->p, PKTFRAGTOTLEN(wlc->osh, f->p) +
					WL_HWRXOFF_AC + DOT3HDR_OFFSET + LLC_HDR_LEN);
		}
	}
	else
		pinfo->host_offset = 0;
	...
	/* extract out rx cplid */
	rxcpl_id = PKTRXCPLID(wlc->osh, f->p);
	ASSERT(rxcpl_id != 0);
	p_rxcpl_info = bcm_id2rxcplinfo(rxcpl_id);

	/* Set fetch bit */
	BCM_RXCPL_SET_PKT_FETCHD(p_rxcpl_info);
	...
	*/
	if (SPLITRX_DYN_MODE3(wlc->osh, pkt)) {
		...
		PKTSETFRAGTOTLEN(wlc->osh, pkt,
			PKTFRAGTOTLEN(wlc->osh, pkt) + copycount + WL_HWRXOFF_AC);
	}
	else
		pinfo->host_offset = 0;
