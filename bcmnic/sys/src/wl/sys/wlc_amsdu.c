 ...
 *
 * $Id: wlc_amsdu.c 613374 2016-01-18 21:22:35Z $
 */
...
#if defined(WLAMSDU_TX) && !defined(WLAMSDU_TX_DISABLED) && defined(BCMDBG_AMSDU)
...
wlc_amsdu_up(void *hdl)
{
	...
	wlc_write_shm(ami->wlc, M_MAXRXFRM_LEN(ami->wlc), (uint16)rxbufsz);
	return BCME_OK;
}
...
wlc_amsdu_agglimit_frag_upd(amsdu_info_t *ami)
{
	...
	for (i = 0; i < NUMPRIO; i++) {
		...
		if (!frag_disabled &&
			wlc->fragthresh[prio2fifo[i]] < ami->txpolicy.amsdu_max_agg_bytes[i]) {
			flush = TRUE;
			ami->txpolicy.amsdu_max_agg_bytes[i] = wlc->fragthresh[prio2fifo[i]];
...
wlc_amsdu_agg_close(amsdu_info_t *ami, struct scb *scb, uint tid)
{
	...
	WL_AMSDU(("wlc_amsdu_agg_close: valid AMSDU, add to txq %d bytes, scb %p, txpending %d\n",
		aggstate->amsdu_agg_bytes, scb, scb_ami->aggstate[tid].amsdu_agg_txpending));
