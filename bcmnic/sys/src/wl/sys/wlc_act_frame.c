 ...
 *
 * $Id: wlc_act_frame.c 613207 2016-01-18 06:00:27Z $
 */
...
static int wlc_act_frame_doiovar(void *ctx, const bcm_iovar_t *vi, uint32 actionid,
...
/* iovar dispatch */
static int
wlc_act_frame_doiovar(void *ctx, const bcm_iovar_t *vi, uint32 actionid, const char *name,
...
wlc_actframe_send_action_frame_cb(void* handler_ctxt, wlc_msch_cb_info_t *cb_info)
{
	...
	if (cb_info->type & MSCH_CT_SLOT_START) {
		...
		if (BSS_P2P_ENAB(cfg->wlc, cfg)) {
			...
			wlc_lifetime_set(cfg->wlc, actframe_cubby->action_frame,
		...
		*/
		if (!wlc_msch_query_ts_shared(msch_info, onchan->timeslot_id) &&
			!cfg->wlc->excursion_active) {
			/* suspend normal tx queue operation for channel excursion */
			wlc_suspend_mac_and_wait(cfg->wlc);
			wlc_excursion_start(cfg->wlc);
			wlc_enable_mac(cfg->wlc);
		}
		if (actframe_cubby->af_tx_cb) {
			actframe_cubby->af_tx_cb(cfg->wlc, actframe_cubby->arg,
				actframe_cubby->action_frame);
		} else {
			if (wlc_tx_action_frame_now(cfg->wlc, cfg, actframe_cubby->action_frame,
				NULL) != BCME_OK) {
	...
	if (cb_info->type &  MSCH_CT_SLOT_END) {
		...
		if (actframe_cubby->action_frame) {
			...
			wlc_bss_mac_event(cfg->wlc, cfg, WLC_E_ACTION_FRAME_COMPLETE, NULL,
				WLC_E_STATUS_NO_ACK, 0, 0, &packetId, sizeof(packetId));
			/* Send back the channel switch indication to the host */
			wlc_bss_mac_event(cfg->wlc, cfg, WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE,
				NULL, 0, 0, 0, 0, 0);
		}
		actframe_cubby->req_msch_actframe_hdl = NULL;

		if (cfg->wlc->excursion_active) {
			wlc_suspend_mac_and_wait(cfg->wlc);
			/* resume normal tx queue operation */
			wlc_excursion_end(cfg->wlc);
			wlc_enable_mac(cfg->wlc);
		}
	}
	return BCME_OK;
...
fail:
	if (actframe_cubby->action_frame) {
		PKTFREE(cfg->wlc->osh, actframe_cubby->action_frame, TRUE);
	...
	if (actframe_cubby->req_msch_actframe_hdl) {
		if (wlc_msch_timeslot_unregister(cfg->wlc->msch_info,
...
wlc_actionframetx_complete(wlc_info_t *wlc, void *pkt, uint txstatus)
{
	...
	if (actframe_cubby->req_msch_actframe_hdl) {
		wlc_bss_mac_event(wlc, cfg, WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE, NULL, 0,
			0, 0, 0, 0);
