 ...
 *
 * $Id: wlc_channel.c 612761 2016-01-14 23:06:00Z $
 */
...
BCMATTACHFN(wlc_channel_mgr_attach)(wlc_info_t *wlc)
{
	...
	*/
	...
	if ((wlc_cm->clm_inc_dataptr = (void*)MALLOC(wlc->pub->osh, clm_inc_size)) != NULL) {
		...
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes", wlc_cm->pub->unit,
...
wlc_is_european_weather_radar_channel(struct wlc_info *wlc, chanspec_t chanspec)
{
	...
	else if (CHSPEC_BW_LE20(chanspec)) {
...
#ifdef WLC_TXPWRCAP
	/* Txpwrcap needs to be updated after set channel
