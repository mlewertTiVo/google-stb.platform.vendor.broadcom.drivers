 ...
 *
 * $Id: wlc_lq.c 612761 2016-01-14 23:06:00Z $
 */
...
#endif /* WLCHANIM */
	{"phy_rssi_ant", IOV_RSSI_ANT, (0), IOVT_BUFFER, sizeof(wl_rssi_ant_t)},
	{NULL, 0, 0, 0, 0}
};
...
static int wlc_lq_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif);
...
BCMATTACHFN(wlc_lq_attach)(wlc_info_t *wlc)
{
	...
	if ((lqi->scbh = wlc_scb_cubby_reserve(wlc, sizeof(scb_lq_info_t *),
	                wlc_lq_scb_init, wlc_lq_scb_deinit, wlc_lq_scb_dump,
	                lqi)) < 0) {
...
BCMATTACHFN(wlc_lq_detach)(wlc_lq_info_t *lqi)
{
	...
	wlc_module_remove_ioctl_fn(wlc->pub, lqi);
...
static int
wlc_lq_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
...
wlc_lq_rssi_ma_reset(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb, int8 rssi)
{
	...
	/* per-ant rssi window */
	wlc_lq_ant_rssi_init(wlc, cfg, scb, rssi);
}
...
wlc_lq_recv_snr_compute(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, int8 noise)
{
	...
	if (WLCISLCN40PHY(wlc->band)) {
		int frameType = wrxh->rxhdr.PhyRxStatus_0 & PRXS0_FT_MASK;
		if (frameType != PRXS0_CCK) {
			snr = (wrxh->rxhdr.PhyRxStatus_1 & PRXS1_SQ_MASK) >> PRXS1_SQ_SHIFT;
			return snr;
		}
		if (!noise)
...
wlc_lq_scb_init(void *ctx, struct scb *scb)
{
	...
	if ((slqi = MALLOCZ(lqi->osh, sizeof(*slqi) + extra)) == NULL) {
		WL_ERROR(("wl%d: %s: mem alloc failed, allocated %d bytes\n",
		          wlc->pub->unit, __FUNCTION__, MALLOCED(lqi->osh)));
		return BCME_NOMEM;
	}
...
wlc_lq_scb_deinit(void *ctx, struct scb *scb)
{
	...
	extra = rssi_window_sz;					/* PHY reported rssi samples */
	if (PER_ANT_RSSI_SAMP(lqi, cfg)) {
		extra += rssi_window_sz * RX_ANT_NUM(lqi);	/* per-ant raw rxpwr samples */
	}
	if (PER_SCB_SNR_SAMP(cfg)) {
		extra += snr_window_sz;				/* SNR samples */
	}

	MFREE(lqi->osh, slqi, sizeof(*slqi) + extra);
...
wlc_lq_chanim_meas(wlc_info_t *wlc, chanim_cnt_t *chanim_cnt)
{
	...
	for (i = 0; i < CCASTATS_MAX; i++) {
		for (i = 0; i < CCASTATS_MAX; i++) {
			ccastats_cnt[i] = wlc_bmac_cca_read_counter(wlc->hw, 4 * i, (4 * i + 2));
