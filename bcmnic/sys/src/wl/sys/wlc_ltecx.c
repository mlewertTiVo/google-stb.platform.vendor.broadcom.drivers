 ...
 *
 * $Id: wlc_ltecx.c 612761 2016-01-14 23:06:00Z $
 */
        ...
        void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_LTECX_MWS_FRAME_CONFIG):
		bcopy(&ltecx->mws_frame_config, (char *)arg,
		sizeof(mws_frame_config_t));
		break;
	case IOV_SVAL(IOV_LTECX_MWS_FRAME_CONFIG):
		bcopy((char *)params, &ltecx->mws_frame_config,
		sizeof(mws_frame_config_t));
...
void wlc_ltecx_update_seci_rxbreak(wlc_ltecx_info_t *ltecx)
{
	...
	sbregCorestatus = si_corereg(wlc->hw->sih, SI_CC_IDX,
		OFFSETOF(chipcregs_t, gci_corestat), 0, 0);
	rxBreakStatus = (sbregCorestatus & 1);
	if (rxBreakStatus == 0) {
		si_corereg(wlc->hw->sih, SI_CC_IDX, OFFSETOF(chipcregs_t, gci_secilcr),
			1 << SECI_ULCR_LBC_POS, rxBreakStatus<< SECI_ULCR_LBC_POS);
	}
	else {
...
wlc_ltecx_update_frame_config(wlc_ltecx_info_t *ltecx)
{
	...
	txdur = ltecx->mws_frame_config.mws_period_dur[LTECX_FRAME_UPLINK_TYPE];
	while (i < LTECX_MAX_NUM_PERIOD_TYPES) {
		if (ltecx->mws_frame_config.mws_period_type[i] == LTECX_FRAME_UPLINK_TYPE) {
			...
			if (txdur != ltecx->mws_ltetx_dur_prev) {
				wlc_bmac_write_shm(wlc_hw,
					...
					ltecx->mws_frame_config.mws_period_dur[i]);
