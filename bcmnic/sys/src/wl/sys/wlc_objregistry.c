 ...
 *
 * $Id: wlc_objregistry.c 607820 2015-12-22 07:46:37Z $
 */
...
void
BCMATTACHFN(obj_registry_free)(obj_registry_t *objr, osl_t *osh)
{
	if (objr) {
		if (objr->value)
...
int
BCMATTACHFN(obj_registry_set)(obj_registry_t *objr, obj_registry_key_t key, void *value)
{
	if (objr) {
		WL_OBJR_DBG(("%s:%d:key[%d]=value[%p]\n", __FUNCTION__, __LINE__, (int)key, value));
		if (key >= objr->count) return BCME_RANGE;
...
void*
BCMATTACHFN(obj_registry_get)(obj_registry_t *objr, obj_registry_key_t key)
{
	void *value = NULL;
	if (objr) {
		if (key >= objr->count) return NULL;
...
int
BCMATTACHFN(obj_registry_ref)(obj_registry_t *objr, obj_registry_key_t key)
{
	int ref = 0;
	if (objr && key < objr->count) {
...
int
BCMATTACHFN(obj_registry_unref)(obj_registry_t *objr, obj_registry_key_t key)
{
	int ref = 0;
	if (objr && key < objr->count) {
		ref = (objr->ref[(int)key]);
...
int
BCMATTACHFN(obj_registry_islast)(obj_registry_t *objr)
{
