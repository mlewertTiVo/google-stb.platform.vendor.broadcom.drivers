 ...
 *
 * $Id: wlc_scan_utils.c 596076 2015-10-29 14:39:25Z $
 */
...
static const bcm_iovar_t scan_utils_iovars[] = {
	{"escan", IOV_ESCAN, 0, IOVT_BUFFER, WL_ESCAN_PARAMS_FIXED_SIZE},
	{"scanabort", IOV_SCANABORT, 0, IOVT_VOID, 0},
	{"scanresults_minrssi", IOV_SCANRESULTS_MINRSSI, 0, IOVT_INT32, 0},
	{"iscan", IOV_ISCAN, 0, IOVT_BUFFER, WL_ISCAN_PARAMS_FIXED_SIZE},
	{"iscanresults", IOV_ISCANRESULTS, 0, IOVT_BUFFER, WL_ISCAN_RESULTS_FIXED_SIZE},
	{NULL, 0, 0, 0, 0}
};
#else
#define scan_utils_iovars NULL
#endif /* STA */

/* local function declarations */
static int wlc_scan_register_iem_fns(wlc_info_t *wlc);
static int wlc_scan_utils_wlc_down(void *ctx);
static int wlc_scan_utils_doiovar(void *hdl, const bcm_iovar_t *vi, uint32 actionid,
	const char *name, void *params, uint p_len, void *arg, int len,
	int val_size, struct wlc_if *wlcif);
static int wlc_scan_utils_doioctl(void *ctx, int cmd, void *arg, int len,
	struct wlc_if *wlcif);
...
static int
wlc_custom_scan_results(wlc_info_t *wlc, wl_scan_results_t *iob, int *len, uint results_state)
{
	wl_bss_info_t *wl_bi;
	uint32 totallen = 0;
	uint32 datalen;
...
/* ioctl dispatcher */
static int
wlc_scan_utils_doioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	wlc_scan_utils_t *sui = ctx;
	wlc_info_t *wlc = sui->wlc;
	int bcmerror = BCME_OK;
...
/* iovar dispatcher */
static int
wlc_scan_utils_doiovar(void *hdl, const bcm_iovar_t *vi, uint32 actionid, const char *name,
