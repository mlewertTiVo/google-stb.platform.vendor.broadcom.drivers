...
wlc_phy_20696_radio_vcocal(phy_info_t *pi, uint8 cal_mode, uint8 coupling_mode)
{
...
#endif /* BCM7271 */
	/* PU cal clock */
	//MOD_RADIO_PLLREG_20696(pi, XTAL6, xtal_pu_caldrv, 0x1);
	//MOD_RADIO_PLLREG_20696(pi, XTAL6, xtal_outbufCalstrg, 0x1);

	//MOD_RADIO_PLLREG_20696(pi, PLL_CFG6, rfpll_vcocal_clk_pu, 0x1);
	//MOD_RADIO_PLLREG_20696(pi, PLL_OVR1, ovr_rfpll_vcocal_clk_pu, 1);
	////MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL2, rfpll_vcocal_force_caps_ovr, 0x0); // LUKA: this one exists

	// if (sicoreunit == DUALMAC_MAIN) {
		// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL21, rfpll_vcocal_force_caps2_ovr, 0x0);
		// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL22, rfpll_vcocal_force_aux1_ovr, 0x0);
		// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL23, rfpll_vcocal_force_aux2_ovr, 0x0);
	// }
	...
	// if (sicoreunit == DUALMAC_MAIN) {
		// if (READ_RADIO_REGFLD_20696(pi, RFP, PLL_VCOCAL24,
			// rfpll_vcocal_enableCoupling) == 1) {
			// /* 2 coupled VCO cal modes. Change only if coupled VCO is enbled */
			// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL24, rfpll_vcocal_couplingMode,
					// coupling_mode);
		// }
	// } else {
	...
	//OSL_DELAY(1000); // instead of waiting for VCOCAL isdone
...
wlc_phy_radio20696_vcocal_isdone(phy_info_t *pi, bool set_delay, bool cache_calcode)
{
	...
	/* PD cal clock */
	//MOD_RADIO_PLLREG_20696(pi, XTAL6, xtal_pu_caldrv, 0x0);
	/* power off vcocal_clk */
	//MOD_RADIO_PLLREG_20696(pi, PLL_CFG6, rfpll_vcocal_clk_pu, 0x0);
	//MOD_RADIO_PLLREG_20696(pi, PLL_OVR1, ovr_rfpll_vcocal_clk_pu, 0x0);
	...
	if (done == 1) {
		if (cache_calcode) {
			...
			// /* power off vcocal_clk */
			// MOD_RADIO_PLLREG_20696(pi, PLL_CFG6, rfpll_vcocal_clk_pu, 0x1);
			// MOD_RADIO_PLLREG_20696(pi, PLL_OVR1, ovr_rfpll_vcocal_clk_pu, 0x1);
			// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL7,
			// rfpll_vcocal_CalCapRBMode, 0x2);

			// maincap = READ_RADIO_REGFLD_20696(pi, RFP, PLL_STATUS2,
			// rfpll_vcocal_calCapRB);
			// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL7,
			// rfpll_vcocal_CalCapRBMode, 0x3);
			// secondcap = READ_RADIO_REGFLD_20696(pi, RFP, PLL_STATUS2,
			// rfpll_vcocal_calCapRB);
			// MOD_RADIO_PLLREG_20696(pi, PLL_VCOCAL7,
			// rfpll_vcocal_CalCapRBMode, 0x4);
			// auxcap = READ_RADIO_REGFLD_20696(pi, RFP, PLL_STATUS2,
			// rfpll_vcocal_calCapRB);
