...
wlc_phy_txpwrctrl_pwr_setup_srom12_acphy(phy_info_t *pi)
{
	if (!(SROMREV(pi->sh->sromrev) < 12)) {
		...
		*/
		...
		/* 7271 meas the idle tssi added the call here because TCL does so as well */
		wlc_phy_txpwrctrl_idle_tssi_meas_acphy(pi);
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		...
		if (core < 3) {
			...
			if (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi)) {
				...
				wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_RFSEQ, 1, (0x17e + 16*core), 16,
					&lpfgain);
				txcal_txgain[core].rad_gain = (txcal_txgain[core].rad_gain & 0xFF0F) |
					((lpfgain & 0x0007) << 4);
			}
		} else {
