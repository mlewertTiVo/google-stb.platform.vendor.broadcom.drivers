	...
	phy_txiqlocal_info_t *cmn_info)
{
	...
	pi->paramsi = phy_param; /* Shadow handle */
	/* Initialize params */
	ac_info->pi = pi;
	ac_info->aci = aci;
...
static void wlc_phy_poll_adc_acphy(phy_info_t *pi, int16 *adc_buf, uint8 nsamps,
...
static void
wlc_phy_txcal_radio_setup_acphy_20696(phy_info_t *pi, uint8 Biq2byp)
{
	/* This stores off and sets Radio-Registers for Tx-iqlo-Calibration;
	 *
	 * Note that Radio Behavior controlled via RFCtrl is handled in the
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		/* save off orig */
		porig->iqcal_ovr1[core]  = READ_RADIO_REG_20696(pi, IQCAL_OVR1, core);
		porig->iqcal_cfg1[core]  = READ_RADIO_REG_20696(pi, IQCAL_CFG1, core);
		porig->iqcal_cfg4[core]  = READ_RADIO_REG_20696(pi, IQCAL_CFG4, core);
		porig->iqcal_cfg5[core]  = READ_RADIO_REG_20696(pi, IQCAL_CFG5, core);
		porig->iqcal_gain_rin[core] = READ_RADIO_REG_20696(pi, IQCAL_GAIN_RIN, core);
		porig->iqcal_gain_rfb[core] = READ_RADIO_REG_20696(pi, IQCAL_GAIN_RFB, core);
		...
		porig->tx2g_cfg1_ovr[core] = READ_RADIO_REG_20696(pi, TX2G_CFG1_OVR, core);
		...
		porig->tx5g_cfg2_ovr[core] = READ_RADIO_REG_20696(pi, TX5G_CFG2_OVR, core);
		porig->tx5g_misc_cfg1[core] = READ_RADIO_REG_20696(pi, TX5G_MISC_CFG1, core);
		porig->auxpga_ovr1[core] = READ_RADIO_REG_20696(pi, AUXPGA_OVR1, core);
		porig->auxpga_cfg1[core] = READ_RADIO_REG_20696(pi, AUXPGA_CFG1, core);
		porig->testbuf_ovr1[core] = READ_RADIO_REG_20696(pi, TESTBUF_OVR1, core);
		porig->testbuf_cfg1[core] = READ_RADIO_REG_20696(pi, TESTBUF_CFG1, core);
	}
	...
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			MOD_RADIO_REG_20696(pi, TX2G_MISC_CFG1, core, pa2g_tssi_ctrl_sel, 0x0);
        		...
        		MOD_RADIO_REG_20696(pi, TX5G_MISC_CFG1, core, pa5g_tssi_ctrl_sel, 0x1);
		}

        	MOD_RADIO_REG_20696(pi, IQCAL_CFG1, core, iqcal_sel_ext_tssi, 0x0); // int-tssi/iqcal select
...
wlc_phy_txcal_phy_setup_acphy(phy_info_t *pi, uint8 Biq2byp)
{
	...
	porig->TSSIMode = READ_PHYREG(pi, TSSIMode);
	...
	if (TINY_RADIO(pi) || (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi))) {
...
wlc_phy_cal_txiqlo_update_ladder_acphy(phy_info_t *pi, uint16 bbmult, uint8 core)
{
	...
	if (ACMAJORREV_4(pi->pubpi.phy_rev)) {
		...
		acphy_txiqcal_ladder_t ladder_lo_7271[] = {
		{2, 0}, {4, 0}, {6, 0}, {9, 0}, {13, 0}, {18, 0},
		{25, 0}, {25, 1}, {25, 2}, {25, 3}, {25, 4}, {25, 5},
		{25, 6}, {25, 7}, {35, 7}, {50, 7}, {71, 7}, {100, 7}};

		/* g_env will be dynamically set later */
		acphy_txiqcal_ladder_t ladder_iq_7271[] = {
		{2, 0}, {4, 0}, {6, 0}, {9, 0}, {13, 0}, {18, 0},
		{25, 0}, {35, 0}, {50, 0}, {71, 0}, {100, 0}, {100, 1},
		{100, 2}, {100, 3}, {100, 4}, {100, 5}, {100, 6}, {100, 7}};
		...
		if (ACMAJORREV_2(pi->pubpi.phy_rev) && CHSPEC_IS80(pi->radio_chanspec)) {
			...
			lad_len = 18;
			wlc_phy_table_read_acphy(pi, AC2PHY_TBL_ID_GAINCTRLBBMULTLUTS0, 1,
				0, 48, &txg);
			txg = (txg & 0x0F000) >> 12;
			/* Prepare the ladder */
			for (indx = 0; indx < lad_len; indx++) {
				ladder_lo_7271[indx].g_env = txg;
				ladder_iq_7271[indx].g_env = txg;
			}
			ladder_lo_pt = &(ladder_lo_7271[0]);
			ladder_iq_pt = &(ladder_iq_7271[0]);
		} else {
			lad_len = 18;
...
wlc_phy_txcal_phy_cleanup_acphy(phy_info_t *pi)
{
	...
	WRITE_PHYREG(pi, TSSIMode, porig->TSSIMode);
...
static void
wlc_phy_txcal_radio_cleanup_acphy_20696(phy_info_t *pi)
{
	phy_info_acphy_t *pi_ac = (phy_info_acphy_t *)pi->u.pi_acphy;
	acphy_txcal_radioregs_t *porig = (pi_ac->ac_txcal_radioregs_orig);
	uint8 core;
	...
	/* CLEANUP: restore reg values */
	FOREACH_ACTV_CORE(pi, pi->sh->phyrxchain, core) {
		WRITE_RADIO_REG_20696(pi, IQCAL_OVR1, core, porig->iqcal_ovr1[core]);
		WRITE_RADIO_REG_20696(pi, IQCAL_CFG1, core, porig->iqcal_cfg1[core]);
		WRITE_RADIO_REG_20696(pi, IQCAL_CFG4, core, porig->iqcal_cfg4[core]);
		WRITE_RADIO_REG_20696(pi, IQCAL_CFG5, core, porig->iqcal_cfg5[core]);
		WRITE_RADIO_REG_20696(pi, IQCAL_GAIN_RIN, core, porig->iqcal_gain_rin[core]);
		WRITE_RADIO_REG_20696(pi, IQCAL_GAIN_RFB, core, porig->iqcal_gain_rfb[core]);
		...
		WRITE_RADIO_REG_20696(pi, TX2G_CFG1_OVR, core, porig->tx2g_cfg1_ovr[core]);
		...
		WRITE_RADIO_REG_20696(pi, TX5G_CFG2_OVR, core, porig->tx5g_cfg2_ovr[core]);
		WRITE_RADIO_REG_20696(pi, TX5G_MISC_CFG1, core, porig->tx5g_misc_cfg1[core]);
		WRITE_RADIO_REG_20696(pi, AUXPGA_OVR1, core, porig->auxpga_ovr1[core]);
		WRITE_RADIO_REG_20696(pi, AUXPGA_CFG1, core, porig->auxpga_cfg1[core]);
		WRITE_RADIO_REG_20696(pi, TESTBUF_OVR1, core, porig->testbuf_ovr1[core]);
		WRITE_RADIO_REG_20696(pi, TESTBUF_CFG1, core, porig->testbuf_cfg1[core]);
	} /* for core */
}
	...
	uint16 bw_idx, uint16 sdadc_config, uint8 Biq2byp)
{
	...
	if (!(ACMAJORREV_32(pi->pubpi.phy_rev) || (ACMAJORREV_33(pi->pubpi.phy_rev) && !ACMINORREV_4(pi)))) {
...
wlc_phy_txcal_phy_setup_acphy_core_loopback_path(phy_info_t *pi, uint8 core, uint8 lpf_config)
{
	...
	if (!(ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi))) {
		/* 7271 manipulates ALPF via radio overrides and not PHY rfctrl overrides */
...
static void
wlc_phy_poll_adc_acphy(phy_info_t *pi, int16 *adc_buf, uint8 nsamps,
                       bool switch_gpiosel, uint16 core, bool is_tssi)
{
	...
	for (samp = 0; samp < nsamps; samp++) {
		...
		&& (is_tssi == 1)) {
			...
			if (core == 0) {
				...
				tmp = (int)((uint16)READ_PHYREG(pi, TssiVal_path2) & 0x3FF);
			} else if (core == 3) {
				MOD_PHYREG(pi, TxPwrCtrl_enable3, force_tssi_en, 1);
				OSL_DELAY(50);
				MOD_PHYREG(pi, TxPwrCtrl_enable3, force_tssi_en, 0);
				tmp = (int)((uint16)READ_PHYREG(pi, TssiVal_path3) & 0x3FF);
			}
	...
	if (switch_gpiosel && (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi) ) ) {
...
wlc_phy_cal_txiqlo_acphy(phy_info_t *pi, uint8 searchmode, uint8 mphase, uint8 Biq2byp)
{
	...
	uint16 *coeff_ptr;
	uint16 zero = 0;
	uint8  cr, k, num_cores, thidx, thidx_start, thidx_stop;
	...
	uint16 cmds_RESTART_7271[] = {0x265, 0x234, 0x084, 0x074, 0x056};
	uint16 cmds_REFINE_7271[] = {0x265, 0x234, 0x084, 0x074, 0x056};
	...
	if (TINY_RADIO(pi))
		...
		wlc_phy_txcal_radio_setup_acphy_20696(pi, Biq2byp);
	else
	...
	if (TINY_RADIO(pi)) {
		...
		/* BCM7271 crash WAR */
		WRITE_PHYREG(pi, TX_iqcal_gain_bwAddress, 0x51);
		WRITE_PHYREG(pi, TX_loft_fine_iAddress, 0x51);
		WRITE_PHYREG(pi, TX_loft_fine_qAddress, 0x51);
		WRITE_PHYREG(pi, TX_loft_coarse_iAddress, 0x51);
		WRITE_PHYREG(pi, TX_loft_coarse_qAddress, 0x51);
	}
	...
	if (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi)) {
		// suggested regs by Ginto JoeCho and Cecilia
	...
	if (searchmode == PHY_CAL_SEARCHMODE_RESTART) {
		if (Biq2byp) {
			...
			cmds = cmds_RESTART_7271;
			/* numb of commands in 20696 are 5 only */
			num_cmds_per_core = 5;
		} else {
			cmds = cmds_RESTART;
	...
	bcmerror = wlc_phy_tx_tone_acphy(pi, (int32)tone_freq, tone_ampl, 1, 0, modify_bbmult);
	...
	/* JIRA:SW4345-352 - WAR for iq cal failure in iboards at low temperatures */
	...
	if (bcmerror == BCME_OK) { /* in case tone doesn't start (still needed?) */
		...
		for (; cmd_idx <= cmd_stop_idx; cmd_idx++) {
			...
			for (thidx = thidx_start; thidx < thidx_stop; thidx++) {
				/* Set thresh_d2 */
				MOD_PHYREG(pi, iqloCalgtlthres, gctl_threshold_d2, thres_ladder[thidx]);
				...
				if (ACMAJORREV_4(pi->pubpi.phy_rev) &&
					...
					stall_val = READ_PHYREGFLD(pi, RxFeCtrl1, disable_stalls);
					if (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi)) {
						/* TCL does not disable stalls for 7271 */
	...
	if (ACMAJORREV_33(pi->pubpi.phy_rev) && ACMINORREV_4(pi)) {
		wlc_phy_resetcca_acphy(pi);
		//MOD_PHYREG(pi, ChannelSwitch, fcbs_soft_reset, 1); // maybe needed
	}
	...
	if (TINY_RADIO(pi))
		...
		wlc_phy_txcal_radio_cleanup_acphy_20696(pi);
	else
