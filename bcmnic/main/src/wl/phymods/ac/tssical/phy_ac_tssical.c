...
wlc_phy_txpwrctrl_idle_tssi_meas_acphy(phy_info_t *pi)
{
	...
	phy_ac_reg_cache_save(pi_ac, RADIOREGS_TSSI);
	...
	phy_ac_reg_cache_restore(pi_ac, RADIOREGS_TSSI);
	/* prevent crs trigger */
...
wlc_phy_tssi_radio_setup_acphy_20696(phy_info_t *pi, uint8 for_iqcal)
{
	...
	//FOREACH_CORE(pi, core) {
		...
		if (PHY_IPA(pi) || for_iqcal == 1) {
			PHY_ERROR(("FIXME: 7271 doesn't suppot iPA tssi yet\n"));
			return;
		} else {
		...
		//MOD_RADIO_REG_20696(pi, IQCAL_OVR1, core, ovr_iqcal_PU_iqcal, 0x1);
		...
		//MOD_RADIO_REG_20696(pi, IQCAL_OVR1, core, ovr_iqcal_PU_wbpga, 0x1);
		MOD_RADIO_REG_20696(pi, IQCAL_CFG5, core, wbpga_pu, 0x0);
		//MOD_RADIO_REG_20696(pi, IQCAL_OVR1, core, ovr_iqcal_PU_loopback_bias, 0x1);
