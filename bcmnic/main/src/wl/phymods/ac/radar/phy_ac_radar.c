...
static int phy_ac_radar_run(phy_type_radar_ctx_t *ctx, int PLL_idx);
...
static const wl_radar_thr_t BCMATTACHDATA(wlc_phy_radar_thresh_acphy_2cores_b1) = {
	WL_RADAR_THR_VERSION,
	0x6ac, 0x30, 0x6ac, 0x30, 0x6ac, 0x30, 0x6b4, 0x30, 0x6b4, 0x30, 0x6b4, 0x30,
};
...
static const wl_radar_thr2_t BCMATTACHDATA(wlc_phy_radar_thresh2_acphy) = {
	...
	0xa, 0xa, 0xa, 0x258, 0x258, 0x258, 0x258, 0x6, 0xc
...
BCMATTACHFN(phy_radar_init_st)(phy_info_t *pi, phy_radar_st_t *st)
{
	...
	st->rparams.radar_args.npulses = 6;
...
WLBANDINITFN(phy_ac_radar_init)(phy_type_radar_ctx_t *ctx, bool on)
{
	...
	if (TINY_RADIO(pi)) {
		if (ACMAJORREV_32(pi->pubpi.phy_rev) || ACMAJORREV_33(pi->pubpi.phy_rev)) {
			//phy_utils_write_phyreg(pi, ACPHY_RadarThresh0(pi->pubpi.phy_rev), 0x6b8);
			//phy_utils_write_phyreg(pi, ACPHY_RadarThresh1(pi->pubpi.phy_rev), 0x30);
			...
			phy_utils_write_phyreg(pi, ACPHY_RadarT3BelowMin(pi->pubpi.phy_rev),
				0x14);
			if (CHSPEC_IS20(pi->radio_chanspec)) {
			phy_utils_write_phyreg(pi, ACPHY_RadarT3Timeout(pi->pubpi.phy_rev),
				0x258);
			} else if (CHSPEC_IS40(pi->radio_chanspec)) {
			phy_utils_write_phyreg(pi, ACPHY_RadarT3Timeout(pi->pubpi.phy_rev),
				0x4b0);
...
static int
phy_ac_radar_run(phy_type_radar_ctx_t *ctx, int PLL_idx)
{
	phy_ac_radar_info_t *info = (phy_ac_radar_info_t *)ctx;
	phy_info_t *pi = info->pi;
	...
	return phy_radar_run_nphy(pi, PLL_idx);
}
