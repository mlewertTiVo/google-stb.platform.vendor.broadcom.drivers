...
static bool wlc_phy_radar_detect_uniform_pw_check(int num_pulses, radar_work_t *rt, uint16 j,
	...
	uint16 *highpow_enb, uint16 *highpow_sp_ratio)
{
	int k;
...
int
phy_radar_run_nphy(phy_info_t *pi, int PLL_idx)
{
...
//	bool filter_notradar = FALSE;
	...
	if (ISNPHY(pi) && NREV_LT(pi->pubpi.phy_rev, 7))
		...
		wlc_phy_radar_read_table(pi, st, 1, PLL_idx);
	}
	...
	for (ant = 0; ant < ant_num; ant++) {
		...
		for (i = 0; i < rt->nphy_length_bin5[ant]; i++) {
			if (rt->width_list_bin5[ant][i] >
			    ...
			    ((ISACPHY(pi) && TONEDETECTION && PLL_idx == 0) ?
	...
	for (i = 0; i < rt->length; i++) {
		...
		(ACMAJORREV_32(pi->pubpi.phy_rev) || ACMAJORREV_33(pi->pubpi.phy_rev))) {
			...
			valid_lp = (rt->width_list[i] >= rparams->radar_args.min_pw_lp) &&
				(rt->width_list[i] <= rparams->radar_args.max_pw_lp) &&
				(rt->fm_list[i] >= ((ISACPHY(pi) && TONEDETECTION && PLL_idx == 0) ?
				rparams->radar_args.min_fm_lp : min_fm_lp_sc)) &&
				(deltat >= rparams->min_deltat_lp);
		}
		...
		if (ACMAJORREV_32(pi->pubpi.phy_rev) || ACMAJORREV_33(pi->pubpi.phy_rev)) {
			if (((st->rparams.radar_thrs2.highpow_war_enb >> 1) & 0x1) == 1)
				valid_lp = valid_lp && (rt->chirp_list[i] >
				((rt->width_list[i] >= rparams->pw_chirp_adj_th_bin5) ?
...
*/
	...
	for (ant = 0; ant < ant_num; ant++) {
		...
		for (i = 0; i < rt->nphy_length[ant]; i++) {
			...
			rt->fm_list_n[ant][i] > ((ISACPHY(pi) && TONEDETECTION && PLL_idx == 0) ?
			rparams->radar_args.min_fm_lp : min_fm_lp_sc)) {
	...
	if (ISACPHY(pi) && TONEDETECTION &&
	(ACMAJORREV_32(pi->pubpi.phy_rev) || ACMAJORREV_33(pi->pubpi.phy_rev))) {
		if (PLL_idx == 1 && ((st->rparams.radar_thrs2.notradar_enb >> 0) & 0x1)) {
			j = 0;
			for (i = 0; i < rt->length; i++) {
			...
			rt->length = j;
		} else if (PLL_idx == 0 && ((st->rparams.radar_thrs2.notradar_enb >> 2) & 0x1)) {
			j = 0;
			for (i = 0; i < rt->length; i++) {
				if ((rt->notradar_list[i] <=
