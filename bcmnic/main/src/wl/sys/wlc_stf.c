 ...
 *
 * $Id: wlc_stf.c 626722 2016-03-22 18:00:08Z $
 */
...
static void
wlc_stf_update_160mode_txcore_80p80phy(wlc_stf_t *stf, chanspec_t chanspec)
{
	ASSERT(CHSPEC_IS8080(chanspec) || CHSPEC_IS160(chanspec));
	...
	if (stf->txstreams == 4) {
		/* 2x2 txcore setting */
		if (CHSPEC_CTL_SB(chanspec) <= WL_CHANSPEC_CTL_SB_LUU) {
			stf->txcore[OFDM_IDX][1] = (CORE_MASK(0) | CORE_MASK(1));
		} else {
			stf->txcore[OFDM_IDX][1] = (CORE_MASK(2) | CORE_MASK(3));
		}
		stf->txcore[NSTS1_IDX][1] =
			(CORE_MASK(0) | CORE_MASK(1) | CORE_MASK(2) | CORE_MASK(3));
		stf->txcore[NSTS2_IDX][1] =
			(CORE_MASK(0) | CORE_MASK(1) | CORE_MASK(2) | CORE_MASK(3));
		stf->txcore[NSTS3_IDX][1] = 0;
		stf->txcore[NSTS4_IDX][1] = 0;
	} else {
		...
		if (CHSPEC_CTL_SB(chanspec) <= WL_CHANSPEC_CTL_SB_LUU) {
			...
			stf->txcore[OFDM_IDX][1] = CORE_MASK(2);
		}
		stf->txcore[NSTS1_IDX][1] = (CORE_MASK(0) | CORE_MASK(2));
		stf->txcore[NSTS2_IDX][1] = 0;
		stf->txcore[NSTS3_IDX][1] = 0;
		stf->txcore[NSTS4_IDX][1] = 0;
	}
...
/* ***HACKHACK*** 7271 currently does not register phy_ac_temp_register_impl   */
/* this function is called by wlc_watchdog and depends on registered functions */
#ifdef BCM7271 
	return;
#endif
...
wlc_stf_spatialpolicy_set_complete(wlc_info_t *wlc)
{
	...
	if (ACREV_IS(wlc->band->phyrev, 33) &&
		(CHSPEC_IS8080(wlc->chanspec) || CHSPEC_IS160(wlc->chanspec))) {
		wlc_stf_update_160mode_txcore_80p80phy(wlc->stf, wlc->chanspec);
		for (idx = 0; idx < MAX_CORE_IDX; idx++)
			txcore[idx] = wlc->stf->txcore[idx][1];
	}
