 ...
 *
 * $Id: wlc_bmac.c 642437 2016-06-08 18:29:13Z $
 */
...
#ifdef BCM7271
	BCM7XXX_ERR(("%s(%d)*****HACKHACK**** need to add BAND SPECIFIC INIT FOR 7271"
...
BCMINITFN(wlc_bmac_dma_max_outstread)(wlc_hw_info_t *wlc_hw)
{
	...
	//#endif /* UNRELEASEDCHIP */
		...
		txmr = DMA_MR_12;
		BCM7XXX_ERR(("%s(%d)*****HACKHACK***** NEED TO DETERMINE THE CORRECT"
...
/* use the same number of descriptors for both RX and TX. */
/* Easier to manage memory allocation and look up */
		nrxd = ntxd;
		...
		if (!wlc_hw->dmacommon) {
			...
			if (D11REV_GE(wlc_hw->corerev, 64)) {
				...
				chnflushstatus_reg = (uint32 *)(&(wlc_hw->regs->chnflushstatus));
			}
...
#ifdef BCM7271
					BCM7XXX_ERR(("%s(%d)*****HACKHACK****"
						"need to add support for 7271 corerev=%d\n",
						__FUNCTION__, __LINE__, wlc_hw->corerev));
#endif /* BCM7271 */
...
BCMATTACHFN(wlc_chipmatch)(uint16 vendor, uint16 device)
{
	...
	if ((device == BCM4366_D11AC_ID) || (device == BCM4366_D11AC2G_ID) ||
		...
		return (TRUE);
#ifdef BCM7271
	if ((device == BCM7271_D11AC_ID) || (device == BCM7271_D11AC2G_ID) ||
	...
	bool piomode, osl_t *osh, void *regsva, uint bustype, void *btparam)
{
	...
	BCM7XXX_ERR(("%s(%d)*****HACKHACK***** 7271 MAC COREREV %d\n",
		__FUNCTION__, __LINE__, wlc_hw->corerev));
	...
	BCM7XXX_ERR(("%s(%d)*****HACKHACK***** FORCE MAC COREREV to %d\n",
		__FUNCTION__, __LINE__, wlc_hw->corerev));
#endif /* BCM7271 */
...
#ifdef BCM7271
	BCM7XXX_DBG(("%s(%d)vendor==0x%08x device=0x%08x\n",
		__FUNCTION__, __LINE__, vendor, device));
#endif /* BCM7271 */
...
#ifdef BCM7271
	BCM7XXX_ERR(("%s(%d)***HACKHACK*** just return good board\n",
		__FUNCTION__, __LINE__));
#else /* !BCM7271 */
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
...
#ifdef BCM7271
	BCM7XXX_DBG(("%s(%d)*****HACKHACK**** need to add ucode for 7271\n",
		__FUNCTION__, __LINE__));
#endif /* BCM7271 */
	if (D11REV_IS(wlc_hw->corerev, 65)) {
		if (WLCISACPHY(wlc_hw->band)) {
	...
	if ((CHIPID(wlc_hw->sih->chip) == BCM4716_CHIP_ID) ||
		...
		/* The value to be written into these registers is (2^26)/(freq)MHz */
		/* MAC clock frequency for 4366 is 192.599998MHz */
		BCM7XXX_ERR(("BCM7271: %s(%d)*****HACKHACK*****\n", __FUNCTION__, __LINE__));
		W_REG(osh, &regs->u.d11regs.tsf_clk_frac_l, 0x5114);
		W_REG(osh, &regs->u.d11regs.tsf_clk_frac_h, 0x5);
#endif /* BCM7271 */
	} else if ((CHIPID(wlc_hw->sih->chip) == BCM4335_CHIP_ID) ||
...
#ifdef BCM7271
	BCM7XXX_ENTER();
	BCM7XXX_DBG(("ucode corerev 0x%08x\n", wlc_hw->corerev));
#endif /* BCM7271 */

#if defined(BCM7271_EMU)
	wlc_hw->wlc->ucode_loaded = false;
#endif /* defined(BCM7271_EMU) */

#if defined(BCM7271_EMU)
#ifdef DISABLE_UCODE_DOWNLOAD
	wlc_hw->ucode_loaded = TRUE;
	wlc_hw->wlc->ucode_loaded = false;
	BCM7XXX_DBG(("%s(%d)*****HACKHACK***** DISABLE UCODE DOWNLOAD\n",
		__FUNCTION__, __LINE__));
	if (wlc_hw->ucode_loaded) {
		BCM7XXX_EXIT();
		return;
	}
#endif /* DISABLE_UCODE_DOWNLOAD */
#endif /* defined(BCM7271_EMU) */
...
#if defined(BCM7271_EMU)
	wlc_hw->wlc->ucode_loaded = TRUE;
#endif /* defined(BCM7271_EMU) */

#ifdef BCM7271
	BCM7XXX_DBG(("%s(%d) ucode download completed\n",	__FUNCTION__, __LINE__));
	BCM7XXX_EXIT();
#endif /* BCM7271 */
}
#endif /* BCMUCDOWNLOAD */
...
wlc_bmac_xtal(wlc_hw_info_t* wlc_hw, bool want)
{
	...
	WL_ERROR(("wl%d: wlc_bmac_xtal: want %d\n", wlc_hw->unit, want));
...
static const bmc_params_t bmc_params_64 =
	{1, 128, 128, 1, 1, 0, {32, 32, 32, 32, 32, 0, 128, 0, 32}};
