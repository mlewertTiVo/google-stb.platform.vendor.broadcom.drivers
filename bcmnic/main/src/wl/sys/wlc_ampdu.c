 ...
 *
 * $Id: wlc_ampdu.c 635057 2016-05-02 14:07:36Z $
 */
...
#define AMPDU_INI_CLEANUP_LIMIT	30		/* max # of sec in pending off state
						 * before we dump more debugging messages
						 */
...
scb_ampdu_update_config(ampdu_tx_info_t *ampdu_tx, struct scb *scb)
{
	...
	if ((scb->flags & SCB_BRCM) && !(scb->flags2 & SCB2_RX_LARGE_AGG) && !(peer3ss))
	{
		...
		bool use_max = FALSE;
#ifdef WL_MU_TX
		use_max = SCB_MU(scb) ? TRUE : FALSE;
#endif
		use_max = use_max || peer3ss;
		if (use_max) {
			scb_ampdu->max_pdu =
...
wlc_ampdu_dbg_stats(wlc_info_t *wlc, struct scb *scb, scb_ampdu_tid_ini_t *ini)
{
	...
	if (ini->dead_cnt >= 2) {
		WL_ERROR(("ampdu_dbg: wl%d.%d scb:%p tid:%d \n",
			wlc->pub->unit, WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), scb, ini->tid));
			...
			case AMPDU_TID_STATE_BA_PENDING_OFF: {
				if (ini->cleanup_ini_cnt++ >= AMPDU_INI_CLEANUP_TIMEOUT) {
					...
					WL_ERROR(("wl%d: %s: cleaning up tid %d from poff"
						...
						" fifordy %08x txe_ctl %08x\n",
						wlc->pub->unit, __FUNCTION__, tid,
						phydebug, txop, fifordy, txe_ctl));
					...
					ampdu_dump_ini(ini);
					ampdu_cleanup_tid_ini(ampdu_tx, scb, tid, FALSE);
					if (ini->cleanup_ini_cnt >= AMPDU_INI_CLEANUP_LIMIT) {
	...
	scb_ampdu_tid_ini_t *ini, uint16 release, uint16 q_avail, taf_scheduler_public_t* taf)
{
	...
	if (taf) {
		...
		uint32 pkt_tag;
		uint16 pktbytes = (uint16)pkttotlen(ampdu_tx->wlc->osh, p);
	...
	while (TRUE) {
...
#ifdef WLCNT
			if (ft == FT_VHT) {
				gid = wlc_vht_get_gid(&rinfo->plcp[0]);
			}
#endif /* WLCNT */
#endif /* WL_MU_TX */
		} /* tot_mpdu == 0 */
		...
		*/
		...
		if (free_pkt) {
			...
			if (delay_stats && (now > pkttag->shared.enqtime)) {
				uint tr = 0;
...
#ifndef WLFCTS
			if (acked) {
				/* This case is already handled in pktcallback */
			} else
#endif
			{
		...
		if (free_pkt) {
			if (acked) {
				/* Callback is already executed in acked case */
...
nextp:
		...
		if (pkt_freed) {
			...
			delay_stats = scb->delay_stats[ac];
			/* Ignore wrap-around case and error case */
			if (delay_stats && (now > WLPKTTAG(p)->shared.enqtime)) {
				uint tr;
...
{
#if defined(WLPKTDLYSTAT) || defined(WLATF_CNT)
	struct scb *scb;
	struct scb_iter scbiter;
#endif 
#if defined(WLATF_CNT)
...
wlc_ampdu_output_aqm(void *ctx, uint ac, uint request_time, struct spktq *output_q)
{
	...
	/* AMPDUs are not allowed for a station in PS. This output funciton should not be configured
...
#if defined(NEW_TXQ) && defined(WL_MULTIQUEUE) && defined(WLAMPDU_MAC)
uint8
wlc_ampdu_get_epoch(ampdu_tx_info_t *ampdu_tx, int ac)
{
	ASSERT(ac < AC_COUNT);
	ASSERT(AMPDU_AQM_ENAB(ampdu_tx->wlc->pub));

	return ampdu_tx->hagg[ac].epoch;
} /* wlc_ampdu_get_epoch */
...
#endif /* NEW_TXQ && WL_MULTIQUEUE && WLAMPDU_MAC */
