 ...
 *
 * $Id: wlc_dfs.c 612297 2016-01-13 13:06:40Z $
 */
...
wlc_dfs_bg_upgrade_wlc(wlc_info_t *wlc)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		new_oper_mode = DOT11_OPER_MODE(0, 4, bw);
...
wlc_dfs_bg_downgrade_wlc(wlc_info_t *wlc)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		new_oper_mode = DOT11_OPER_MODE(0, 3, bw);
		err = wlc_modesw_handle_oper_mode_notif_request(wlc->modesw, bsscfg,
				new_oper_mode, TRUE, MODESW_CTRL_OPMODE_IE_REQD_OVERRIDE);
		if (err != BCME_OK) {
			WL_DFS(("wl%d: failed to request modesw %d\n", wlc->pub->unit, err));
...
wlc_radar_detected(wlc_dfs_info_t *dfs, bool scan_core)
{
	...
	int radar_info = 0;
	int radar_type = 0;
	int radar_interval = 0;
	int min_pw = 0;
	int subband_result = 0;

	bool has_subband_info = DFS_HAS_SUBBAND_INFO(wlc);
#if defined(WLMSG_DFS) || defined(BCMDBG_ERR) || defined(WLTEST_DFSMSG)
	uint i;
	...
	radar_info = phy_radar_detect_run((phy_info_t *)WLC_PI(wlc), PLL_idx);

	if (has_subband_info) {
		radar_interval = 0;
		BCM_REFERENCE(radar_interval);
		subband_result = radar_info >> 14 & 0xf;
		BCM_REFERENCE(subband_result);
	} else {
		radar_interval = radar_info >> 14;
		BCM_REFERENCE(radar_interval);
		subband_result = 0;
	}
