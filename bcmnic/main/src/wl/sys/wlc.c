 ...
 *
 * $Id: wlc.c 635474 2016-05-04 01:40:13Z $
 */
...
//BCMQT
// VELOCE IS SLOW - bump up the watchdog.
#define	TIMER_INTERVAL_WATCHDOG	10000	/* watchdog timer, in unit of ms */
#define	TIMER_INTERVAL_RADIOCHK	20000	/* radio monitor timer, in unit of ms */
...
#if defined(BCM7271_EMU)
// BCMQT
#define	BEACON_INTERVAL_DEF_QT	4	/* beacon interval, as fast as possible */
#else /* !(defined(BCM7271_EMU)) */
...
#endif
#ifdef SPLIT_ASSOC
	IOV_SPLIT_ASSOC_REQ, /* association split */
	IOV_SPLIT_ASSOC_RESP, /* association split */
#endif
...
static const bcm_iovar_t wlc_iovars[] = {
...
#ifdef WLATF
	{"atf", IOV_ATF, IOVF_RSDB_SET, IOVT_UINT32, 0 },
#endif
#if defined(WLPKTENG)
	{"pkteng", IOV_PKTENG,
	...
	(0), IOVT_BUFFER, sizeof(wl_el_set_params_t)
	},
	...
	(0), IOVT_BUFFER, sizeof(wl_el_tag_params_t)
	},
#endif /* EVENT_LOG_COMPILE */
...
/* all hi-prec are mapped as if standard prec */
...
static char* wlc_dump_next_args(char **buf);
...
#if !(defined(BCMNODOWN) && !defined(SRHWVSDB)) /* prevents 'defined but not used' \
	compiler warn */
static int wlc_srvsdb_force_set(wlc_info_t *wlc, uint8 val);
#endif /* !(BCMNODOWN && !SRHWVSDB) */
...
static bool wlc_is_prop_ht_rate_capable(wlc_pub_t *pub);
...
wlc_iovar_dump(wlc_info_t *wlc, const char *params, int p_len, char *out_buf, int out_len)
{
	...
	wlc->dump_args = wlc_dump_next_args(&name_list_ptr);
...
void BCMINITFN(wlc_init)(wlc_info_t *wlc)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		wlc->pub->m_amt_info_blk = (wlc_read_shm(wlc, M_AMT_INFO_PTR) << 1);
	}
#ifdef WLC_LOW
	...
	if (wlc->bcn_wait_prd == 0) {
		wlc->bcn_wait_prd = wlc_read_shm(wlc, M_NOSLPZNATDTIM) << 3 >> 10;
...
wlc_mac_promisc(wlc_info_t *wlc)
{
	...
	if (PROMISC_ENAB(wlc->pub) && !AP_ENAB(wlc->pub) &&
	    !WET_ENAB(wlc) && !PSTA_ENAB(wlc->pub))
...
wlc_set_home_chanspec(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	if (((phy_info_t *)WLC_PI(wlc))->cmni->phymode == PHYMODE_3x3_1x1)
...
BCMATTACHFN(wlc_timers_init)(wlc_info_t *wlc, int unit)
{
	...
	if (D11REV_IS(wlc->pub->corerev, 64)) {
		if (!(wlc->txdma_timer =
			wl_init_timer(wlc->wl, wlc_tx_dma_timer, wlc, "txdmatimer"))) {
			WL_ERROR(("wl%d: txdma_timer failed\n", unit));
			goto fail;
		}
	}
#endif
...
#if defined(WLPROPRIETARY_11N_RATES)
/** These two chips don't have a capability bit for the feature */
static bool wlc_is_prop_ht_rate_capable(wlc_pub_t *pub)
{
	return (CHIPID(pub->sih->chip) == BCM43602_CHIP_ID) ||
		(CHIPID(pub->sih->chip) == BCM43462_CHIP_ID) ||
		(CHIPID(pub->sih->chip) == BCM43430_CHIP_ID) ||
		BCM4349_CHIP(pub->sih->chip);
}
#endif /* WLPROPRIETARY_11N_RATES */
...
BCMATTACHFN(wlc_attach_module)(wlc_info_t *wlc)
{
	...
	{
		MODULE_ATTACH(pdsvc_info, wlc_proxd_attach, 84);
	}
#endif
	...
	MODULE_ATTACH(rmc, wlc_rmc_attach, 109 /* reliable multicast */);
#endif
	...
	MODULE_ATTACH(nan, wlc_nan_attach, 134);
#endif
	...
	MODULE_ATTACH(macdbg, wlc_macdbg_attach, 141);
...
#ifdef WL_MU_TX
	MODULE_ATTACH(mutx, wlc_mutx_attach, 143);
#endif /* WL_MU_TX */

#ifdef WL11H
#ifdef WLDFS
	MODULE_ATTACH(dfs, wlc_dfs_attach, 144);
#endif
#endif /* WL11H */

#if defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED)
	MODULE_ATTACH(obss_dynbw, wlc_obss_dynbw_attach, 145);
#endif /* defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED) */

#if defined(WL11ULB) && !defined(WL11ULB_DISABLED)
	MODULE_ATTACH(ulb_info, wlc_ulb_attach, 146);
	wlc->pub->_ulb = TRUE;
#endif /* (WL11ULB) && !defined(WL11ULB_DISABLED) */

#if defined(WLTAF) && !defined(WLTAF_DISABLED)
	MODULE_ATTACH(taf_handle, wlc_taf_attach, 147);
#endif /* (WLTAF) && !defined(WLTAF_DISABLED)    */

#if defined(WL_MU_RX) && !defined(WL_MU_RX_DISABLED)
	MODULE_ATTACH(murx, wlc_murx_attach, 148);
#endif
                      ...
                      osl_t *osh, void *regsva, uint bustype, void *btparam, void *objr, uint *perr)
{
	 ...
	 * initialized.
	 */
	if (wlc_is_prop_ht_rate_capable(wlc->pub)) {
	...
	for (j = 0; j < NBANDS(wlc); j++) {
		...
		if (WLC_PHY_VHT_CAP(wlc->band)) {
			...
			if (WLC_VHT_PROP_RATES_CAP_PHY(wlc)) {
				/* (Default) enable all 5G VHT MCS rates 0-9 for all bandwidths,
				*  that makes bit1 of the "wl vht_features" to be 1
				*/
...
#if defined(WLPKTDLYSTAT) && defined(WLPKTDLYSTAT_IND)
	if ((wlc->pub->txdelay_params = MALLOCZ(wlc->osh,
		sizeof(txdelay_params_t))) == NULL) {
		WL_ERROR(("wl%d: txdelay_params_t alloc failed\n", unit));
		err = 2001;
		goto fail;
	}
...
#if defined(BCM_DMA_CT) && defined(WLC_LOW)
	if (wlc->txdma_timer) {
		wl_free_timer(wlc->wl, wlc->txdma_timer);
		wlc->txdma_timer = NULL;
	}
#endif
...
#endif
#if defined(WL_NAN) && !defined(WL_NAN_DISABLED)
	MODULE_DETACH(nan, wlc_nan_detach);
#endif
...
#endif /* WL_OKC || WLRCC */
#ifdef WL_MU_TX
	MODULE_DETACH(mutx, wlc_mutx_detach);
#endif
#ifdef WL_MU_RX
	MODULE_DETACH(murx, wlc_murx_detach);
#endif
#if defined(WL_BEAMFORMING) && !defined(WLTXBF_DISABLED)
...
#if defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED)
	MODULE_DETACH(obss_dynbw, wlc_obss_dynbw_detach);
#endif /* defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED) */

#if defined(WL11ULB) && !defined(WL11ULB_DISABLED)
	MODULE_DETACH(ulb_info, wlc_ulb_detach);
#endif /* (WL11ULB) && !defined(WL11ULB_DISABLED) */

#if defined(WLTAF) && !defined(WLTAF_DISABLED)
	MODULE_DETACH(taf_handle, wlc_taf_detach);
#endif /* (WLTAF) && !defined(WLTAF_DISABLED) */
...
#if defined(WLPKTDLYSTAT) && defined(WLPKTDLYSTAT_IND)
	if (wlc->pub->txdelay_params) {
		MFREE(wlc->osh, wlc->pub->txdelay_params, sizeof(txdelay_params_t));
		wlc->pub->txdelay_params = NULL;
	}
#endif /* defined(WLPKTDLYSTAT) && defined(WLPKTDLYSTAT_IND) */
}
...
wlc_link_monitor_watchdog(wlc_info_t *wlc)
{
	...
	FOREACH_AS_STA(wlc, i, cfg)	{
		...
		if (cxn->ign_bcn_lost_det == 0 &&
		    ...
		    (FALSE ||
#if defined(BCMQT_CPU) || defined(BCM7271_EMU)
		     TRUE ||
#endif
...
static void wlc_collect_rates(wlc_info_t *wlc)
{
	...
	if (cnt->rxframe > rates->rxframes_prev) {
		rates->rxframes_rate = cnt->rxframe - rates->rxframes_prev;
	} else {
		rates->rxframes_rate = 0;
	}
	rates->rxframes_prev = cnt->rxframe;

	if (cnt->txframe > rates->txframes_prev) {
		rates->txframes_rate = cnt->txframe - rates->txframes_prev;
	} else {
		rates->txframes_rate = 0;
	}
	rates->txframes_prev = cnt->txframe;
}
...
wlc_watchdog(void *arg)
{
	...
	if (R_REG(wlc->osh, &wlc->hw->regs->psm_srs_status) & 0x40) {
...
#if defined(BCM_DMA_CT) && defined(WLC_LOW)
	if (wlc->txdma_timer) {
		if (!wl_del_timer(wlc->wl, wlc->txdma_timer)) {
			callbacks++;
		}
	}
#endif
...
wlc_ioctl(wlc_info_t *wlc, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	 ...
	 * invoke the main IOCTL handling function wlc_module_do_ioctl(). The internal error code
	 * BCME_IOCTL_PATCH_UNSUPPORTED is used as an indication to try other IOCTL handlers,
	 * including those in ROM.
	 */
	if (bcmerror != BCME_IOCTL_PATCH_UNSUPPORTED) {
		return bcmerror;
	}
#endif /* WLC_PATCH_IOCTL */
...
_wlc_ioctl(void *ctx, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_INIT:
/* Compatibilty for chips rommed without EXT_STA */
#if !(defined(EXT_STA_RELOC) && defined(WLC_PATCH_IOCTL_CHECKSUM))
#endif /* !(defined(EXT_STA_RELOC) && defined(WLC_PATCH_IOCTL_CHECKSUM)) */
		break;
	...
	case WLC_GET_UCFLAGS:
		...
		if ((bcmerror = wlc_iocregchk(wlc, band)))
			break;
#ifdef WL_PSMX
		if (D11REV_GE(wlc->pub->corerev, 64) && (val >= MXHF0)) {
	...
	case WLC_SET_UCFLAGS:
		...
		i = (uint16)val;
#ifdef WL_PSMX
		if (D11REV_GE(wlc->pub->corerev, 64) && (i >= MXHF0)) {
			if (i >= MXHF0+MXHFMAX) {
				bcmerror = BCME_RANGE;
				break;
			}
...
#ifdef BCM7271
		if ((r->byteoff) & (SI_ENUM_BASE & 0xF00000)) {
			void* wl_bcm7xxx_get_justy_addr(void);
			uint32 offset;
			uint32 val;
			uint32 addr = ((uint32)wl_bcm7xxx_get_justy_addr());
			
			offset = r->byteoff;
			
			addr |= offset;
			
			val = readl((volatile uint32*)(addr));

			WL_ERROR(("%s(%d) base=0x%08x offset=0x%08x val=0x%08x\n",
				__FUNCTION__, __LINE__, addr, offset, val));
	...
	case WLC_W_REG:
	{
		...
		if ((r->byteoff) & (SI_ENUM_BASE & 0xF00000)) {
			...
			writel((uint32)(val), (volatile uint32*)(addr));
...
#if defined(BCMDBG_ERR) || defined(DBG_PHY_IOV) || defined(BCM7271)
...
/* Get or set an iovar.
 *
 * The params/p_len pair specifies any additional
 ...
 * Operation (get/set) is specified by the 'set' argument,
 * interface context provided by wlcif.
 *
 * All pointers may point into the same buffer.
 */
/* NOTE:
 * It's the callers' responsibility to align 'params' and/or 'arg' to
 * a 32 bit word boundary when this function is called from with wlc
 * (except WLC_GET_VAR/WLC_SET_VAR handler) as a direct function call.
 */
int
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	*/
	...
	case IOV_GVAL(IOV_CHANSPECS): {
		...
		if (chanspec == 0) {
			max_num_chanspecs = (WL_NUMCHANSPEC_ALL);
		} else {
	...
	case IOV_SVAL(IOV_CHANSPEC): {
		...
		/* For 160Mhz in 4365c0, txstreams and rxstreams must be at least 2 */
		if (WLCISACPHY(wlc->band) && ACREV_IS(wlc->band->phyrev, 33) &&
			(CHSPEC_IS160(chspec) || CHSPEC_IS8080(chspec))) {
			if ((wlc->stf->txstreams == 1) || (wlc->stf->rxstreams == 1)) {
				err = BCME_BADCHAN;
				break;
			}
		}
...
#if defined(BCMDBG_PHYDUMP) || defined(BCMDBG_AMPDU) || defined(MCHAN_MINIDUMP) || \
	...
	case IOV_GVAL(IOV_ASSERT_TYPE):
		*ret_int_ptr = get_g_assert_type();
		break;

	case IOV_SVAL(IOV_ASSERT_TYPE):
		set_g_assert_type((uint32)int_val);
#ifdef WLC_HIGH_ONLY
		wlc_bmac_assert_type_set(wlc->hw, get_g_assert_type());
#endif
		break;
#ifdef STA
#ifdef PHYCAL_CACHING
...
#endif /* WLC_LOW */
#if defined(BCM_DMA_CT) && defined(WLC_LOW)
	case IOV_GVAL(IOV_CPBUSY_WAR):
		if (!BCM_DMA_CT_ENAB(wlc) ||
			!D11REV_IS(wlc->pub->corerev, 64)) {
			return BCME_UNSUPPORTED;
		}
		*((uint*)arg) = wlc->cpbusy_war;
		break;

	case IOV_SVAL(IOV_CPBUSY_WAR):
		if (!BCM_DMA_CT_ENAB(wlc) ||
			!D11REV_IS(wlc->pub->corerev, 64)) {
			return BCME_UNSUPPORTED;
		}
	...
	case IOV_GVAL(IOV_SPLIT_ASSOC_REQ):
		*ret_int_ptr = ((int32)(bsscfg->flags2 & WLC_BSSCFG_FL2_SPILT_ASSOC_REQ) == 0 ?
			FALSE : TRUE);
		break;

	case IOV_SVAL(IOV_SPLIT_ASSOC_REQ):
		if (bool_val) {
			bsscfg->flags2 |= WLC_BSSCFG_FL2_SPILT_ASSOC_REQ;
		} else {
			bsscfg->flags2 &= ~WLC_BSSCFG_FL2_SPILT_ASSOC_REQ;
		}
		break;
...
BCMATTACHFN(wlc_mfbr_retrylimit_upd)(wlc_info_t *wlc)
{
	...
	/* Convention: 0 indicates use ucode_defaults */
	/* short_retry, short_fallback, long_retry, long_fallback, max_rate */
	wme_tx_params_t prms[AC_COUNT] =
		{ { RETRY_SHORT_DEF,  RETRY_SHORT_FB,  RETRY_LONG_DEF_AQM,
			  RETRY_LONG_FB,  0 },	/* AC_BK */
		  { RETRY_SHORT_DEF,  RETRY_SHORT_FB,  RETRY_LONG_DEF_AQM,
			  RETRY_LONG_FB,  0 },	/* AC_BE */
		  { RETRY_SHORT_DEF_VI,  RETRY_SHORT_FB, RETRY_LONG_DEF_AQM_VI,
			  RETRY_LONG_FB,  0 },	/* AC_VI */
		  { RETRY_SHORT_DEF_VO,  RETRY_SHORT_FB, RETRY_LONG_DEF_AQM_VO,
			  RETRY_LONG_FB,  0 } }; /* AC_VO */
	...
	for (ac = 0; ac < AC_COUNT; ac++) {
		if (prms[ac].short_retry != 0)
			WLC_WME_RETRY_SHORT_SET(wlc, ac, prms[ac].short_retry);

		if (prms[ac].short_fallback != 0)
			WLC_WME_RETRY_SFB_SET(wlc, ac, prms[ac].short_fallback);

		if (prms[ac].long_retry != 0)
			WLC_WME_RETRY_LONG_SET(wlc, ac, prms[ac].long_retry);

		if (prms[ac].long_fallback != 0)
			WLC_WME_RETRY_LFB_SET(wlc, ac, prms[ac].long_fallback);

		if (prms[ac].max_rate != 0)
			wlc->wme_max_rate[ac] = prms[ac].max_rate;
	}
...
#if defined(WL_PSMX)
	if (D11REV_GE(wlc->pub->corerev, 64)) {
		xtlv_desc[2].type = WL_CNT_XTLV_GE64_UCODEX_V1;
		xtlv_desc[2].len = WL_CNT_MCXST_STRUCT_SZ;
		xtlv_desc[2].ptr = wlc->pub->_mcxst_cnt;
	}
#endif
...
wlc_pkttag_info_move(wlc_info_t *wlc, void *pkt_from, void *pkt_to)
{
	...
	WLPKTTAG(pkt_from)->pktinfo.atf.pkt_time = 0;
#endif /* WLATF */
#ifdef WLTAF
...
#ifdef WLC_LOW
		wlc_upd_suspended_fifos_clear(wlc->hw, queue);
#endif
		dma_txinit(di);
	}
...
#ifdef BCMPCIEDEV
	/* Gen bits are not handled in PCIe code as suppression
	 * will terminate in dongle itself
	 */
	if (BCMPCIEDEV_ENAB()) {
...
wlc_pkt_set_core(wlc_info_t* wlc, void* p, uint8 core)
{
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		rate_info[0].PhyTxControlWord_0 &= ~D11AC_PHY_TXC_CORE_MASK;
		rate_info[0].PhyTxControlWord_0 |=
...
wlc_pkt_set_txpwr_offset(wlc_info_t* wlc, void* p, uint16 pwr_offset)
{
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		rate_info = WLC_TXD_RATE_INFO_GET(vhtHdr, wlc->pub->corerev);
		if (D11REV_LT(wlc->pub->corerev, 64)) {
			rate_info[0].PhyTxControlWord_1 &= ~D11AC_PHY_TXC_TXPWR_OFFSET_MASK;
			rate_info[0].PhyTxControlWord_1 |=
				(pwr_offset << D11AC_PHY_TXC_TXPWR_OFFSET_SHIFT);
		} else {
			rate_info[0].PhyTxControlWord_1 &= ~D11AC2_PHY_TXC_TXPWR_OFFSET_MASK;
			rate_info[0].PhyTxControlWord_1 |=
...
wlc_dotxstatus(wlc_info_t *wlc, tx_status_t *txs, uint32 frm_tx2)
{
...
#ifdef WLOFFLD
	if (((WLC_TXFID_GET_QUEUE(txs->frameid)) == TX_ATIM_FIFO) &&
	...
	if (ETHER_ISMULTI(txh_info.TxFrameRA)) {
		...
		if  ((D11REV_GE(wlc->pub->corerev, 42))	&&
			(WLC_TXFID_GET_QUEUE(txs->frameid) == TX_AC_BK_FIFO) &&
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub)) {
		if (!(scb && SCB_ISMULTI(scb))) {
			uint8 wlfc_status = WLFC_D11_STATUS_INTERPRET(txs->status.was_acked,
			...
			if (scb && (wlfc_status == WLFC_CTL_PKTFLAG_D11SUPPRESS)) {
				...
				if (supr_status == TX_STATUS_SUPR_PMQ) {
					/* We have PMQ Suppress from a SCB that is not in
					 * PowerSave yet. Suppress TxFIFO first. MI_PMQ gets
					 * delayed sometimes, so we handle PS inline here
					 */
					if (!SCB_PS(scb)) {
						wlc_apps_process_ps_switch(wlc,
							&scb->ea, TRUE);
					}
				}
			} else {
	...
	if (update_rate && scb) {
		...
		if (MU_TX_ENAB(wlc) && SCB_MU(scb)) {
			...
			wlc_mutx_update_txcounters(wlc->mutx, scb, rspec, FALSE,
				...
				1, txs->status.was_acked ? 0 : 1, gid);
		}
	...
	if (scb && scb->delay_stats[ac] && free_pdu) {
...
wlc_cfg_set_pmstate_upd(wlc_bsscfg_t *cfg, bool pmenabled)
{
	...
	/* Consider wake only for associated cfgs.
	 * A bsscfg which is not up, not enabled or not associated is not considered
	 */
	if (wake && cfg->associated) {
		wlc->bss_wake_map |= (1 << WLC_BSSCFG_IDX(cfg));
	} else {
		wlc->bss_wake_map &= ~(1 << WLC_BSSCFG_IDX(cfg));
	}

	/* LTR_ACTIVE if any associated STA has PM OFF or any AP is up */
	if ((wlc->bss_wake_map && wlc->stas_associated) || wlc->aps_associated) {
		wl_indicate_maccore_state(wlc->wl, LTR_ACTIVE);
	} else {
		wl_indicate_maccore_state(wlc->wl, LTR_SLEEP);
	}
...
wlc_set_pmstate(wlc_bsscfg_t *cfg, bool state)
{
	...
	else if (wlc->excursion_active || !wlc_sendpmnotif(wlc, cfg, &cfg->BSSID, 0, MAXPRIO,
		SCAN_IN_PROGRESS(wlc->scan) && state)) {
	...
	if (WL11H_ENAB(wlc) && BSSCFG_IS_PRIMARY(cfg)) {
		...
		if ((wlc_radar_chanspec(wlc->cmi, wlc->home_chanspec) ||
			...
			WL_REGULATORY(("%s: Setting quiet bit for chanspec %s\n",
				__FUNCTION__, wf_chspec_ntoa_ex(wlc->home_chanspec, chanbuf)));
		}
	}
...
#ifdef RADAR
	/* restart DFS state when opreating on primary interface in MBSS/VAP */
	if (!band_chg && WL11H_AP_ENAB(wlc) && AP_ACTIVE(wlc) && to_radar &&
			wlc->dfs && wlc_dfs_get_radar(wlc->dfs) && BSSCFG_IS_PRIMARY(cfg)) {
		wlc_set_dfs_cacstate(wlc->dfs, ON);
	}
#endif /* RADAR */
...
{
	if (WLEXTSTA_ENAB(wlc->pub) || MONITOR_ENAB(wlc) ||
...
wlc_sendup_chain(wlc_info_t *wlc, void *head)
{
	...
	uint8 *plcp;
	bool chained_sendup;
#if defined(WLCNT) && defined(WL11AC) && defined(WL_MU_RX)
	uint8 gid;
#endif
#ifdef PKTC_DONGLE
	uint32 tsf_l, tsf_h;
	...
	while (p != NULL) {
		...
		if (MU_RX_ENAB(wlc)) {
			 ...
			 * MU MPDUs.
			 */
			if ((rxh->PhyRxStatus_0 & PRXS0_FT_MASK) == PRXS0_STDN) {
				if (plcp[0] | plcp[1] | plcp[2]) {
...
#endif /* MURX_ROM_COMPAT */
#endif /* WLCNT && WL11AC && WL_MU_RX */
		...
		if (MU_RX_ENAB(wlc)) {
			wlc_murx_update_rxcounters(wlc->murx, rxh->PhyRxStatus_0 & PRXS0_FT_MASK,
				scb, h);
		}
#endif
...
wlc_process_brcm_ie(wlc_info_t *wlc, struct scb *scb, brcm_ie_t *brcm_ie)
{
	...
	if (DWDS_ENAB(SCB_BSSCFG(scb))) {
		if (DWDS_BRCM_IE(SCB_BSSCFG(scb))) {
...
{
	uint8 txstreams = wlc->stf->op_rxstreams;
	chanspec_t chanspec = wlc->default_bss->chanspec;

	/* For 160Mhz in 4365c0, only 2x2 and 1x1 are avaiable.
...
wlc_wlrspec2ratespec(uint32 wlrspec, ratespec_t *pratespec)
{
	...
	*pratespec = 0;
	...
	if (islegacy) {
		...
		rspec = HT_RSPEC(rate);
		if (IS_PROPRIETARY_11N_MCS(rspec & RSPEC_RATE_MASK))
			isldpc = TRUE;
	} else if (isvht) {
	...
	const struct ether_addr *sa, const struct ether_addr *bssid, uint body_len)
{
	...
	struct dot11_management_header *hdr;
	uint8 *pbody;
	bool first = TRUE, mutx_pkteng_on = FALSE;
	uint16 fc = 0;
	...
	fc = mutx_pkteng_on? FC_QOS_DATA : FC_DATA;
...
/* Copy from HW key table to a buffer.
 * 'offset' needs to be multiple of 4 address and
 * Buffer length 'len' must be an multiple of 4 bytes
 */
void
...
/* Set a range of HW key table to a value.
 * 'offset' needs to be multiple of 4 address and
 * Buffer length 'len' must be an multiple of 4 bytes
 */
void
...
#ifdef PROP_TXSTATUS
uint32 wlc_link_txflow_scb(wlc_info_t *wlc, struct wlc_if *wlcif, uint16 flowid,
	uint8 op, uint8 * sa, uint8 *da, uint8 tid_ac)
{
	uint32	ret = 0xff;
	switch (op) {
		...
		case FLOW_RING_FLUSH :
			wlc_flush_flowring_pkts(wlc, wlcif, da, flowid, tid_ac);
			break;
		...
		case FLOW_RING_FLUSH_TXFIFO:
		{
			...
			if (!wlc->txfifo_detach_pending) {
				if (bsscfg && BSSCFG_AP(bsscfg) && (ETHER_ISMULTI(da)))
					fifo = TX_BCMC_FIFO;
				else if (!(tid_ac & PCIEDEV_IS_AC_TID_MAP_MASK))
					fifo = wme_ac2fifo[tid_ac];
				else {
					if (tid_ac < NUMPRIO)
						fifo = prio2fifo[tid_ac];
				}
				if (TXPKTPENDGET(wlc, fifo)) {
					...
					wlc_bmac_tx_fifo_sync(wlc->hw,
						(1 << fifo), FLUSHFIFO_FLUSHID);
				}
			}
#endif /* WL_MULTIQUEUE */
			break;
		...
		case FLOW_RING_RESET_WEIGHT:
			if (BCMPCIEDEV_ENAB()) {
				ret = wlc_flow_ring_reset_weight(wlc, wlcif, da, tid_ac);
			}
			break;
...
void
wlc_set_hw_up(wlc_info_t *wlc, bool hw_up)
{
	wlc->pub->hw_up = hw_up;
}
