 ...
 *
 * $Id: wlc_tx.c 612439 2016-01-14 00:39:28Z $
 *
 */
...
#ifdef NEW_TXQ
static void txq_init(txq_t *txq, uint nfifo, uint nswq,
	txq_supply_fn_t supply_fn, void *supply_ctx, int high, int low);
static void txq_hw_fill(txq_info_t *txqi, txq_t *txq, uint ac);
...
static void wlc_low_txq_buffered_time(wlc_info_t *wlc, void *pkt, int *buffered);
/*
...
static void BCMFASTPATH
txq_hw_fill(txq_info_t *txqi, txq_t *txq, uint ac)
{
	void *p;
	struct pktq *q;
	osl_t *osh;
	...
	if (!PIO_ENAB(wlc->pub)) {
		...
		while ((p = pktq_pdeq(q, ac))) {
			...
			if (!wlc_pio_txavailable(WLC_HW_PIO(wlc, ac), nbytes, 1)) {
				/* mark this hw fifo as stopped */
				txq_hw_stop_set(txq, ac);
				WLCNTINCR(cnt->hw_stops);
				break;
			}

			/* Following code based on original wlc_txfifo() */
			wlc_pio_tx(WLC_HW_PIO(wlc, ac), p);
		}
	}
...
static void
txq_init(txq_t *txq, uint nfifo, uint nswq, txq_supply_fn_t supply_fn, void *supply_ctx, int high,
	int low)
{
	txq->next = NULL;
...
wlc_low_txq_alloc(txq_info_t *txqi, txq_supply_fn_t supply_fn, void *supply_ctx,
                  uint nfifo, uint nswq, int high, int low)
{
	txq_t *txq;
	...
	txq_init(txq, nfifo, nswq, supply_fn, supply_ctx, high, low);
...
#ifdef NEW_TXQ
void
wlc_low_txq_scb_flush(wlc_info_t *wlc, txq_t *low_txq, struct scb *remove)
{
	uint swq;
	void *head_pkt, *pkt;
	struct pktq *free_q = wlc_low_txq(low_txq);
	for (swq = 0; swq < low_txq->nswq; swq++) {
		head_pkt = NULL;
		while (pktq_ppeek(free_q, swq) != head_pkt) {
			pkt = pktq_pdeq(free_q, swq);
			if (WLPKTTAGSCBGET(pkt) != remove) {
				if (!head_pkt)
					head_pkt = pkt;
				pktq_penq(free_q, swq, pkt);
			} else {
				uint fifo_idx;
...
void wlc_low_txq_buffered_time(wlc_info_t *wlc, void *pkt, int *buffered)
{
#if defined(TXQ_MUX)
	if (buffered) {
...
#endif
		wlc_low_txq_buffered_time(low_txq_map->wlc, pkt, buffered);
	}
	return ret;
}
...
wlc_low_txq_map_pkts(wlc_info_t *wlc, wlc_txq_info_t *qi, map_pkts_cb_fn cb, void *ctx)
{
	...
	for (swq = 0; swq < low_txq->nswq; swq++) {
		...
		low_txq_map.time  = 0;
		wlc_tx_map_pkts(wlc, free_q, swq, wlc_low_txq_pkt_cb, &low_txq_map);
#if defined(TXQ_MUX)
...
#ifdef WL_NAN
	if (NAN_ENAB(wlc->pub) && BSSCFG_NAN(bsscfg)) {
		wlc_nan_set_d11actxh(wlc, bsscfg, p, h, fc, &mcl, &mch, txh);
	}
#endif
#ifdef WL_MU_TX
	if (SCB_MU(scb)) {
...
wlc_attach_queue(wlc_info_t *wlc, wlc_txq_info_t *qi)
{
	...
	if (AMPDU_AQM_ENAB(wlc->pub)) {
		/* make a backup of epoch bit settings before switching queue. Packets on the dma
		 * ring may be flushed back to the low_q and instead of traversing the lowq to find
		 * epoch bit setting, we restore the epoch state when switching back to this queue.
		 */
		for (ac = 0; ac < AC_COUNT; ac++) {
			cur_active_queue->epoch[ac] = wlc_ampdu_get_epoch(wlc->ampdu_tx, ac);
		}
	}
#endif /* WLAMPDU_MAC */
...
wlc_proptxstatus_process_pkt(wlc_info_t *wlc, void *pkt, int *buffered)
{
	if (PROP_TXSTATUS_ENAB(wlc->pub) && (wlc->excursion_active == FALSE)) {
		...
		if (wlc_should_retry_suppressed_pkt(wlc, pkt, TX_STATUS_SUPR_PMQ) ||
			...
			wlc_low_txq_buffered_time(wlc, pkt, buffered);
...
#ifdef PROP_TXSTATUS
/** PROP_TXSTATUS && NEW_TXQ specific function */
static void
wlc_low_txq_sync(wlc_info_t *wlc, txq_t *low_txq, int prec, int *time_adj)
{
	void *pkt;
	struct pktq *pq;
		...
		if (!PIO_ENAB((wlc)->pub)) {
			...
			while (NULL != (pkt = wlc_bmac_dma_getnexttxp(wlc, i, HNDDMA_RANGE_ALL))) {
				...
				case SYNCFIFO:
					...
					break;
				case FLUSHFIFO:
					/* flush all pkt in thos fifo */
					break;
#ifdef PROP_TXSTATUS
				case FLUSHFIFO_FLUSHID:
					/* flush pkt with specific flowring id, and put rest pkt
					 * back to low queue.
					 */
					if (wlc->fifoflush_id == PKTFRAGFLOWRINGID(
						wlc->osh, pkt)) {
						pktenq(&flushid_pkt_list, pkt);
						continue;
					}
					break;
#endif
...
#ifdef PROP_TXSTATUS
			/*
			* Need to suppress all host pkts to host in proptxstatus case
			* Otherwise host packets in low txq will try to go out
			* Before the suppressed packets while syncing fifos
			* This leads to various problems including epoch mismatch
			* leading to PHY Tx erros and ASSERT in AMPDU
			*/
			wlc_low_txq_sync(wlc, low_txq, i, &time_adj);

			/* Make the time adjustment for the deleted packets in the dongle */
			ASSERT((wlc_txq_buffered_time(low_txq, i)) >= time_adj);
			wlc_txq_complete(wlc->txqi, low_txq, i, time_adj, time_adj);
#endif /* PROP_TXSTATUS */
			break;
		case FLUSHFIFO:
			...
			ASSERT(TXPKTPENDGET(wlc, i) > 0);
			WLC_TXFIFO_COMPLETE(wlc, i, (uint16)TXPKTPENDGET(wlc, i),
				wlc_txq_buffered_time(low_txq, i));
			break;
#ifdef PROP_TXSTATUS
		case FLUSHFIFO_FLUSHID: {
			int qlen = pktq_len(&flushid_pkt_list);

			pktqflush(wlc->osh, &flushid_pkt_list);
			/* put the rest pkts at the head of the low software TxQ */
			pktq_prepend(&low_txq->q, i, &pkt_list);

			ASSERT(TXPKTPENDGET(wlc, i) > 0);
			WLC_TXFIFO_COMPLETE(wlc, i, qlen + time_adj,
				wlc_txq_buffered_time(low_txq, i) + time_adj);
...
wlc_txq_alloc(wlc_info_t *wlc, osl_t *osh)
{
	 ...
	 * Allocated for all physical queues in the device
	 */
	qi->low_txq = wlc_low_txq_alloc(wlc->txqi, wlc_pull_q, wlc, no_hw_fifos, NFIFO,
...
wlc_txq_flush_flowid_pkts(wlc_info_t *wlc, uint16 flowid)
{
	...
	for (tx_q = wlc->tx_queues; tx_q != NULL; tx_q = tx_q->next) {
		...
		while ((pkt = pktq_deq(&low_txq->q, &prec))) {
			if (PKTISTXFRAG(wlc->osh, pkt) &&
				(flowid == PKTFRAGFLOWRINGID(wlc->osh, pkt))) {
				wlc_get_txh_info(wlc, pkt, &txh_info);
				fifo = WLC_TXFID_GET_QUEUE(ltoh16(txh_info.TxFrameID));
