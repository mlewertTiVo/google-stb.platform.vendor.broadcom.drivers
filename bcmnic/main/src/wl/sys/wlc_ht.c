 ...
 *
 * $Id: wlc_ht.c 600278 2015-11-18 10:40:00Z $
 */
...
wlc_ht_up(void *ctx)
{
	...
	chanspec_t chanspec = bss->chanspec;
	...
	if (WLCISACPHY(wlc->band) && ACREV_IS(wlc->band->phyrev, 33) &&
		...
		wlc_rateset_bw_mcs_filter(&bss->rateset, WLC_160_MHZ);
		wlc_rateset_mcs_upd(&bss->rateset, (wlc->stf->txstreams >> 1));
	}
