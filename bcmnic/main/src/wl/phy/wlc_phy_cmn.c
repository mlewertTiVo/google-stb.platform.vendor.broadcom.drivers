 ...
 *
 * $Id: wlc_phy_cmn.c 640506 2016-05-27 17:29:27Z $
 */
...
#ifdef WL_MU_TX
static void wlc_phy_offload_ppr_to_svmp(phy_info_t *pi, ppr_t* tx_power_offset);
#endif
#ifdef WLTXPWR_CACHE
...
#if defined(WLMEDIA_N2DBG) || defined(WL_EXPORT_TXPOWER)
...
wlc_phy_txpower_recalc_target(phy_info_t *pi, ppr_t *txpwr_reg, ppr_t *txpwr_targets)
{
	...
	int8 cga;
	uint8 core;
#if (ACCONF || ACCONF2) && defined(WLC_HIGH) && defined(WLPKTENG)
	...
	if (ISACPHY(pi) && (ACMAJORREV_32(pi->pubpi.phy_rev) || ACMAJORREV_33(pi->pubpi.phy_rev))) {
		wlc_phy_offload_ppr_to_svmp(pi, pi->tx_power_offset);
	}
#endif
...
#ifdef WL_MU_TX
void wlc_phy_offload_ppr_to_svmp(phy_info_t *pi, ppr_t* tx_power_offset)
{
                  ...
                  uint8 force_gain_type, int16 *iqest)
{
	...
	if (ISNPHY(pi)) {
		...
		/* Overide the digital LPF */
		if (!TINY_RADIO(pi) && !IS_28NM_RADIO(pi) && dig_lpf) {
...
*/
		...
		if ((ACMAJORREV_3(pi->pubpi.phy_rev) ||
			...
			wlc_phy_hwaci_mitigation_acphy_tiny(pi);
		}
...
wlc_phy_iovar_forcecal(phy_info_t *pi, int32 int_val, int32 *ret_int_ptr, int vsize, bool set)
{
	...
	if (ISHTPHY(pi)) {
		...
		if (int_val == 4) {
			wlc_phy_cals_hardcode_iq(pi);
			wlc_phy_cals_hardcode_lo(pi);
		} else {
...
#if defined(WLMEDIA_N2DBG) || defined(DBG_PHY_IOV)
	case WLC_GET_PHYREG:
		*ta_ok = TRUE;
...
wlc_phy_cal_perical(wlc_phy_t *pih, uint8 reason)
{
	...
	if (CHIPID(pi->sh->chip) == BCM7271_CHIP_ID) {
		/* QQQ  7271 - Cals are not yet up in TCL so do not try for driver */
//		printf("Not running PHY calibrations\n");
		return;
	}
#endif /* BCM7271 */
