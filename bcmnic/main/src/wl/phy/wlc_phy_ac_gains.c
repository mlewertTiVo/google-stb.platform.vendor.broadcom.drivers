 ...
 *
 * $Id: wlc_phy_ac_gains.c 625155 2016-03-15 19:47:25Z $
 */
...
static const uint16 *wlc_phy_get_txgain_tbl(phy_info_t *pi);

static const uint16 *
wlc_phy_get_txgain_tbl(phy_info_t *pi)
{
...
wlc_phy_ac_gains_load(phy_info_t *pi)
{
	...
	if (RADIOID(pi->pubpi.radioid) == BCM2069_ID)
		...
		tx_pwrctrl_tbl = wlc_phy_get_txgain_tbl(pi);
	else {
...
BCMATTACHFN(wlc_phy_set_txgain_tbls)(phy_info_t *pi)
{
	...
	if (RADIOID(pi->pubpi.radioid) == BCM20691_ID) {
		...
		tx_pwrctrl_tbl = acphy28nm_txgain_epa_2g_p5_20696a0_rev0;
	} else {
		ASSERT(0);
		return;
	}
	...
	if (RADIOID(pi->pubpi.radioid) == BCM20691_ID) {
		...
		tx_pwrctrl_tbl = acphy28nm_txgain_epa_5g_p5_20696a0_rev0;
	} else {
		ASSERT(0);
		return;
	}
