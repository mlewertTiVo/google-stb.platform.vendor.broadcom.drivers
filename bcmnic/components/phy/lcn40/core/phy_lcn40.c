 ...
 *
 * $Id: phy_lcn40.c 630449 2016-04-09 00:27:18Z vyass $
 */
...
BCMATTACHFN(phy_lcn40_register_impl)(phy_info_t *pi, phy_type_info_t *ti, int bandtype)
{
	...
	if (pi->misci != NULL &&
	    (lcn40i->misci = phy_lcn40_misc_register_impl(pi, lcn40i, pi->misci)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_misc_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with Channel Manager module */
	if (pi->chanmgri != NULL && (lcn40i->chanmgri =
	    phy_lcn40_chanmgr_register_impl(pi, lcn40i, pi->chanmgri)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_chanmgr_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with BTCX module */
	if (pi->btcxi != NULL &&
	    (lcn40i->btcxi = phy_lcn40_btcx_register_impl(pi, lcn40i, pi->btcxi)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_btcx_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with TXIQLO CAL module */
	if (pi->txiqlocali != NULL &&
		(lcn40i->txiqlocali = phy_lcn40_txiqlocal_register_impl(pi, lcn40i,
		pi->txiqlocali)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_txiqlocal_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with Link Power Control module */
	if (pi->lpci != NULL &&
		(lcn40i->lpci = phy_lcn40_lpc_register_impl(pi, lcn40i,
		pi->lpci)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_lpc_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with TEMPerature sense module */
	if (pi->tempi != NULL &&
		(lcn40i->tempi = phy_lcn40_temp_register_impl(pi, lcn40i,
		pi->tempi)) == NULL) {
		PHY_ERROR(("%s: phy_lcn40_temp_register_impl failed\n", __FUNCTION__));
		goto fail;
	}
...
BCMATTACHFN(phy_lcn40_unregister_impl)(phy_info_t *pi, phy_type_info_t *ti)
{
	...
	if (lcn40i->tempi != NULL)
		phy_lcn40_temp_unregister_impl(lcn40i->tempi);

	/* Unregister from Link Power Control module */
	if (lcn40i->lpci != NULL)
		phy_lcn40_lpc_unregister_impl(lcn40i->lpci);

	/* Unregister from TXIQLO CAL module */
	if (lcn40i->txiqlocali != NULL)
		phy_lcn40_txiqlocal_unregister_impl(lcn40i->txiqlocali);

	/* Unregister from BTCX module */
	if (lcn40i->btcxi != NULL)
		phy_lcn40_btcx_unregister_impl(lcn40i->btcxi);

	/* Unregister from Channel Manager module */
	if (lcn40i->chanmgri != NULL)
		phy_lcn40_chanmgr_unregister_impl(lcn40i->chanmgri);
	...
	/* Unregister from RSSICompute module */
	if (lcn40i->misci != NULL)
		phy_lcn40_misc_unregister_impl(lcn40i->misci);
	/* ...Add your module registration here... */
}
