 ...
 *
 * $Id: phy_lcn40_antdiv.c 629393 2016-04-05 06:55:25Z gpasrija $
 */
...
/* forward declaration */
typedef struct phy_lcn40_antdiv_mem phy_lcn40_antdiv_mem_t;
...
struct phy_lcn40_antdiv_info {
	...
	phy_lcn40_antdiv_mem_t *mem;
	phy_swdiv_t *swdiv;
};

/* module private states memory layout */
struct phy_lcn40_antdiv_mem {
	...
	phy_antdiv_info_t *di)
{
	...
	if ((mem = phy_malloc(pi, sizeof(phy_lcn40_antdiv_mem_t))) == NULL) {
		PHY_ERROR(("%s: phy_malloc failed\n", __FUNCTION__));
		goto fail;
	}
	bzero(mem, sizeof(phy_lcn40_antdiv_mem_t));
...
#ifdef WLC_SW_DIVERSITY
	fns.initswdiv = phy_lcn40_swdiv_init;
	fns.getswdiv = phy_lcn40_swdiv_get_ant;
	fns.setswdiv = phy_lcn40_swdiv_set_ant;
#endif
...
fail:
	if (mem != NULL)
		phy_mfree(pi, mem, sizeof(phy_lcn40_antdiv_mem_t));
...
BCMATTACHFN(phy_lcn40_antdiv_unregister_impl)(phy_lcn40_antdiv_info_t *info)
{
	...
	phy_mfree(pi, info->mem, sizeof(phy_lcn40_antdiv_mem_t));
}
	...
	if (ant > ANT_RX_DIV_FORCE_1) {
		...
		if (CHIPID(pi->sh->chip) == BCM4314_CHIP_ID ||
			...
			wlapi_bmac_write_shm(pi->sh->physhim,
				M_PHY_ANTDIV_REG_4314(pi), 0x4b1);
			wlapi_bmac_write_shm(pi->sh->physhim,
				M_PHY_ANTDIV_MASK_4314(pi), 0x200);
		}
	} else
