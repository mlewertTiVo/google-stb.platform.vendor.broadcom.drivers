 ...
 *
 * $Id: phy_tpc_shared.c 606350 2015-12-15 07:20:50Z manojkrn $
 */
...
phy_tpc_upd_shm(phy_info_t *pi)
{
	...
	if (pi->hwpwrctrl) {
		...
		wlapi_bmac_write_shm(pi->sh->physhim, M_TXPWR_MAX(pi), 63);
		wlapi_bmac_write_shm(pi->sh->physhim, M_TXPWR_M(pi), 1 << NUM_TSSI_FRAMES);

		/* Need to lower the target power to OFDM level, then add boost for CCK. */
		wlapi_bmac_write_shm(pi->sh->physhim, M_TXPWR_TARGET(pi),
			wlc_phy_txpower_get_target_min((wlc_phy_t*)pi) << NUM_TSSI_FRAMES);

		wlapi_bmac_write_shm(pi->sh->physhim, M_TXPWR_CUR(pi), pi->hwpwr_txcur);
		...
		ppr_set_ofdm(pi->tx_power_offset, WL_TX_BW_20, WL_TX_MODE_NONE, WL_TX_CHAINS_1,
			&ofdm_offsets);
		wlapi_bmac_write_shm(pi->sh->physhim, M_CCKBOOST_ADJ(pi),
