 ...
 *
 * $Id: phy_ac_rssi.c 649330 2016-07-15 16:17:13Z mvermeid $
 */
...
BCMATTACHFN(wlc_phy_nvram_rssioffset_read)(phy_info_t *pi)
{
	...
	pi_ac->sromi->dot11b_opts =
		(int32)PHY_GETINTVAR(pi, phy_var_name);
		...
		for (subband_idx = 0; subband_idx < CH_5G_4BAND; subband_idx++) {
			...
			if ((PHY_GETVAR_SLICE(pi, phy_var_name)) != NULL) {
			  for (j = 0; j < ACPHY_NUM_BW; j++) {
				...
				[ant][i][j][subband_idx] = (int8)
				PHY_GETINTVAR_ARRAY_SLICE(pi, phy_var_name, (i+4*j+k));
			      /* Check if all the 40/80MHz rssi delta coefficients are zero */
			      if ((j > 0) && (flag == 1)) {
				if (pi_ac->sromi->rssioffset.rssi_corr_gain_delta_5g_sub
...
_phy_ac_rssi_init_gain_err(phy_type_rssi_ctx_t *ctx)
{
	...
	FOREACH_CORE(pi, core) {
		...
		*/
		if (ACMAJORREV_5(pi->pubpi->phy_rev) &&
				(pi->u.pi_acphy->rxgaincal_rssical == FALSE)) {
...
phy_ac_rssi_set_gain_delta_5g(phy_type_rssi_ctx_t *ctx, uint32 aid, int8 *deltaValues)
{
	...
	if (pi_ac->rssi_cal_rev == FALSE) {
		...
		printf("\n");
	} else if (pi_ac->rssi_cal_rev == TRUE) {
		for (bw_idx = 0; bw_idx < ACPHY_NUM_BW; bw_idx++) {
...
{
	ri->rssi_qdB_en = set_val;
	return BCME_OK;
}

int
phy_ac_rssi_get_qdb_en(phy_ac_rssi_info_t *ri, int32 *ret_val)
{
	*ret_val = (int32) ri->rssi_qdB_en;
	return BCME_OK;
}
