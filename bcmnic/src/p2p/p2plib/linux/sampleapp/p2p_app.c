...
ver2str(unsigned int vms, unsigned int vls)
{
        ...
        maj = (vms >> EPI_VER_SHIFT) & EPI_VER_MASK;
        if (maj > 1000) {
                /* it is probably a date... */
                year = (vms >> EPI_VER_SHIFT) & EPI_VER_MASK;
                month = vms & EPI_VER_MASK;
                day = (vls >> EPI_VER_SHIFT) & EPI_VER_MASK;
                build = vls & EPI_VER_MASK;
                sprintf(verstr, "%d/%d/%d build %d",
                        month, day, year, build);
        } else {
                /* it is a tagged release. */
                sprintf(verstr, "%d.%d RC%d.%d",
                        (vms>>EPI_VER_SHIFT) & EPI_VER_MASK, vms & EPI_VER_MASK,
                        (vls>>EPI_VER_SHIFT) & EPI_VER_MASK, vls & EPI_VER_MASK);
        }
        return verstr;
}

static void
print_p2papp_version()
{

        /* Display the application version info */
        printf("BCMP2P App Version: %s\n",
                ver2str((EPI_MAJOR_VERSION << EPI_VER_SHIFT)| EPI_MINOR_VERSION,
                (EPI_RC_NUMBER << EPI_VER_SHIFT) | EPI_INCREMENTAL_NUMBER));
