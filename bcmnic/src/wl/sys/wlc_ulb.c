 ...
 *
 * $Id: wlc_ulb.c 624017 2016-03-10 02:12:25Z $
 */
...
static const bcm_iovar_t ulb_iovars[] = {
	{"ulb_mode", IOV_ULB_MODE, (IOVF_SET_DOWN), 0, IOVT_UINT8, 0},
	{"ulb_bw", IOV_ULB_BW, (IOVF_OPEN_ALLOW), 0, IOVT_UINT8, 0},
	{"ulb_mode_sw_req", IOV_ULB_MODE_SW_REQ, (IOVF_SET_UP), 0, IOVT_UINT32, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_ulb_doiovar(void *hdl, uint32 actionid,
...
BCMATTACHFN(wlc_ulb_attach)(wlc_info_t *wlc)
{
	...
	ulb_info->ulb_mode = ULB_MODE_STD_ALONE_MODE;
	...
	/* Set phy feature flag corresonding to ulb mode */
	wlc_phy_ulb_feature_flag_set(wlc->pi);
	/*
...
wlc_ulb_cfg_cubby_init(void *ctx, wlc_bsscfg_t *cfg)
{
	 ...
	 */
	 if (wlc_ulb_set_bss_min_bw(ulb_info, cfg, CHSPEC_BW(wlc->default_bss->chanspec), FALSE)) {
...
static int
wlc_ulb_doiovar(void *hdl, uint32 actionid,
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_ULB_BW): {
			...
			*ret_uint_ptr = wlc_ulb_conv_chspec_bw_2_ulb_bw(bss_ulb->min_bw);
			break;
		}
...
uint
wlc_ulb_conv_chspec_bw_2_ulb_bw(uint16 chspec_bw)
{
	uint retval = 0;
	switch (chspec_bw) {
		case WL_CHANSPEC_BW_2P5:
	...
	switch (data->ft) {
		...
		case FC_REASSOC_REQ:
			...
			if (!wlc_ulb_validate_opr_field(ulb_info, cur_chspec,
				...
				(wlc_ulb_conv_chspec_bw_2_ulb_bw(bss_ulb->min_bw) !=
	...
	ulb_opr_attr_t *opr_attr)
{
	...
	opr_attr->ulb_opr_field.cur_opr_bw = wlc_ulb_conv_chspec_bw_2_ulb_bw(min_bw);
	opr_attr->ulb_opr_field.pri_opr_bw = wlc_ulb_conv_chspec_bw_2_ulb_bw(min_bw);
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	 ...
	 * bandwidth
	 */
	else if (opr_field->cur_opr_bw == wlc_ulb_conv_chspec_bw_2_ulb_bw(bss_ulb->min_bw)) {
		rsn_code = 5;
		goto fail;
	}
...
#endif /* PROP_TXSTATUS */
		wlc_set_chanspec(wlc, new_chspec, CHANSW_ULB);
		wlc_enable_mac(wlc);
		...
		if (++BSS_ULB_GET_MODE_SW_AF_RTCNT(bss_ulb) <= MAX_NUM_MODE_SW_AF_RETRY_CNT) {
			...
			if (scb == NULL) {
				WL_ERROR(("wl%d: %s: unable to find scb from the pkt %p\n",
					wlc->pub->unit, __FUNCTION__,
					OSL_OBFUSCATE_BUF(BSS_ULB_GET_MODE_SW_PKT(bss_ulb))));
