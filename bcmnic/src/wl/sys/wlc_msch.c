...
*
* $Id: wlc_msch.c 643250 2016-06-14 01:32:09Z $
*/
...
_msch_slot_skip(wlc_msch_info_t *msch_info,  msch_req_entity_t *entity)
{
	...
	if (req_param->interval) {
		 ...
		 */
		if ((!cur_ts || (start_time >= (cur_ts->end_time + chansw_time))) &&
			n_ts && (start_time <= (n_ts->end_time + chansw_time))) {
				...
				*/
				if (entity->req_hdl->req_param->flags &
					...
					MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
						"%s: ts_d idx %d chan 0x%x New start time %u\n",
...
_msch_req_entity_alloc(wlc_msch_info_t *msch_info)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_INFORM_ON,
		"%s: req_entity ALLOC %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(req_entity)));
...
_msch_chan_ctxt_alloc(wlc_msch_info_t *msch_info, chanspec_t chanspec)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_INFORM_ON,
		"%s: chan_ctxt ALLOC %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(chan_ctxt)));
	...
	msch_chan_ctxt_t *chan_ctxt, wlc_msch_req_param_t *req_param, msch_req_timing_t *slot)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"%s: TS REQ chan 0x%x type 0x%x pre_start_time %s duration %u\n", __FUNCTION__,
	...
	if (ts == msch_info->cur_msch_timeslot) {
		...
		if (new_end_time) {
			if (new_end_time != ts->end_time) {
				...
				MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
					"%s: Updated cur ts end_time to %s!!\n", __FUNCTION__,
	...
	wlc_msch_req_handle_t *req_hdl, uint16 chan_idx)
{
	...
	if (!MSCH_START_FLEX(req_param->req_type)) {
		...
		chansw_time = wlc_msch_calc_avg_chswtime(msch_info, req_entity->chanspec);
	...
	if (cur_ts) {
		...
		if (cur_ts->state == MSCH_TIMER_STATE_TS_COMPLETE) {
			if (!next_ts || next_ts->chan_ctxt != cur_chan_ctxt ||
				_msch_check_bf_timeslot(msch_info, next_ts, FALSE)) {
...
_msch_next_chn_seq_pend_slot(wlc_msch_info_t *msch_info, msch_req_entity_t *req_entity)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON, "%s: start %u end %u\n",
		__FUNCTION__, (uint32)req_entity->pend_slot.start_time,
		...
		if (req_param->interval) {
			...
			if (req_entity->pend_slot.start_time < cur_time) {
				...
				gap_in_intervals = bcm_uint64_div((uint32)(time_gap >> 32),
					(uint32)time_gap, req_param->interval);
				req_entity->pend_slot.start_time +=
					((uint64)req_param->duration * (gap_in_intervals + 1));
...
_msch_schd_new_piggyback_curts(wlc_msch_info_t *msch_info, msch_req_entity_t *new_entity)
{
	...
	if (MSCH_START_FIXED(req_param->req_type)) {
		...
		chansw_time = wlc_msch_calc_avg_chswtime(msch_info, new_entity->chanspec);
		/* since the callback happens in timer, account for the delay */
		dur_left -= chansw_time;
		if (MSCH_START_FLEX(req_param->req_type) &&
	...
	uint64 slot_start_time)
{
	...
	if (entity->pend_slot.timeslot) {
		MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
			"wl%d: %s: timeslot already scheduled\n", msch_info->wlc->pub->unit,
			__FUNCTION__));
		ASSERT(0);
		return;
	}
	...
	if (req_hdl->req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		if (entity->pend_slot.flags & MSCH_RC_FLAGS_SPLIT_SLOT_START) {
			...
			if ((slot_start_time < (entity->pend_slot.start_time + prep_dur)) &&
				...
				MSCH_MESSAGE((msch_info, MSCH_INFORM_ON,
					"%s, update remaing slot start_time %s\n", __FUNCTION__,
		...
		if ((next_chan != (chanspec_t)-1) &&
			(wf_chspec_ctlchan(entity->chan_ctxt->chanspec) !=
			wf_chspec_ctlchan(next_chan))) {
...
_msch_slotskip_timer(void *arg)
{
	...
	uint64 cur_time, slot_due, slot_end_time, prep_time = 0;
	int switch_dur;
	msch_timeslot_t *cur_ts;
	msch_timeslot_t *next_ts;
	msch_req_entity_t *entity = NULL;
	msch_ret_type_t ret;
	msch_chan_ctxt_t *last_chan_ctxt;
	uint32 chansw_time;
	MSCH_MESSAGE((msch_info, MSCH_TRACE_ON, "%s: Entry\n", __FUNCTION__));
	if (!msch_info) {
	...
	while ((req_elem = msch_list_iterate(req_elem))) {
		...
		if (!next_ts) {
			...
			last_chan_ctxt = msch_info->cur_msch_timeslot->chan_ctxt;
		} else {
			slot_end_time = next_ts->end_time;
			last_chan_ctxt = msch_info->next_timeslot->chan_ctxt;
		}
		if (last_chan_ctxt->bf_sch_pending) {
			prep_time = MSCH_MAX_OFFCB_DUR;
		}
		...
		MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
			"%s:  entity %p start time : %s, channel 0x%x\n", __FUNCTION__, entity,
			_msch_display_time(entity->pend_slot.start_time), entity->chanspec));

		if (entity->chan_ctxt != last_chan_ctxt) {
			chansw_time = wlc_msch_calc_avg_chswtime(msch_info, entity->chanspec);
			slot_due = entity->pend_slot.start_time - (prep_time + chansw_time);
		} else {
			slot_due = entity->pend_slot.start_time - prep_time;
		}

		MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON, "cur time : %s slot_due %u\n",
			_msch_display_time(cur_time), (uint32)slot_due));
		switch_dur = (int)(slot_due - MSCH_ONCHAN_PREPARE - cur_time);
...
_msch_schd(wlc_msch_info_t *msch_info)
{
	...
	uint64 slot_dur = -1;
	uint32 prep_dur = wlc_msch_calc_avg_chswtime(msch_info, 0);
	msch_req_entity_t *entity = NULL;
	...
	if (sfix_entity && ((sfix_slot->start_time <= (slot_start_time + prep_dur)) ||
		...
		if (sfix_prio >= dfix_prio && sfix_prio >= bflex_prio) {
			...
			if (!sfix_entity->cur_slot.timeslot ||
				...
				if (cur_chan_ctxt == sfix_entity->chan_ctxt) {
					chansw_time = wlc_msch_calc_avg_chswtime(msch_info,
						sfix_entity->chan_ctxt->chanspec);
					prep_dur -= chansw_time;
				}
				/* sfix is due, schedule it as next timeslot */
				_msch_sch_fixed(msch_info, sfix_entity, prep_dur, slot_start_time);
				goto done;
			}
		}
	}
	...
	if (dfix_entity && dfix_prio >= bflex_prio) {
		...
		req_dur = dfix_entity->req_hdl->req_param->duration + prep_dur +
			chansw_time;
		if (cur_chan_ctxt == dfix_entity->chan_ctxt) {
			req_dur -= chansw_time;
		}
	...
	if (bflex_entity && (bflex_prio > sfix_prio)) {
		bf_min_dur = bflex_entity->req_hdl->req_param->flex.bf.min_dur + prep_dur +
			MSCH_PREPARE_DUR;
	...
	if (bflex_entity && (slot_dur >= (prep_dur + MSCH_MIN_FREE_SLOT))) {
		if (cur_chan_ctxt == bflex_entity->chan_ctxt) {
			chansw_time = wlc_msch_calc_avg_chswtime(msch_info,
		...
		/* VSDB is in use */
		if (msch_info->flex_list_cnt > 1) {
			bf_start_time = (bflex_entity->pend_slot.start_time - prep_dur);
			...
			if (cur_ts && (slot_start_time > bf_start_time)) {
				...
				if (slot_dur >= (prep_dur + MSCH_MIN_FREE_SLOT +
		...
		if (slot_dur && slot_dur != -1) {
			slot_dur -= (prep_dur + MSCH_PREPARE_DUR);
		...
		if (slot_dur && _msch_sch_both_flex(msch_info, slot_start_time,
			slot_dur, bflex_entity, prep_dur) == MSCH_OK) {
			goto done;
		}
	}
	...
	uint32 prep_dur, uint64 slot_dur)
{
	 ...
	 */
	if (!_msch_check_bf_timeslot(msch_info, ts, FALSE)) {
	...
	chanspec_t chspec)
{
	...
	if ((next_chan != (chanspec_t)-1) &&
		(wf_chspec_ctlchan(entity->chan_ctxt->chanspec) == wf_chspec_ctlchan(next_chan)) &&
		(wf_chspec_ctlchan(next_chan) != wf_chspec_ctlchan(chspec))) {
		return TRUE;
	}
	return FALSE;
}
	...
	chanspec_t chanspec, uint64 *curts_end_time, uint32 flags, bool *split_curts)
{
	...
	if (!curts_end_time) {
		MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
			"%s: Cur ts end time is not valid\n", __FUNCTION__));
		return -1;
	}

	if (!split_curts) {
		MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
			"%s: split_curts %p Null parameters\n", __FUNCTION__, split_curts));
		ASSERT(0);
		return -1;
	}
	...
	*split_curts = ((flags & MSCH_REQ_FLAGS_PREMT_IMMEDIATE) != 0);
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"%s: Cur ts end_time %s\n", __FUNCTION__, _msch_display_time(ts_end_time)));
	...
	if (!ts_end_time || ts_end_time == -1) {
		...
		preempt_slot_prep += MSCH_MAX_OFFCB_DUR;
		if (wf_chspec_ctlchan(chanspec) != wf_chspec_ctlchan(cur_ts->chan_ctxt->chanspec)) {
			chansw_time = wlc_msch_calc_avg_chswtime(msch_info,
				chanspec);
	...
	if (_msch_check_bf_timeslot(msch_info, cur_ts, TRUE)) {
		preempt_slot_prep += MSCH_MAX_OFFCB_DUR;
		ts_end_time -= MSCH_MAX_OFFCB_DUR;
	}
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (entity->cur_slot.timeslot &&
			...
			if (!(entity->req_hdl->req_param->flags &
				...
				MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
					"%s:preempt req fail, cur ts does not support\n",
					__FUNCTION__));
				return -1;
			}
			if (!extra_prep_time &&
	...
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"%s: preempt_start_time %s curts_end_time %s\n", __FUNCTION__,
...
_msch_reduce_curts(wlc_msch_info_t *msch_info, uint64 end_time)
{
	...
	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"%s: Cutting ts end time %s\n", __FUNCTION__,
		_msch_display_time(msch_info->cur_msch_timeslot->end_time)));

	MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
		"%s: Updating current ts end time to %s\n", __FUNCTION__,
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		entity->cur_slot.end_time = end_time;
		if (!MSCH_BOTH_FLEX(entity->req_hdl->req_param->req_type) &&
			(entity->cur_slot.flags & MSCH_RC_FLAGS_START_FIRE_DONE)) {
...
_msch_check_param(wlc_msch_info_t *msch_info, wlc_msch_req_param_t *param)
{
	...
	if (param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		if (!MSCH_BOTH_FIXED(param->req_type)) {
			WL_ERROR(("wl%d: %s: wrong request type for chan contigous,"
		...
		if (time_check < cur_time) {
			WL_ERROR(("wl%d: %s: start time not in future\n",
				msch_info->wlc->pub->unit, __FUNCTION__));
			return BCME_BADARG;
		}
		return BCME_OK;
	...
	if (MSCH_START_FIXED(param->req_type)) {
		...
		if (!param->interval && (time_check < (cur_time + MSCH_PREPARE_DUR))) {
			WL_ERROR(("wl%d: %s: start time not in future\n",
				msch_info->wlc->pub->unit, __FUNCTION__));
			return BCME_BADARG;
		}
	...
	wlc_msch_req_handle_t **p_req_hdl)
{
	...
	if (MSCH_PREEMPT_REQ(req_param->flags)) {
		if (!MSCH_START_FLEX(req_param->req_type) ||
			...
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
				"%s: Request not supported, type %d dur %d flags 0x%x\n",
				__FUNCTION__, req_param->req_type, req_param->duration,
				req_param->flags));
			return BCME_BADARG;
		}
		if ((preempt_time = _msch_check_curts_preemption(msch_info, req_param->duration,
			chanspec_list[0], &new_end_time, req_param->flags, &split_curts)) == -1) {
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
				"%s: current timeslot not pre-emptable.\n", __FUNCTION__));
	...
	if ((req_param->flags & MSCH_REQ_FLAGS_MERGE_CONT_SLOTS) &&
		...
		MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
			"%s: Merge slots set but duration (%d) and interval (%d) are different\n",
			__FUNCTION__, req_param->duration, req_param->interval));
		return BCME_BADARG;
	}
	...
	if (req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		...
		if (!req_hdl->chn_list) {
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
				"%s: chanspec list memory alloc (%d bytes) failed!!\n",
				__FUNCTION__, ((sizeof(chanspec_t) * chanspec_cnt))));
			goto fail;
		}
	...
	wlc_msch_req_param_t *preempt_param, chanspec_t preempt_chanspec)
{
	...
	if (wf_chspec_ctlchan(preempt_chanspec) != wf_chspec_ctlchan(ts->chan_ctxt->chanspec)) {
		chansw_time = wlc_msch_calc_avg_chswtime(msch_info,
			preempt_chanspec);
	...
	*/
	...
	if (drift < 0) {
		...
		if (_msch_check_curts_preemption(msch_info, ABS(drift), entity->chanspec,
			...
			MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
				"%s: Cutting current ts end time from %s to %s\n", __FUNCTION__,
				_msch_display_time(msch_info->cur_msch_timeslot->end_time),
	...
	wlc_msch_req_param_t *param, uint32 update_mask)
{
	...
	if (update_mask & MSCH_UPDATE_START_TIME) {
		...
		if (param_req_type == MSCH_RT_START_FLEX ||
			...
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
				"wl%d: %s:error update start time"
		...
		*/
		...
		if (cur_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
			if ((uint32)ABS(drift) > cur_param->interval) {
				MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
						"wl%d: %s: drift is more than"
						"interval [drift %d]\n",
		...
		while ((elem = msch_list_iterate(elem))) {
			...
			MSCH_MESSAGE((msch_info, MSCH_ERROR_ON,
				"%s: idx %d chan 0x%x drift %d New start time %u\n",
				__FUNCTION__, entity->cur_chn_idx, entity->chanspec,
...
_msch_chan_cont_req_update(wlc_msch_info_t * msch_info, wlc_msch_req_handle_t *req_hdl)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		MSCH_MESSAGE((msch_info, MSCH_DEBUG_ON,
			"%s: idx %d chan 0x%x New start time %u\n",
			__FUNCTION__, entity->cur_chn_idx, entity->chanspec,
			(uint32)entity->pend_slot.start_time));
...
uint32
BCMRAMFN(wlc_msch_calc_avg_chswtime)(wlc_msch_info_t *msch_info, chanspec_t to)
{
