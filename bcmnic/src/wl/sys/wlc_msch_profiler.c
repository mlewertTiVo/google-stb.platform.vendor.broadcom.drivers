...
*
* $Id: wlc_msch_profiler.c 639253 2016-05-21 01:31:10Z $
*
*/
...
#define MSCH_RSDB_PROFILER_VER	1
...
static void _msch_dump_data(wlc_msch_profiler_info_t *profiler_info, char *data, int type)
{
	...
	if (profiler_info->cmn->lastMessages && (type != WLC_PROFILER_MESSAGE)) {
		MSCH_EVENTS_PRINTF(mschbufp, "\n");
...
_msch_write_profiler_tlv(wlc_msch_profiler_info_t *profiler_info, uint type, int len)
{
	...
	if (cmn->ver >= MSCH_RSDB_PROFILER_VER) {
		type |= (profiler_info->wlc->pub->unit << WLC_PROFILER_WLINDEX_SHIFT);
	}
	...
	if (cmn->dump_status & MSCH_PROFILE_BIT) {
		...
		if (copysize > 0)
			bcopy(cmn->edata, p->value, copysize);
		if (copysize < len) {
			size = len - copysize;
			bcopy(cmn->edata + copysize, profiler->buffer, size);
	...
	wlc_msch_req_param_t *req_param)
{
	...
	edata = (msch_req_param_profiler_data_t *)(profiler_info->cmn->edata + p);
	...
	msch_timeslot_t *timeslot)
{
	...
	edata = (msch_timeslot_profiler_data_t *)(profiler_info->cmn->edata + p);
	...
	msch_req_timing_t *req_timing, bool allowEmpty)
{
	...
	edata = (msch_req_timing_profiler_data_t *)(profiler_info->cmn->edata + p);
	...
	msch_chan_ctxt_t *chan_ctxt)
{
	...
	edata = (msch_chan_ctxt_profiler_data_t *)(profiler_info->cmn->edata + p);
	...
	msch_req_entity_t *req_entity)
{
	...
	edata = (msch_req_entity_profiler_data_t *)(profiler_info->cmn->edata + p);
	...
	wlc_msch_req_handle_t *req_handle)
{
	...
	edata = (msch_req_handle_profiler_data_t *)(profiler_info->cmn->edata + p);
...
_msch_dump_profiler(wlc_msch_profiler_info_t *profiler_info)
{
	...
	if (cmn->dump_status) {
		...
		msch_profiler_profiler_data_t *edata =
			(msch_profiler_profiler_data_t *)cmn->edata;
	...
	int chanspec_cnt, wlc_msch_req_param_t *req_param)
{
	...
	if (cmn->dump_status) {
		...
		msch_req_profiler_data_t *edata =
			(msch_req_profiler_data_t *)cmn->edata;
...
_msch_dump_callback(wlc_msch_profiler_info_t *profiler_info, wlc_msch_cb_info_t *cb_info)
{
	...
	if (cmn->dump_status) {
		...
		msch_callback_profiler_data_t *edata =
			(msch_callback_profiler_data_t *)cmn->edata;
		uint16 len = ROUNDUP(OFFSETOF(msch_callback_profiler_data_t, start_time_l));
...
msch_message_printf(wlc_msch_info_t *msch_info, int filter, const char *fmt, ...)
{
	...
	if (!filter || (cmn->dump_print_messages & filter)) {
	...
	if (cmn->dump_status) {
		...
		msch_message_profiler_data_t *edata =
			(msch_message_profiler_data_t *)cmn->edata;
...
_msch_tesing_callback(void *ctx, wlc_msch_cb_info_t *cb_info)
{
	...
	MSCH_TESTING_DEBUG(("wl%d: callback: %s: type 0x%02x chan 0x%04x(%d)\n",
	...
	void *params, uint p_len, void *arg, uint a_len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_MSCH_EVENT):
		*ret_int_ptr = (cmn->dump_enable & MSCH_EVENT_BIT)?
		(MSCH_CMD_ENABLE_BIT |
			((cmn->dump_profiler & MSCH_EVENT_BIT)?
			MSCH_CMD_PROFILE_BIT : 0) |
			((cmn->dump_callback & MSCH_EVENT_BIT)?
			MSCH_CMD_CALLBACK_BIT : 0) |
			((cmn->dump_register & MSCH_EVENT_BIT)?
			MSCH_CMD_REGISTER_BIT : 0) |
			((cmn->dump_event_messages & MSCH_ERROR_ON)?
			MSCH_CMD_ERROR_BIT : 0) |
			((cmn->dump_event_messages & MSCH_DEBUG_ON)?
			MSCH_CMD_DEBUG_BIT : 0) |
			((cmn->dump_event_messages & MSCH_INFORM_ON)?
			MSCH_CMD_INFOM_BIT : 0) |
			((cmn->dump_event_messages & MSCH_TRACE_ON)?
	...
	case IOV_SVAL(IOV_MSCH_EVENT):
		if (int_val & MSCH_CMD_ENABLE_BIT) {
			cmn->ver = (int_val & MSCH_CMD_VER_MASK) >> MSCH_CMD_VER_SHIFT;
			...
			if (!cmn->edata) {
				cmn->edata = MALLOC(wlc->osh, WLC_PROFILER_BUFFER_SIZE);
				if (cmn->edata == NULL) {
					err = BCME_NOMEM;
		...
		if (cmn->edata) {
			...
			edata = (msch_start_profiler_data_t *)cmn->edata;
			edata->time_hi = hton32((uint32)(cur_time >> 32));
			edata->time_lo = hton32((uint32)(cur_time));
		...
		if (!(int_val & MSCH_CMD_ENABLE_BIT)) {
			...
			if (cmn->edata &&
				...
				MFREE(wlc->osh, cmn->edata, WLC_PROFILER_BUFFER_SIZE);
				cmn->edata = NULL;
			}
		}
		break;
	...
	case IOV_GVAL(IOV_MSCH_COLLECT):
		*ret_int_ptr = (cmn->dump_enable & MSCH_PROFILE_BIT)?
			(MSCH_CMD_ENABLE_BIT |
			((cmn->dump_profiler & MSCH_PROFILE_BIT)?
			MSCH_CMD_PROFILE_BIT : 0) |
			((cmn->dump_callback & MSCH_PROFILE_BIT)?
			MSCH_CMD_CALLBACK_BIT : 0) |
			((cmn->dump_register & MSCH_PROFILE_BIT)?
			MSCH_CMD_REGISTER_BIT : 0) |
			((cmn->dump_print_messages & MSCH_ERROR_ON)?
			MSCH_CMD_ERROR_BIT : 0) |
			((cmn->dump_print_messages & MSCH_DEBUG_ON)?
			MSCH_CMD_DEBUG_BIT : 0) |
			((cmn->dump_print_messages & MSCH_INFORM_ON)?
			MSCH_CMD_INFOM_BIT : 0) |
			((cmn->dump_print_messages & MSCH_TRACE_ON)?
			MSCH_CMD_TRACE_BIT : 0)) :
	...
	case IOV_SVAL(IOV_MSCH_COLLECT):
		if (int_val & MSCH_CMD_ENABLE_BIT) {
			...
			if (cmn->edata && !cmn->profiler) {
				int size = ((int_val & MSCH_CMD_SIZE_MASK) >>
					MSCH_CMD_SIZE_SHIFT) * 1024;
...
BCMATTACHFN(wlc_msch_profiler_detach)(wlc_msch_profiler_info_t *profiler_info)
{
	...
	if (cmn != NULL) {
		if (obj_registry_unref(wlc->objr, OBJR_MSCH_PROFILER_INFO) == 0) {
			...
			if (cmn->profiler) {
				MFREE(wlc->osh, cmn->profiler, sizeof(wlc_msch_profiler_t) +
					cmn->profiler->total_size);
			}
			if (cmn->edata) {
				MFREE(wlc->osh, cmn->edata, WLC_PROFILER_BUFFER_SIZE);
