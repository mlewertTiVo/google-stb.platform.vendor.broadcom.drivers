 ...
 *
 * $Id: dhdu.c 588552 2015-09-24 13:58:27Z $
 */
...
static cmd_func_t dhd_membytes, dhd_download, dhd_dldn,
	...
	dhd_macdbg_pmac, dhd_dump_sctpl;
...
cmd_t dhd_cmds[] = {
	...
	"bitmap : 32-bit value\n"
	"-v: verbose mode"},
	{ "dump_sctpl", dhd_dump_sctpl, -1, -1,
	"Read sample capture template area\n"
	"\tusage: dhd -i IF dump_sctpl [-k]\n"
...
static uint16
dhd_get_ihr16(void *dhd, uint16 addr)
{
	sdreg_t sdreg;
	uint16 *pval, val;
	...
	dhd_var_getbuf(dhd, "sbreg", &sdreg, sizeof(sdreg), (void**)&pval);
	val = *pval;
	return val;
}

static void
dhd_set_ihr16(void *dhd, uint16 addr, uint16 val)
{
	sdreg_t sdreg;
	...
	sdreg.offset = (0x1000 | addr);
	sdreg.value = val;
	dhd_var_setbuf(dhd, "sbreg", &sdreg, sizeof(sdreg));
}

static uint32
dhd_get_ihr32(void *dhd, uint16 addr)
{
	sdreg_t sdreg;
	uint32 *pval, val;
...
static void
dhd_set_ihr32(void *dhd, uint16 addr, uint32 val)
{
	sdreg_t sdreg;
...
static int
dhd_dump_sctpl(void *dhd, cmd_t *cmd, char **argv)
{
	int err = BCME_OK;
	uint32 val;
	uint32 gpio_sel;
	uint16 scpctl, addr0, addr1, curptr, len, offset, i;
	char *p, opt;
	bool killarm = FALSE;
	...
	if (killarm) {
		printf("PCIE: type \"wl bus:disconnect 99\", and retry\n");
		goto exit;
	}

	scpctl = dhd_get_ihr16(dhd, 0xb2e);
	/* stop sample capture */
	dhd_set_ihr16(dhd, 0xb2e, 0);
	gpio_sel = dhd_get_ihr32(dhd, 0x1a0);
	addr0 = dhd_get_ihr16(dhd, 0x552);
	addr1 = dhd_get_ihr16(dhd, 0x554);
	curptr = dhd_get_ihr16(dhd, 0x556);
	len = (addr1 - addr0 + 1) * 4;
	offset = addr0 * 4;
	...
	while ((val = dhd_get_ihr32(dhd, 0x130)) & 3);
	dhd_set_ihr32(dhd, 0x130, offset | 2);

	for (i = 0; i < (uint)len; i += 4) {
		while ((val = dhd_get_ihr32(dhd, 0x130)) & 3);
		val = dhd_get_ihr32(dhd, 0x134);
		printf("%04X: %04X %04X\n", i, (val & 0xffff), (val >> 16));
	}
exit:
	return err;
...
static uint32
dhd_get_d11obj32(void *dhd, uint16 objaddr, uint32 sel, bool verbose)
{
	uint32 *pval, val;
	sdreg_t sdreg;
...
objaddr_read:
	...
	if (verbose) {
		printf("DEBUG: %s: Indirect: Read 0x%08x, size %d, value 0x%08x\n",
			(sel & 0x00020000) ? "SCR":"SHM",
			(sdreg.offset + 0x18000000), sdreg.func, *pval);
	}
...
static uint16
dhd_get_d11obj16(void *dhd, uint16 objaddr, uint32 sel, bool cache_valid, bool verbose)
{
	uint32 val;
	if (cache_valid && ((obj_cache.sel ^ sel) & (0xffffff)) == 0) {
		if (obj_cache.addr32 == (objaddr & ~0x3)) {
			if (verbose) {
				printf("DEBUG: %s: Read cache value: addr32 0x%04x, "
			...
			if (verbose) {
				printf("DEBUG: %s: Read objdata only: addr32 0x%04x, "
					"sel 0x%08x, value 0x%08x\n",
					(sel & 0x00020000) ? "SCR":"SHM",
					obj_cache.addr32, obj_cache.sel, obj_cache.val);
			}
			val = dhd_get_d11obj32(dhd, 0xffff, sel, verbose);
			goto exit;
		}
	}
	val = dhd_get_d11obj32(dhd, (objaddr & ~0x2), sel, verbose);
exit:
...
static int
dhd_print_d11reg(void *dhd, int idx, int type, uint16 addr, bool verbose)
{
	...
	if (type == D11REG_TYPE_IHR32) {
		...
		val = dhd_get_ihr32(dhd, addr);
		if (verbose) {
			printf("DEBUG: IHR32: read 0x%08x, size %d, value 0x%08x\n",
				(addr + 0x18001000), 4, val);
		}
		...
		case D11REG_TYPE_IHR16: {
			...
			val = dhd_get_ihr16(dhd, addr);
			if (verbose) {
				printf("DEBUG: IHR16: read 0x%08x, size %d, value 0x%08x\n",
					(addr + 0x18001000), 2, val);
			}
			break;
		}
		case D11REG_TYPE_IHRX16:
			val = dhd_get_d11obj16(dhd, (addr - 0x400) << 1, 0x020b0000,
				(idx != 0), verbose);
			break;
		case D11REG_TYPE_SCR:
			val = dhd_get_d11obj16(dhd, addr << 2, 0x02020000, (idx != 0), verbose);
			break;
		case D11REG_TYPE_SCRX:
			val = dhd_get_d11obj16(dhd, addr << 2, 0x020a0000, (idx != 0), verbose);
			break;
		case D11REG_TYPE_SHM:
			val = dhd_get_d11obj16(dhd, addr, 0x02010000, (idx != 0), verbose);
			break;
		case D11REG_TYPE_SHMX:
			val = dhd_get_d11obj16(dhd, addr, 0x02090000, (idx != 0), verbose);
			break;
		default:
...
static int
dhd_print_d11regs(void *dhd, d11regs_list_t *pregs, bool verbose)
{
	uint16 addr;
	int idx = 0;
	...
	if (pregs->bitmap) {
		while (pregs->bitmap) {
			if (pregs->bitmap && (pregs->bitmap & 0x1)) {
				dhd_print_d11reg(dhd, idx, pregs->type, addr, verbose);
				idx++;
			}
...
static int
dhd_macdbg_pmac(void *dhd, cmd_t *cmd, char **argv)
{
	int err = BCME_OK;
	...
	if (reglist.type == D11REG_TYPE_SHM && reglist.addr == 0x0 &&
		reglist.bitmap == 0x0 && reglist.step == 2 &&
		reglist.cnt == (uint16)(-1)) {
		uint32 val;
		val = dhd_get_ihr32(dhd, 0x15c);
		switch ((val & 0xe) >> 1) {
			case 0: reglist.cnt = (4*1024); break;
			case 1: reglist.cnt = (6*1024); break;
			case 2: reglist.cnt = (8*1024); break;
			default: printf("Invalid val %d!!\n", val);
		}
		if (verbose) {
			printf("DEBUG: SHM SIZE: read 0x%08x, size %d, value 0x%08x, shmsize %d\n",
				(0x1800115c), 4, val, reglist.cnt);
		}
	}

	err = (dhd_print_d11regs(dhd, &reglist, verbose) > 0) ?
		BCME_OK : BCME_ERROR;
exit:
	return err;
}
