...
if [ ${TARGETARCH} == x86 ] ; then
	OBJDIR=dhd/src/p2p/p2plib/linux/sampleapp/obj/x86-debug-intsec
elif [ ${TARGETARCH} == mips ] ; then
	if [ "${TARGETMACH}" == mipseb ] ; then 
		export CROSS_COMPILE=mips-linux-	
		export BUILDCFG=mipseb-mips-
		export STBLINUX=1
		OBJDIR=dhd/src/p2p/p2plib/linux/sampleapp/obj/mips-debug-intsec
	elif [ "${TARGETMACH}" == mipsel ] ; then
		export CROSS_COMPILE=mipsel-linux-	
		export BUILDCFG=mipsel-mips-
		export STBLINUX=1
		OBJDIR=dhd/src/p2p/p2plib/linux/sampleapp/obj/mipsel-debug-intsec
	else
	...
	if [ "${TARGETMACH}" == armle ] ; then 
		...
		export STBLINUX=1
		OBJDIR=dhd/src/p2p/p2plib/linux/sampleapp/obj/arm-debug-intsec
...
make -C dhd/src/p2p/p2plib/linux/sampleapp BCM_P2P_IOTYPECOMPAT=1 $1
