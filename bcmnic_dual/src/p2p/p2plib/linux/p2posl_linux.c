...
int
p2posl_wl_ioctl_core(p2posl_wl_hdl_t *wl, int cmd, void *buf, int len,
	P2PWL_BOOL set, struct ifreq *ifr)
{
	int s;
	wl_ioctl_t ioc;
	int ret = 0;
...
*/
	...
	ioc.len = len;
	ioc.set = set;
	pthread_mutex_lock(&wl->ioctl_mutex);
	ifr->ifr_data = (caddr_t) &ioc;
	ret = ioctl(s, SIOCDEVPRIVATE, ifr);
	pthread_mutex_unlock(&wl->ioctl_mutex);
/*
	BCMP2PLOG((BCMP2P_LOG_VERB, TRUE,
	...
	int bsscfg_idx)
{
	...
	if (bsscfg_idx == 0) {
		BCMP2PLOG((BCMP2P_LOG_VERB, TRUE, "   (on primary ifr)\n"));
		ifr = &wl->primary_ifr;
	} else if (bsscfg_idx == wl->connection_bssidx) {
		BCMP2PLOG((BCMP2P_LOG_VERB, TRUE, "   (on connection ifr)\n"));
		ifr = &wl->connection_ifr;
	} else if (bsscfg_idx == wl->discovery_bssidx) {
		BCMP2PLOG((BCMP2P_LOG_VERB, TRUE, "   (on discovery ifr)\n"));
		ifr = &wl->discovery_ifr;
	} else {
		BCMP2PLOG((BCMP2P_LOG_MED, TRUE, "   (on unknown ifr %d)\n",
			bsscfg_idx));
		ifr = &wl->primary_ifr;
	}

	return p2posl_wl_ioctl_core(wl, cmd, buf, len, set, ifr);
}
