...
#elif defined(linux) && defined(STBLINUX) && !defined(IL_BIGENDIAN)
...
void
p2papp_run_script(char *script_name, char *apsta, char *ifname, BCMP2P_BOOL enable_ping)
{
	char cmd[80];
...
p2papp_run_conn_script(void)
{
	...
	p2papp_run_script(script, apsta, ifname, p2papp_enable_ping);
}
...
p2papp_run_disc_script(void)
{
	...
	p2papp_run_script(script, apsta, ifname, 0);
}
...
#if defined(linux) && defined(STBLINUX)
	status = BCMP2PCreateGroup(p2papp_dev_hdl, (unsigned char*) ssid, TRUE, FALSE);
#else
	status = BCMP2PCreateGroup(p2papp_dev_hdl, (unsigned char*) ssid, TRUE);
#endif
	if (status == BCMP2P_SUCCESS)
