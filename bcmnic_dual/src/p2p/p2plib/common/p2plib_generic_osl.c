...
#include <BcmP2PAPI.h>
#if defined(linux) /* OS-specific p2posl include */
	#include <sys/types.h>
	#include <unistd.h>
	#include <pthread.h>
	#include <signal.h>
	...
	#include <p2posl_macos.h>
	#define RANDOM	random
#elif defined(TARGETOS_symbian)
	#define RANDOM	rand
	#include <p2posl_symbian.h>
#endif /* OS-specific p2posl include */
#include <p2posl.h>
...
p2papi_osl_delete_bss(p2papi_instance_t* hdl, int bssidx)
{
	...
	p2posl_save_bssidx(wl, P2PAPI_BSSCFG_CONNECTION, 0);
...
*/
		...
		if (ret == 0) {
			...
			if (ret == 0) {
				hdl->bssidx[P2PAPI_BSSCFG_CONNECTION] = index;
				p2posl_save_bssidx(wl, P2PAPI_BSSCFG_CONNECTION, index);
				BCMP2PLOG((BCMP2P_LOG_MED, TRUE,
