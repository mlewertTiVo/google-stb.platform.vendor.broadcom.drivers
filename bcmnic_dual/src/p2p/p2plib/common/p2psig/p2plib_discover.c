...
p2papi_generate_grp_cap_bitmap(p2papi_instance_t* hdl)
{
	...
	if (hdl->persistent_grp)
		grp_cap_bm |= P2P_CAPSE_PERSIST_GRP;
	if (hdl->is_intra_bss)
	...
	uint16 *total_ie_len)
{
	...
	devpwdid = hdl->ap_config.WPSConfig.wpsIsButtonPushed ?
		WPS_DEVICEPWDID_PUSH_BTN : WPS_DEVICEPWDID_DEFAULT;
	BCMP2PLOG((BCMP2P_LOG_MED, TRUE,
		"enc_prbresp_wps_ie: name=%s len=%u push=%d\n",
		name, name_len, hdl->ap_config.WPSConfig.wpsIsButtonPushed));
	BCMP2PLOG((BCMP2P_LOG_INFO, TRUE,
	...
	length = wpscli_softap_encode_probe_response_wps_ie(buf, max_length,
		cfg_methods, primdev_cat, primdev_subcat, devpwdid, device_name, window_is_open);
	...
	wifi_p2p_ie_t *p2p_ie, uint16 *ie_len)
{
	...
	*ie_len = 0;
	...
	if (ssid_len > 0) {
		/* Add the P2P Group ID attribute */
		subel = p2papi_enc_se_group_id(peer_dev_addr, ssid, ssid_len,
			subel, ie_len);
	}
...
p2papi_init_discovery(p2papi_instance_t* hdl)
{
	...
	hdl->bssidx[P2PAPI_BSSCFG_DEVICE] = index;
	BCMP2PLOG((BCMP2P_LOG_MED, TRUE,
		"p2papi_init_discovery: discov_bssidx=%d\n", index));
	p2posl_save_bssidx(wl, P2PAPI_BSSCFG_DEVICE, index);
	...
	if (ret != 0) {
		...
		hdl->bssidx[P2PAPI_BSSCFG_DEVICE] = 0;
		p2posl_save_bssidx(wl, P2PAPI_BSSCFG_DEVICE, 0);
		return 0;
	}
...
p2papi_add_to_peers_list(p2papi_instance_t* hdl, uint8 *name,
	...
	uint8 *ie_data, uint32 ie_data_len)
{
	...
	BCMP2PLOG((BCMP2P_LOG_MED, TRUE, "  ch=%d:%d isgrp=%d cfg_methods=%u\n",
		peer->listen_channel.channel_class, peer->listen_channel.channel,
		peer->is_p2p_group, peer->wps_cfg_methods));
