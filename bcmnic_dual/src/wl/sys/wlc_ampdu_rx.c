 ...
 *
 * $Id: wlc_ampdu_rx.c 636417 2016-05-09 12:03:55Z $
 */
...
struct scb_ampdu_tid_resp {
    ...
    uint16 rxq_pad[AMPDU_BA_MAX_WSIZE];
    void *wrxh_pad[AMPDU_BA_MAX_WSIZE];
#endif /* BCMPKTIDMAP_ROM_COMPAT */
#else  /* ! BCMPKTIDMAP */
...
/* set optional wsize override from bmac dongle */
	...
	if (VHT_ENAB(wlc->pub)) {
		...
		if (!(wlc->pub->is_ss & WL_SS_ENAB)) {
			ampdu_rx->rx_factor = AMPDU_RX_FACTOR_128K;
		}
#endif /* WLC_HIGH_ONLY */
	} else
#endif /* WL11AC */
...
/** host reorder feature specific */
static void *
wlc_ampdu_get_resparr_ptr(ampdu_rx_info_t *ampdu_rx, struct scb *scb, void *p)
{
	scb_ampdu_rx_t *scb_ampdu;
	scb_ampdu_tid_resp_t *resp;
...
wlc_ampdu_frwd_handle_host_reorder(ampdu_rx_info_t *ampdu_rx, void *pkt, bool forward)
{
	...
	if (ptr == NULL) {
		...
		if (pkttag->_scb != NULL) {
			...
			/* Initialize the reorder buffer pointer by ampdu reorder buffer pointer */
			ptr->p = wlc_ampdu_get_resparr_ptr(ampdu_rx, pkttag->_scb, pkt);
			if (ptr->p == NULL) {
				MFREE(wlc->osh, ptr, sizeof(ampdu_reorder_info_t));
				return pkt;
			}
		...
		if ((exp_idx == ptr->exp_idx) && (cur_idx != ptr->exp_idx)) {
			...
			if (forward) {
				...
				if (ptr->p[cur_idx] != NULL) {
					PKTFREE(wlc->osh, ptr->p[cur_idx], TRUE);
					ptr->pend_pkts--;
					ptr->p[cur_idx] = NULL;
				}
				ptr->p[cur_idx] = pkt;
