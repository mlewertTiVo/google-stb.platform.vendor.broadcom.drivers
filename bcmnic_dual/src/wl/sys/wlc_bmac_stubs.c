 ...
 *
 * $Id: wlc_bmac_stubs.c 524316 2015-01-06 13:54:20Z $
 */
...
void
wlc_bmac_radar_detect_upd(wlc_hw_info_t *wlc_hw, bool radar_on)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32), WLRPC_WLC_BMAC_RADAR_DETECT_ID);
	ASSERT(rpc_buf != NULL);

	err = bcm_xdr_pack_uint32(&b, (uint32)(radar_on));
	ASSERT(!err);
