...
#ifdef WLAMPDU
static void wlc_ampdu_agg_state_update_all(wlc_info_t *wlc, bool aggr);
static void wlc_ampdu_agg_state_update_tx_all(wlc_info_t *wlc, bool aggr);
#endif
	...
	if (!btch->siso_ack_ovr) {
		...
		if (siso_ack == AUTO) {
			...
			if (wlc->pub->boardflags & BFL_FEM_BT) {
				...
				if (wlc->hw->btc->btcx_aa == TXCORE01_MASK) { /* two antenna */
					if (wlc->pub->boardflags2 &
					    BFL2_BT_SHARE_ANT0) { /* core 0 shared */
						siso_ack = TXCORE1_MASK;
					} else {
						siso_ack = TXCORE0_MASK;
					}
				} else if (wlc->hw->btc->btcx_aa == TXCORE012_MASK) {
					; /* not supported yet */
				}
			} else
#endif /* WLC_LOW */
...
wlc_ampdu_agg_state_update_all(wlc_info_t *wlc, bool aggr)
{
	...
	FOREACH_BSS(wlc, idx, cfg) {
		...
		wlc_ampdu_agg_state_update_tx(wlc, cfg, aggr);
	}
}
...
#endif /* BCMLTECOEX */
					{
						wlc_ampdu_agg_state_update_all(wlc, ON);
					}
					if ((btc_mode == WL_BTC_HYBRID) &&
						(CHIPID(wlc->pub->sih->chip) == BCM43225_CHIP_ID) &&
        ...
        void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_BTC_SISO_ACK):
		wlc_btc_siso_ack_set(wlc, int_val, true);
		break;
