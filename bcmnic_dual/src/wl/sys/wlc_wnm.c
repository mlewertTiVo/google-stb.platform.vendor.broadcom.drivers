 ...
 *
 * $Id: wlc_wnm.c 563782 2015-06-15 16:21:20Z $
 */
...
typedef struct wnm_bsstrans_sta_info {
	...
	bool bssload_present; /* bssload_present in all targets */
	uint8 resp_token;
	uint8 req_mode;
		...
		if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA) {
			...
			if (!(nd_msg->icmph.opt.nd_advt.router) &&
				...
				struct nd_msg_opt *nd_msg_opt_reply = (struct nd_msg_opt *)
					(((uint8 *)nd_msg) + sizeof(struct nd_msg));
...
/* Check if all targets support bss_load element and update channel-free value in bss_info. */
void
wlc_wnm_process_join_trgts_bssload(wlc_info_t *wlc, wlc_bss_info_t **bip, int trgt_count)
{
	...
	for (i = 0; i < trgt_count; i++) {
		...
		if (qbss_load_ie == NULL ||
			...
			wlc->wnm_info->bsstrans_stainfo->bssload_present = FALSE;
	...
	wlc->wnm_info->bsstrans_stainfo->bssload_present = TRUE;
}
