 ...
 *
 * $Id: wlc_ap.c 576533 2015-08-04 01:43:45Z $
 */
...
#if defined(WLMSG_ASSOC) || defined(WLMSG_WSEC)
	char eabuf[ETHER_ADDR_STR_LEN], *sa = bcm_ether_ntoa(&hdr->sa, eabuf);
#endif
	...
	FOREACH_AP(wlc, i, bsscfg) {
		if (bsscfg->enable) {
			...
			if (bsscfg->up &&
				...
				wlc_bmac_radar_detect_upd(wlc->hw,
					(bool)wlc_dfs_get_radar(wlc->dfs));
				radar_ap = TRUE;
			}
#endif /* RADAR */
		}
...
wlc_ap_ioctl(void *hdl, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_RADAR:
		...
		if (AP_ACTIVE(wlc)) {
			wlc_bmac_radar_detect_upd(wlc->hw, val? ON: OFF);
		}
		break;
