 ...
 *
 * $Id: wlc_wowl.c 564022 2015-06-16 09:45:09Z $
 */
...
#if defined(WLC_HIGH_ONLY) || defined(WOWL_GPIO)
                 ...
                 void *p, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_WOWL_WAKEIND): {
		...
		if (!si_deviceremoved(wlc->pub->sih)) {
			wake->pci_wakeind = si_pci_pmestat(wlc->pub->sih);
		}
#else
	...
	case IOV_SVAL(IOV_WOWL_GPIOPOL):
		...
		break;
#endif /* defined( WLC_HIGH_ONLY ) || defined ( WOWL_GPIO ) */
...
enable:
	...
	if ((D11REV_IS(wlc->pub->corerev, 42) ||
		...
		/* GPIO 10 for 11AC */
		if (D11REV_IS(wlc->pub->corerev, 42)) {
			wlc_write_shm(wlc, M_WOWL_GPIOSEL, 1 << wlc->hw->wowl_gpio);
		}
	}
	/*
