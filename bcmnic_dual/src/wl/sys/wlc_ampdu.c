...
#endif
	{"ampdu_cfg_txaggr", IOV_AMPDU_CFG_TXAGGR, (0), IOVT_BUFFER,
	sizeof(struct ampdu_cfg_txaggr)},
	{NULL, 0, 0, 0, 0}
};
...
static void wlc_ampdu_tx_cleanup_per_tid(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
	ampdu_tx_info_t *ampdu_tx, uint8 tid);
...
struct ampdu_tx_cfg_cubby {
	uint8 txaggr[AMPDU_MAX_SCB_TID];
};
...
static void wlc_ampdu_bss_updn(void *ctx, bsscfg_up_down_event_data_t *evt);
...
#ifdef MFP
	if (!WLC_MFP_ENAB(ampdu_tx->wlc->pub) ||
		(WLC_MFP_ENAB(ampdu_tx->wlc->pub) && SCB_AUTHORIZED(scb)))
#endif
...
BCMATTACHFN(wlc_ampdu_tx_attach)(wlc_info_t *wlc)
{
	...
	if ((ampdu_tx->cfg_cubby_handle = wlc_bsscfg_cubby_reserve(wlc,
		sizeof(struct ampdu_tx_cfg_cubby), bsscfg_ampdu_tx_init, bsscfg_ampdu_tx_deinit,
	...
	if (wlc_bsscfg_updown_register(wlc, wlc_ampdu_bss_updn, ampdu_tx) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_bsscfg_updown_register failed\n",
			wlc->pub->unit));
		goto fail;
	}
...
static int
bsscfg_ampdu_tx_init(void *context, wlc_bsscfg_t *cfg)
{
	ampdu_tx_info_t *ampdu_info = (ampdu_tx_info_t *)context;
	struct ampdu_tx_cfg_cubby *cfg_cubby = BSSCFG_CUBBY(cfg, ampdu_info->cfg_cubby_handle);
	int idx;

	ASSERT(cfg != NULL);

	for (idx = 0; idx < AMPDU_MAX_SCB_TID; idx++)
		cfg_cubby->txaggr[idx] = 1;
	return BCME_OK;
}

/* bsscfg cubby deinit fn */
static void
bsscfg_ampdu_tx_deinit(void *context, wlc_bsscfg_t *cfg)
{
	/* do nothing for now */
...
static void
wlc_ampdu_tx_cleanup_per_tid(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	ampdu_tx_info_t *ampdu_tx, uint8 tid)
{
	scb_ampdu_tx_t *scb_ampdu = NULL;
	struct scb *scb;
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_AMPDU_CFG_TXAGGR): {
		struct ampdu_cfg_txaggr *txaggr = (struct ampdu_cfg_txaggr *)p;
		struct ampdu_tx_cfg_cubby *cfg_cubby = BSSCFG_CUBBY(bsscfg,
			ampdu_tx->cfg_cubby_handle);
		int idx;

		txaggr->control_all_tid = 0;
		for (idx = 0; idx < AMPDU_MAX_SCB_TID; idx++) {
			txaggr->control[idx].tid = (uint8) idx;
			txaggr->control[idx].enable = cfg_cubby->txaggr[idx];
		}
		bcopy(txaggr, a, sizeof(*txaggr));
		}
		break;
	case IOV_SVAL(IOV_AMPDU_CFG_TXAGGR): {
		struct ampdu_cfg_txaggr *txaggr = (struct ampdu_cfg_txaggr *)a;
		struct ampdu_tx_cfg_cubby *cfg_cubby = BSSCFG_CUBBY(bsscfg,
			ampdu_tx->cfg_cubby_handle);
		int idx;

		if (txaggr->control_all_tid == 1) {
			if (!txaggr->enable)
				wlc_ampdu_tx_cleanup(wlc, bsscfg, wlc->ampdu_tx);
			for (idx = 0; idx < AMPDU_MAX_SCB_TID; idx++)
				cfg_cubby->txaggr[idx] = txaggr->enable;
		} else {
			for (idx = 0; idx < AMPDU_MAX_SCB_TID; idx++) {
				struct ampdu_tid_control *ampdu_tid = &txaggr->control[idx];

				if (ampdu_tid->tid >= AMPDU_MAX_SCB_TID) {
					continue;
				}
				if (ampdu_tx->ini_enable[ampdu_tid->tid] &&
					!ampdu_tid->enable) {
					wlc_ampdu_tx_cleanup_per_tid(wlc, bsscfg, wlc->ampdu_tx,
						ampdu_tid->tid);
				}
				cfg_cubby->txaggr[ampdu_tid->tid] = ampdu_tid->enable;
			}
		}
		}
		break;
...
wlc_ampdu_agg_pktc(void *ctx, struct scb *scb, void *p, uint prec)
{
	...
	struct ampdu_tx_cfg_cubby *cfg_cubby = BSSCFG_CUBBY(SCB_BSSCFG(scb),
		ampdu_tx->cfg_cubby_handle);
	...
	if ((ampdu_tx->txaggr == OFF) ||
		(cfg_cubby->txaggr[tid] == 0) ||
		(WLPKTTAG(p)->flags3 & WLF3_TDLS_BYPASS_AMPDU)) {
		WL_AMPDU(("%s: tx agg off -- returning\n", __FUNCTION__));
		goto txmod_ampdu;
	}
...
{
	if (scb && ampdu_tx && (SCB_AMPDU(scb)) && (ampdu_tx->txaggr == ON)) {
		return (void*)SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	}
	return NULL;
}
                                  ...
                                  void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
	...
	*/
	...
	if (fix_rate) {
		...
		WLCNTADD(wlc->pub->_cnt->txretrans, tx_cnt[0] - txsucc_cnt[0]);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_total, txsucc_cnt[0]);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_retries, tx_cnt[0] - txsucc_cnt[0]);
#ifdef WL11K
		wlc_rrm_tscm_upd(scb, tid, OFFSETOF(rrm_tscm_t, msdu_tx), txsucc_cnt[0]);
#endif
		...
		WLCNTCONDADD(prec_cnt, prec_cnt->retry, tx_cnt[0] - txsucc_cnt[0]);
		WLCNTCONDADD(actq_cnt, actq_cnt->retry, tx_cnt[0] - txsucc_cnt[0]);
		SCB_BS_DATA_CONDADD(bs_data_counters, retry, tx_cnt[0] - txsucc_cnt[0]);
		...
		WLCNTADD(wlc->pub->_cnt->txretrans, try_count - txsucc_cnt[0]);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_total, succ_count);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_retries, try_count - txsucc_cnt[0]);
#ifdef WL11K
		...
		WLCNTCONDADD(prec_cnt, prec_cnt->retry, try_count - succ_count);
		WLCNTCONDADD(actq_cnt, actq_cnt->retry, try_count - succ_count);
		SCB_BS_DATA_CONDADD(bs_data_counters, retry, try_count - succ_count);
	...
	uint32 s1, uint32 s2, uint32 s3, uint32 s4)
{
	...
	if (AMPDU_HOST_ENAB(wlc->pub)) {
		...
		WLCNTADD(wlc->pub->_cnt->txretrans, (tx_mrt - txsucc_mrt) + tx_fbr);
		WLCNTADD(wlc->pub->_cnt->txfail, ncons - (txsucc_mrt + txsucc_fbr));
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_total, txsucc_mrt + txsucc_fbr);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_retries, (tx_mrt - txsucc_mrt) + tx_fbr);
		WLCNTSCBADD(scb->scb_ext_stats.tx_pkts_retry_exhausted,
			ncons - (txsucc_mrt + txsucc_fbr));
		WLCNTSCBADD(scb->scb_stats.tx_failures, ncons - (txsucc_mrt + txsucc_fbr));
#ifdef WL11K
...
/* bsscfg up/down */
static void
wlc_ampdu_bss_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{

	ampdu_tx_info_t *ampdu_info = (ampdu_tx_info_t *)ctx;
	wlc_bsscfg_t *cfg;
	int idx;
	ASSERT(evt != NULL);
	...
	if (evt->up) {
		struct ampdu_tx_cfg_cubby *cfg_cubby = BSSCFG_CUBBY(cfg,
			ampdu_info->cfg_cubby_handle);
		for (idx = 0; idx < AMPDU_MAX_SCB_TID; idx++)
			cfg_cubby->txaggr[idx] = 1;
	}
}
