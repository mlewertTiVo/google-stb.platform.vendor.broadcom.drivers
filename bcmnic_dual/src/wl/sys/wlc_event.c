 ...
 *
 * $Id: wlc_event.c 567606 2015-06-30 02:01:22Z $
 */
        ...
        void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_SVAL(IOV_EVENT_MSGS): {
			err = wlc_eventq_register_ind(wlc->eventq, arg, len);
			break;
		}
...
#ifndef WLNOEIND
int
wlc_eventq_register_ind(wlc_eventq_t *eq, void *bitvect, uint32 len)
{
	bcopy(bitvect, eq->event_inds_mask,
		MIN(sizeof(eq->event_inds_mask), len));
