...
wlc_mcnx_bssid_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	if (D11REV_GE(mcnx->wlc->pub->corerev, 40) &&
		...
		/* For corerev >= 40, clear ADDR_BMP_BSSID if it is not infra STA BSS.
		 * If the RA == BSSID (e.g. AP, GO) and the flag is set, AC ucode
		 * thinks that A1 match is only BSSID match, not RA match, thus discarding the pkt.
		 */
		val &= ~ADDR_BMP_BSSID;
	}
...
wlc_mcnx_ra_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	int idx;
	int bss;
	uint16 mask = ADDR_BMP_RA;
	uint16 val = ADDR_BMP_RA;
