 ...
 *
 * $Id: wlc_high_stubs.c 502782 2014-09-16 04:32:03Z $
 */
...
wlc_rpc_bmac_dispatch(wlc_rpc_ctx_t *rpc_ctx, struct rpc_buf* buf)
{
	...
	case WLRPC_WLC_BMAC_RADAR_DETECT_ID: {
		int32 radar_on;

		err = bcm_xdr_unpack_int32(&b, &radar_on);
		ASSERT(!err);

		wlc_bmac_radar_detect_upd(wlc_hw, (bool)radar_on);
		break;
	}
