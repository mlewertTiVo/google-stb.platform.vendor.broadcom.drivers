...
enum {
	...
	IOV_DFS_NOP_SEC,	/* nop time in sec */
	IOV_LAST
};
...
static const bcm_iovar_t wlc_dfs_iovars[] = {
	...
	{"dfs_nop_sec", IOV_DFS_NOP_SEC, 0, IOVT_UINT32, 0},
	{NULL, 0, 0, 0, 0}
};
...
wlc_dfs_chanspec(wlc_dfs_info_t *dfs)
{
	...
	if (dfs->dfs_cac.chanspec_forced_list) {
		...
		for (i = 0; i < dfs->dfs_cac.chanspec_forced_list->num; i++) {
			...
			if (wlc_valid_dfs_chanspec(dfs, chspec)) {
 ...
 */
bool
wlc_valid_dfs_chanspec(wlc_dfs_info_t *dfs, chanspec_t chspec)
{
 ...
 wlc_dfs_info_t *dfs, uint txstatus)
{
	...
	if (txstatus & TX_STATUS_ACK_RCV) {
		...
		dfs->dfs_cac.actionfr_retry_counter = 0;
	}
	else if ((txstatus & TX_STATUS_MASK) == TX_STATUS_NO_ACK)
	{
		WL_DFS(("ACK NOT RECEIVED... retrying....%d\n",
			dfs->dfs_cac.actionfr_retry_counter+1));
		if (dfs->dfs_cac.actionfr_retry_counter < WLC_RADAR_REPORT_RETRY_LIMIT) {
			dfs->dfs_cac.actionfr_retry_counter++;
