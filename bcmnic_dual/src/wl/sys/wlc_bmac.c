...
static uint16
BCMATTACHFN(wlc_bmac_dma_max_outstread)(wlc_hw_info_t *wlc_hw, uint bustype)
{

	if (bustype != SI_BUS)
		return DMA_MR_1;
	else if (wlc_hw->sih->buscoretype != PCIE2_CORE_ID)
		return DMA_MR_1;
		...
		if (D11REV_GE(wlc_hw->corerev, 32)) {
			...
			if (D11REV_GE(wlc_hw->corerev, 32)) {
				if ((CHIPID(wlc_hw->sih->chip) == BCM4360_CHIP_ID) ||
					...
					dma_param_set(di, HNDDMA_PID_TX_MULTI_OUTSTD_RD,
						wlc_bmac_dma_max_outstread(wlc_hw, bustype));
...
good_phy:
	...
	/* set Legacy BT Coexistence default mode */
...
wlc_bmac_templateptr_wreg(wlc_hw_info_t *wlc_hw, int offset)
{
	 ...
	 * global struct. In fact this should be done inside
...
#ifdef UNRELEASEDCHIP
	/* REMOVE WHEN READY */
	if ((CHIPID(wlc_hw->sih->chip) != BCM43430_CHIP_ID)) {
#endif
...
wlc_bmac_wakeucode_dnlddone(wlc_hw_info_t *wlc_hw)
{
	...
	si_gpiocontrol(wlc_hw->sih, 1 << wlc_hw->wowl_gpio,
	1 << wlc_hw->wowl_gpio, GPIO_DRV_PRIORITY);
...
wlc_bmac_uflush_tx_fifos(wlc_hw_info_t *wlc_hw, uint fifo_bitmap)
{
	...
	*/
	for (i = 0, fbmp = fifo_bitmap; fbmp; i++, fbmp = fbmp >> 1) {
		...
		/* no need to update wlc_hw->suspended_fifos because dma_txsuspend is called
		 * hereafter
		 */
	...
	void *params, uint p_len, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_BMAC_PCIEASPM): {
		...
		*/
		...
		L1substate = si_pcie_get_L1substate(wlc_hw->sih);
#endif
		*ret_int_ptr = ((clkreq & 0x1) << 8) |
			((L1substate & 0x3) << 2) |
			(aspm & PCIE_ASPM_ENAB);
		break;
	}
	...
	case IOV_SVAL(IOV_BMAC_PCIEASPM): {
		...
		si_pcie_set_L1substate(wlc_hw->sih, ((int_val & 0xc) >> 2));
#endif
		break;
	}
...
#define D11MAC_BMC_BUFSIZE_512BLOCK	1     /* 1 = 512B, 0 = 256B */
...
#ifdef WL_LTR
void wlc_ltr_init_reg(si_t *sih, uint32 reg, uint32 latency, uint32 unit)
{
	uint32 val;
	...
	WL_PS(("%s: rd[%d]=0x%x wr[%d]=0x%x\n", __FUNCTION__,
		reg, si_pcieltr_reg(sih, reg, 0, 0), reg, val));
	si_pcieltr_reg(sih, reg, 1, val);
}
...
wlc_ltr_init(wlc_info_t *wlc)
{
	 ...
	 */
	 wlc_ltr_init_reg(sih, PCIE_CAP_LTR0_REG, wlc->ltr_info.active_lat, LTR_SCALE_US);
	 wlc_ltr_init_reg(sih, PCIE_CAP_LTR1_REG, wlc->ltr_info.active_idle_lat, LTR_SCALE_US);
	 wlc_ltr_init_reg(sih, PCIE_CAP_LTR2_REG, wlc->ltr_info.sleep_lat, LTR_SCALE_MS);
...
void wlc_bmac_radar_detect_upd(wlc_hw_info_t *wlc_hw, bool radar_on)
{
	wlc_phy_radar_detect_on_off_cfg(((wlc_hw_info_t *)wlc_hw)->band->pi, radar_on);
}
