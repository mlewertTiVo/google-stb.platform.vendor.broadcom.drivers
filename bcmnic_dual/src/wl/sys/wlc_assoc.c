...
#if defined(WLMSG_ASSOC) || defined(WLMSG_INFORM)
...
#ifdef WLWNM
	if (WLWNM_ENAB(cfg->wlc->pub)) {
		wlc_wnm_process_join_trgts_bssload(wlc, bip, wlc->join_targets->count);
	}
#endif /* WLWNM */
...
wlc_join_chkdfs_cac_ism(wlc_bsscfg_t *cfg)
{
	...
	if (WL11H_STA_ENAB(wlc) && wlc_radar_chanspec(wlc->cmi, bi->chanspec)) {
			...
			wlc_enable_mac(wlc);
			ASSERT(pih);
			wlc_phy_radar_detect_enable(pih, TRUE);
...
wlc_join_bss_start(wlc_bsscfg_t *cfg)
{
	...
	/* When a STA tries to join an AP on non-radar channel disable the PHY radar detection.
	 * This can happen when a STA (associated to 5G AP on radar channel) tries to join
	 * another 2G AP or any other 5G AP on non-radar channel.
	 */
#if defined(STA) && defined(SLAVE_RADAR)
		if (WL11H_STA_ENAB(wlc) &&
			!wlc_radar_chanspec(wlc->cmi, wlc->join_targets->ptrs[wlc->join_targets_last]->chanspec)) {
			wlc_phy_t *pih = wlc->band->pi;
			wlc_phy_radar_detect_enable(pih, FALSE);
		}
	...
	wlc_bss_info_t *bi, bool associated)
{
	...
	if (N_ENAB(wlc->pub)) {
		if ((wsec_scb == WEP_ENABLED || wsec_scb == TKIP_ENABLED) &&
			...
			if (IS_BTCX_FULLTDM(wlc_btc_mode_get(wlc))) {
				if (wlc_btc_get_bth_period(wlc) &&
				    wlc_btc_get_bth_period(wlc) < BT_AMPDU_THRESH) {
					/* wlc_txmod_config() will make both
					 *  Tx ON, which is NOT desired if BT in ON
					 */
					wlc_ampdu_agg_state_update_tx(wlc, cfg, OFF);
				}
			}
#endif /* WLAMPDU */
		}
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
	...
	if ((BSSCFG_IS_MFP_CAPABLE(cfg)) &&
		(cfg->target_bss->wpa2.flags & RSN_FLAGS_MFPC))
			scb->flags2 |= SCB2_MFP;
	if ((cfg->WPA_auth & WPA2_AUTH_SHA256) &&
...
wlc_roamscan_start(wlc_bsscfg_t *cfg, uint roam_reason)
{
	...
	if (roam->partialscan_period) {
		...
		if (!roam->active && !roam->cache_valid)
			roamscan_new = TRUE;
		else
			roamscan_new = FALSE;
