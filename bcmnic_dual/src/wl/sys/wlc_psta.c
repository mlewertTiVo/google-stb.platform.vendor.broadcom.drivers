 ...
 *
 * $Id: wlc_psta.c 573629 2015-07-23 07:54:55Z $
 */
...
struct wlc_psta_info {
	...
	uint8		psta_force_vmac;	/* Force to use virtual MAC */
#ifdef DPSTA
...
BCMATTACHFN(wlc_psta_attach)(wlc_info_t *wlc)
{
	...
	var = getvar(NULL, "psta_force_vmac");
	if (var) {
		psta->psta_force_vmac = bcm_strtoul(var, NULL, 0);
		WL_PSTA(("wl%d: psta_force_vmac %d\n", psta->pub->unit, psta->psta_force_vmac));
	}
...
wlc_psta_watchdog(void *arg)
{
	...
	if ((psta->wlc->pub->now & 31) == 0) {
		wlc_sendnulldata(psta->wlc, cfg, &cfg->current_bss->BSSID, 0, 0, PRIO_8021D_BE);
	}
                ...
                wlc_bsscfg_t *pcfg)
{
	...
	if (cfg == NULL) {
		...
		if (PSTA_IS_REPEATER(psta->wlc) ||
			(PSTA_IS_PROXY(psta->wlc) && psta->psta_force_vmac)) {
			if (psta->mrpt)
