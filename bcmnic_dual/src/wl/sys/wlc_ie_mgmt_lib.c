	...
	wlc_iem_uiel_t *uiel, wlc_iem_cbparm_t *cbparm)
{
	...
	for (i = 0; i < build_cnt; i ++) {
		...
		if (uiel != NULL) {
			...
			wlc_iem_proc_uiel(cfg, ft, nocbvec,
			        prev, tag, is_tag, uiel, cbparm,
			        wlc_iem_calc_ins_uie_len, wlc_iem_calc_mod_uie_len, &ucbp,
			        &done);
			...
			if (l > 0) {
				IEM_TRACE(("%s: uiel %u len %u ttl %u\n", __FUNCTION__, i, l, ll));
			}
		...
		calc.ie = NULL;
		calc.ie_len = 0;
		ASSERT(build_cb[i].calc != NULL);
		l = (build_cb[i].calc)(build_cb[i].ctx, &calc);
		ll += l;
		IEM_TRACE(("%s: len %u ttl %u\n", __FUNCTION__, l, ll));
	...
	uint8 *buf, uint buf_len)
{
	...
	for (i = 0; i < build_cnt; i ++) {
		...
		if (uiel != NULL) {
			...
			if ((err = wlc_iem_proc_uiel(cfg, ft, nocbvec,
			                prev, tag, is_tag, uiel, cbparm,
			                wlc_iem_build_ins_uie, wlc_iem_build_mod_uie, &ucbp,
			                &done)) != BCME_OK) {
				WL_ERROR(("%s: err %d\n", __FUNCTION__, err));
				return err;
			}
		...
		buf1 = build.buf;
		buf_len1 = build.buf_len;
		ASSERT(build_cb[i].build != NULL);
		err = (build_cb[i].build)(build_cb[i].ctx, &build);
		if (err != BCME_OK) {
			WL_ERROR(("%s: err %d\n", __FUNCTION__, err));
			return err;
		}
		build.buf = buf1 + l;
		build.buf_len = buf_len1 - l;
	...
	if (IEM_DUMP_ON()) {
		...
		prhex(name, buf, (uint)(build.buf - buf));
	}
