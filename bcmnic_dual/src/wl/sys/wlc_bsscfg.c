 ...
 *
 * $Id: wlc_bsscfg.c 636417 2016-05-09 12:03:55Z $
 */
...
wlc_bsscfg_up(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_AP(cfg)) {
		...
		if (BSSCFG_AP(cfg) && radar_chan) {
			/* Reconfigure radar thresholds for dfs channels */
			wlc_phy_radar_detect_enable(wlc->band->pi, TRUE);
		}
...
_wlc_bsscfg_init(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct ether_addr *ea, uint flags, bool ap)
{
	...
	if (L2_FILTER_ENAB(wlc->pub)) {
		if (wlc_l2_filter_proxy_arp_enab(wlc, bsscfg) && ap) {
