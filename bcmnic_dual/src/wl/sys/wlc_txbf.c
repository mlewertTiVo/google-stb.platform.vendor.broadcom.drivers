...
wlc_txbf_schedule(wlc_txbf_info_t *txbf)
{
	...
	for (i = 0; i < txbf->max_link_ext; i++) {
		...
		if (bfi->tx_pkts - bfi->last_tx_pkts >= txbf->pkt_thre_sched)
			bfi->no_traffic = 0;
		else
			bfi->no_traffic = 1;

		bfi->last_tx_pkts = bfi->tx_pkts;
	}
...
wlc_txbf_sched_timer(void *arg)
{
	...
	for (i = 0; i < txbf->max_link_ext; i++) {
		...
		if (txbf->sched_msg & TXBF_SCHED_MSG_SCB) {
			...
			WL_PRINT(("[%2d] %08x %s, %d %d %d, %d %08x, %d %d, %d %d, %d %d\n",
...
wlc_txbf_up(void *context)
{
	...
	*/
	...
	if (txbf->sched_timer_interval != 0) {
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_TXBF_SCHED_TIMER):
		if (txbf->sched_timer_enb && txbf->sched_timer_interval != 0)
			wl_del_timer(wlc->wl, txbf->sched_timer);
...
void BCMFASTPATH
wlc_txbf_increase_tx_pkt_cnt(wlc_txbf_info_t *txbf, struct scb *scb)
{
	struct txbf_scb_info *txbf_scb_info;
	...
	if (scb->psta_prim) {
		txbf_scb_info = (struct txbf_scb_info *)TXBF_SCB_INFO(txbf, scb->psta_prim);
		txbf_scb_info->tx_pkts ++;
	}
}
